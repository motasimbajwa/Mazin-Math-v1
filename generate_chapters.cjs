...
* **Figure Reference:** Interactive plot.
...
const ch14_9 = `  {
    ...
    subheadings: [
      ...
      { id: "ch14-9-cheatsheet", title: "Cheat Sheet" },
...
if (spliceIndex !== -1) {
  const beginning = content.slice(0, spliceIndex + 4);
  const newContent = beginning + ",\n" + ch14_7 + ",\n" + ch14_8 + ",\n" + ch14_9 + ",\n" + ch15_1 + ",\n" + ch15_2 + "\n];\n";
  fs.writeFileSync('src/data.ts', newContent);
  console.log("Success");
} else {
  console.log("Failed: Could not find insertion point.");
}
