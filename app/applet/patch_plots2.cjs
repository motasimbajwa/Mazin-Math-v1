...
const replacements = {
  ...
  'gradient_field': `
      tourSteps = [
        { text: "This plot shows the contour lines of the saddle function $f(x,y)=x^2 - y^2$." },
        { 
          text: "The gradient $\\\\nabla f = \\\\langle 2x, -2y \\\\rangle$ is drawn as arrows. Notice that every arrow points in the direction of steepest ascent.",
          changes: { layout: { annotations: [{ x: 1, y: 0, text: 'Steepest Uphill Direction', showarrow: true, arrowhead: 2, arrowcolor: 'red', ax: 50, ay: -30, bgcolor: 'white' }] } }
        },
        { 
          text: "At each point, the gradient vector is entirely perfectly perpendicular (orthogonal) to the contour line!",
          changes: { layout: { annotations: [{ x: 1, y: 1, text: 'Orthogonal Intersections', showarrow: true, arrowhead: 2, arrowcolor: 'green', ax: 50, ay: -50, bgcolor: 'white' }] } }
        },
        { text: "If you were at a point on a contour, walking exactly along the gradient direction would take you to higher values fastest." }
      ];`
};
...
for (const [caseName, newCode] of Object.entries(replacements)) {
  ...
  if(endSteps !== -1) {
     ...
     console.error("Could not find end of tourSteps for", caseName);
  }
}
