...
2. From second eq, either \\(y = 0\\) or \\(\\lambda = 1\\).
   ...
   **Case B: \\(\\lambda = 1\\)**
   ...
   If \\(x=2/3\\), \\(y^2 = 1 - 4/9 = 5/9 \implies y = \pm \frac{\sqrt{5}}{3}\\). Points (2/3, ±√5/3).
...
**Conclusion**: Minimum is -1 at (-1, 0). Maximum is 1 at (1,0) and (0, ±1).
</details>
...
3. **Conclusion**:
   Maximum is 12 at (0, ±2). Minimum is 2 at (±1, 0).
</details>
`;
...
function replaceExamsStr(str, targetRe, replacement) {
    if (str.match(targetRe)) {
        return str.replace(targetRe, replacement);
    }
    return str;
}

// Due to tricky formatting of the appended ones, let's just use string operations carefully
// I will slice off everything from "## Mock Exams" to the end of the markdown literal, and replace it.
