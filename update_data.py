...
def update_data():
    ...
    # Chapter 14.1 replacement
    new_14_1 = r"""## Introduction: Escaping Flatland
...
"""
    ...
    # We need to replace the content of chapter-14-1
    # We find the index of `id: "chapter-14-1"` and then replace the `markdown: \`...\`` part.

    idx_ch14_1_start = content.find('id: "chapter-14-1"')
    md_start = content.find('markdown: `', idx_ch14_1_start)
    md_end = content.find('\n`', md_start) + 2

    content = content[:md_start] + 'markdown: `\n' + new_14_1.replace('\\', '\\\\') + '`' + content[md_end:]

    # For 14.2 append the questions before `\n<br/>\n` or `\n` at the end
    # I will replace the old assessment block for the rest of chapters.
    # The prompt says: "add these to the existing 3" and "insert at the end of their respective markdowns, replacing the old 3-question block.)" -> wait, if I add to existing 3, it should just append them before the `Course Progress` line.
    
    adds = {
        '14-2': r"""
> **QUIZ:** Use polar coordinates to evaluate lim_{(x,y)->(0,0)} (x² y)/(x² + y²).
>
...
""",
        '14-3': r"""
> **QUIZ:** Find f_xy and f_yx for f(x,y) = e^{xy} sin(x+y). Verify Clairaut’s theorem.
>
...
""",
        '14-7': r"""
> **QUIZ:** Find all critical points of f(x,y) = x³ + y³ - 3xy and classify them.
>
...
"""
    ...
    # In my adds string above, I used \(...\) but the actual formatting in the data is $...$ or similar. I'll replace `\(` with `$` and `\)` with `$`. Wait, the prompt used `\(`? No, the prompt used `\$` because it escapes the slashes. No, wait, in JS, `\\(` gives `\(`. Let me check the user prompt explicitly. The user prompt used `\\lim`, so `\\`. I'll replace it correctly. The prompt has: `$\\lim_{(x,y)\\to(0,0)} \\frac{x^2 y}{x^2 + y^2}$`. Okay, so `$` and escaping slashes!
    # Wait, in the raw python string with `r"""`, a backslash stays a backslash. But since I'm writing it into a javascript template literal, I need to escape the backslashes to `\\` and use `$`. Let's just fix it automatically.
    
    for ch_suffix in ['14-2', '14-3', '14-4', '14-5', '14-6', '14-7']:
        chapter_id = f'id: "chapter-{ch_suffix}"'
        idx_ch_start = content.find(chapter_id)
        if idx_ch_start == -1:
            print(f"Could not find {chapter_id}")
            continue
        
        md_end_for_ch = content.find('\n<br/>', idx_ch_start)
        
        text_to_insert = adds[ch_suffix]
        text_to_insert_js = text_to_insert.replace('\\', '\\\\').replace('\\(', '$').replace('\\)', '$')
        
        content = content[:md_end_for_ch] + text_to_insert_js + content[md_end_for_ch:]
