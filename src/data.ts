...
**The Square's Area:**
Imagine a literal square. The area of the square is the length of its side multiplied by itself. To express the area as a function of the side, let y be the area, x the side, then write y = x². Here, y depends entirely on x. 
**The Magic Black Box:**
...
**Formal Set Mapping:**
A function f: X → Y is a rule that assigns to each element x ∈ X exactly one element y ∈ Y.
- X is the **domain**.
  ...
  <table class="w-full text-left border-collapse min-w-[500px]">
    ...
    <tbody class="bg-white">
      <tr class="">
        ...
        <td class="p-3 border-b text-sm">Check for denominators (cannot be 0) and even roots (must be ≥ 0).</td>
...
<div class="grid grid-cols-2 gap-4 mt-2 border-t border-slate-200 pt-2 min-w-[600px]">
  ...
  <div class="text-sm font-mono bg-slate-100 p-1 rounded whitespace-normal">A = s × s</div>
...
<div class="grid grid-cols-2 gap-4 mt-2 border-t border-slate-200 pt-2 min-w-[600px]">
  ...
  <div class="col-span-2 border-t border-slate-100"></div>
  <div class="text-sm text-slate-700">Use the Pythagorean theorem for an isosceles right triangle: d² = s² + s².</div>
  <div class="text-sm font-mono bg-slate-100 p-1 rounded whitespace-normal">d² = 2s²</div>

  <div class="col-span-2 border-t border-slate-100"></div>
  <div class="text-sm text-slate-700">Take the positive square root because length is positive.</div>
  <div class="text-sm font-mono bg-slate-100 p-1 rounded whitespace-normal">d(s) = s√(2)</div>
...
**Question:** Find the domain of y = √(x-2).
...
<div class="grid grid-cols-2 gap-4 mt-2 border-t border-slate-200 pt-2 min-w-[600px]">
  ...
  <div class="text-sm font-mono bg-slate-100 p-1 rounded whitespace-normal">x - 2 ≥ 0</div>

  <div class="col-span-2 border-t border-slate-100"></div>
  <div class="text-sm text-slate-700">Solve the inequality.</div>
  <div class="text-sm font-mono bg-slate-100 p-1 rounded whitespace-normal">x ≥ 2
Domain: [2, ∞)</div>
...
<div class="grid grid-cols-2 gap-4 mt-2 border-t border-slate-200 pt-2 min-w-[600px]">
  ...
  <div class="text-sm font-mono bg-slate-100 p-1 rounded whitespace-normal">x - 5 ≠ 0</div>
...
<div class="grid grid-cols-2 gap-4 mt-2 border-t border-slate-200 pt-2 min-w-[600px]">
  ...
  <div class="col-span-2 border-t border-slate-100"></div>
  <div class="text-sm text-slate-700">Numerator root requires x ≥ 0.</div>
  <div class="text-sm font-mono bg-slate-100 p-1 rounded whitespace-normal">x ≥ 0</div>

  <div class="col-span-2 border-t border-slate-100"></div>
  <div class="text-sm text-slate-700">Denominator requires x - 1 ≠ 0.</div>
  <div class="text-sm font-mono bg-slate-100 p-1 rounded whitespace-normal">x ≠ 1</div>

  <div class="col-span-2 border-t border-slate-100"></div>
  <div class="text-sm text-slate-700">Combine both conditions.</div>
  <div class="text-sm font-mono bg-slate-100 p-1 rounded whitespace-normal">Domain: [0, 1) ∪ (1, ∞)</div>
...
<div class="grid grid-cols-2 gap-4 mt-2 border-t border-slate-200 pt-2 min-w-[600px]">
  ...
  <div class="text-sm font-mono bg-slate-100 p-1 rounded whitespace-normal">V = (4)/(3)π r³</div>

  <div class="col-span-2 border-t border-slate-100"></div>
  <div class="text-sm text-slate-700">Write it explicitly as V(r).</div>
  <div class="text-sm font-mono bg-slate-100 p-1 rounded whitespace-normal">V(r) = (4)/(3)π r³</div>
...
**Question:** The period T of a simple pendulum relates to its length L by T = 2π√((L)/(g)). Identify the independent and dependent variables.
...
**Question:** Does the equation x² + y² = 9 represent y as a function of x?
...
<div class="grid grid-cols-2 gap-4 mt-2 border-t border-slate-200 pt-2 min-w-[600px]">
  ...
  <div class="text-sm font-mono bg-slate-100 p-1 rounded whitespace-normal">y² = 9 - x² implies y = ±√(9 - x²)</div>
...
**Question:** Given f(x) = begin{cases} x² & x < 0 \\ 2x + 1 & x ≥ 0 end{cases}, find f(-2) and f(0).

| Inner voice | What you write |
| :--- | :--- |
| "For x = -2, the condition x < 0 is true. I use the first piece." | f(-2) = (-2)² = 4 |
| "For x = 0, the condition x ≥ 0 is true. I use the second piece." | f(0) = 2(0) + 1 = 1 |
...
<div class="user-quiz" data-question="3. True or False: y² = x is a function of x."></div>
<div class="user-quiz" data-question="4. Find f(2) if f(x) = 3x² - x + 1."></div>
...
## ELI5 Explanation (Engineering Analogy)
**Zeno's Runner:**
Imagine a runner going from A to B. He first runs half the distance, then half the remaining distance, then again half the remaining distance, and so on. The distances he runs get smaller and smaller in the halving series (1)/(2) + (1)/(4) + (1)/(8) + dots. Distances from B approach zero as their limit! Does the runner ever reach the goal? In reality, motion is continuous and time also shrinks, meaning the sequence converges in a finite time.
**The Bouncing Ball:**
...
- **Divergent Series:** An infinite series that does not converge to a finite limit (e.g., it goes to ∞).
...
- **Harmonic Series:** The series 1 + (1)/(2) + (1)/(3) + (1)/(4) + dots which famously diverges to infinity.

## Why This Matters in Electrical Engineering
Limits prevent engineers from dividing by zero and causing universal explosions. In circuit analysis, what happens to the steady-state value of a capacitor voltage as time t → ∞? It's a limit! What happens to an inductor's impedance as frequency ω → 0? It's a limit! Limits allow us to mathematically predict the behavior of dynamic systems at the extreme boundaries (initial startup or infinite time) without physically simulating eternity.

## Formal Definitions and Notation
**Formal Definition of a Limit (The epsilon - δ definition):**
lim _x → a f(x) = L means that for every real number epsilon > 0, there exists a real number δ > 0 such that for all x, if 0 < |x - a| < δ, then |f(x) - L| < epsilon.
**In English:** As x gets arbitrarily close to a (but not exactly a), f(x) gets arbitrarily close to L.

**For Series:**
The limit of an infinite sum S = sum_n=1^∞ a_n is defined as the limit of its partial sums as N → ∞.

## Step\u2011by\u2011Step Method for Problem Solving
1. **Direct Substitution:** Always try plugging in the objective value first. If it yields a real number, that is your limit.
2. **Handle Indeterminate Forms:** If you get 0/0 or ∞/∞, the limit might still exist! You must use algebraic manipulation (factoring, conjugate) or L'Hôpital's Rule.
3. **Analyze Infinite Bounds:** If the denominator shrinks to 0 while the numerator is a constant (e.g., 1/0), the limit goes to ±∞ or does not exist.
4. **For Series:** Check if it's a geometric series. If it is a + ar + ar² + dots, and |r| < 1, the limit is (a)/(1 - r).
  ...
  <table class="w-full text-left border-collapse min-w-[500px]">
    ...
    <tbody class="bg-white">
      <tr class="">
        ...
        <td class="p-3 border-b text-sm">Take the limit of the function as x → ∞ and x → -∞.</td>
      </tr>
    </tbody>
  </table>
</div>
...
<div class="grid grid-cols-2 gap-4 mt-2 border-t border-slate-200 pt-2 min-w-[600px]">
  ...
  <div class="text-sm font-mono bg-slate-100 p-1 rounded whitespace-normal">S = (a)/(1-r)</div>

  <div class="col-span-2 border-t border-slate-100"></div>
  <div class="text-sm text-slate-700">Substitute the values.</div>
  <div class="text-sm font-mono bg-slate-100 p-1 rounded whitespace-normal">S = (1/2)/(1 - 1/2) = (1/2)/(1/2) = 1</div>
...
**Question:** Evaluate lim _x → 3 (x² + 2x - 5).
...
<div class="grid grid-cols-2 gap-4 mt-2 border-t border-slate-200 pt-2 min-w-[600px]">
  ...
  <div class="text-sm font-mono bg-slate-100 p-1 rounded whitespace-normal">lim _x→ 3 (x²+2x-5) = 3² + 2(3) - 5</div>
...
**Question:** Evaluate lim _x → 2 (x² - 4)/(x - 2).
...
<div class="grid grid-cols-2 gap-4 mt-2 border-t border-slate-200 pt-2 min-w-[600px]">
  ...
  <div class="text-sm font-mono bg-slate-100 p-1 rounded whitespace-normal">lim _x → 2 ((x-2)(x+2))/(x-2)</div>

  <div class="col-span-2 border-t border-slate-100"></div>
  <div class="text-sm text-slate-700">Cancel the common (x-2) terms, then plug in.</div>
  <div class="text-sm font-mono bg-slate-100 p-1 rounded whitespace-normal">lim _x → 2 (x+2) = 2 + 2 = 4</div>
...
**Question:** Evaluate lim _x → ∞ (3x² + 5x)/(2x² - 1).
...
<div class="grid grid-cols-2 gap-4 mt-2 border-t border-slate-200 pt-2 min-w-[600px]">
  ...
  <div class="col-span-2 border-t border-slate-100"></div>
  <div class="text-sm text-slate-700">As x → ∞, only the highest power terms matter. Divide everything by x².</div>
  <div class="text-sm font-mono bg-slate-100 p-1 rounded whitespace-normal">lim _x → ∞ (3 + 5/x)/(2 - 1/x²)</div>

  <div class="col-span-2 border-t border-slate-100"></div>
  <div class="text-sm text-slate-700">Any number over ∞ goes to 0.</div>
  <div class="text-sm font-mono bg-slate-100 p-1 rounded whitespace-normal">=(3 + 0)/(2 - 0) = (3)/(2)</div>
...
<div class="grid grid-cols-2 gap-4 mt-2 border-t border-slate-200 pt-2 min-w-[600px]">
  ...
  <div class="col-span-2 border-t border-slate-100"></div>
  <div class="text-sm text-slate-700">Let's treat the bounces (up and down) as a series: 6 + 6(3/4) + 6(3/4)²dots</div>
  <div class="text-sm font-mono bg-slate-100 p-1 rounded whitespace-normal">a=6, r=(3)/(4)</div>

  <div class="col-span-2 border-t border-slate-100"></div>
  <div class="text-sm text-slate-700">Sum the infinite bounces, then add the initial 4ft drop.</div>
  <div class="text-sm font-mono bg-slate-100 p-1 rounded whitespace-normal">(6)/(1 - 3/4) = (6)/(1/4) = 24
...
<div class="grid grid-cols-2 gap-4 mt-2 border-t border-slate-200 pt-2 min-w-[600px]">
  ...
  <div class="text-sm font-mono bg-slate-100 p-1 rounded whitespace-normal">sum_n=1^∞ (1)/(n)</div>

  <div class="col-span-2 border-t border-slate-100"></div>
  <div class="text-sm text-slate-700">State the mathematical known fact.</div>
  <div class="text-sm font-mono bg-slate-100 p-1 rounded whitespace-normal">The series diverges to ∞.</div>
...
**Question:** Evaluate lim _x → 0 (√(x+9) - 3) / x.
...
<div class="grid grid-cols-2 gap-4 mt-2 border-t border-slate-200 pt-2 min-w-[600px]">
  ...
  <div class="col-span-2 border-t border-slate-100"></div>
  <div class="text-sm text-slate-700">Plugging in 0 gives 0/0. Multiply top and bottom by the conjugate √(x+9) + 3.</div>
  <div class="text-sm font-mono bg-slate-100 p-1 rounded whitespace-normal">(√(x+9) - 3)/x • (√(x+9) + 3)/(√(x+9) + 3)</div>

  <div class="col-span-2 border-t border-slate-100"></div>
  <div class="text-sm text-slate-700">Foil the top.</div>
  <div class="text-sm font-mono bg-slate-100 p-1 rounded whitespace-normal">= ((x+9)-9)/(x(√(x+9)+3)) = (x)/(x(√(x+9)+3))</div>

  <div class="col-span-2 border-t border-slate-100"></div>
  <div class="text-sm text-slate-700">Cancel x and plug in 0.</div>
  <div class="text-sm font-mono bg-slate-100 p-1 rounded whitespace-normal">lim _x→ 0 (1)/(√(x+9)+3) = (1)/(√(9)+3) = (1)/(6)</div>
...
<div class="grid grid-cols-2 gap-4 mt-2 border-t border-slate-200 pt-2 min-w-[600px]">
  ...
  <div class="text-sm font-mono bg-slate-100 p-1 rounded whitespace-normal">1 / (tiny positive) → ∞</div>

  <div class="col-span-2 border-t border-slate-100"></div>
  <div class="text-sm text-slate-700">The value explodes to positive infinity.</div>
  <div class="text-sm font-mono bg-slate-100 p-1 rounded whitespace-normal">= ∞</div>
...
<div class="grid grid-cols-2 gap-4 mt-2 border-t border-slate-200 pt-2 min-w-[600px]">
  ...
  <div class="text-sm font-mono bg-slate-100 p-1 rounded whitespace-normal">S = (3/10)/(1 - 1/10) = (3/10)/(9/10) = (3)/(9) = (1)/(3)</div>
...
**Question:** Find lim _x → 0 (sin  x)/(x).
...
<div class="grid grid-cols-2 gap-4 mt-2 border-t border-slate-200 pt-2 min-w-[600px]">
  ...
  <div class="text-sm font-mono bg-slate-100 p-1 rounded whitespace-normal">lim _x→ 0(sin  x)/(x) = 1</div>
...
**Question:** Find the sum of the series sum_n=0^∞ 3 ≤ft((1)/(2)right)^n.

| Inner voice | What you write |
| :--- | :--- |
| "This is a geometric series. The first term a is 3 (when n=0)." | a = 3 |
| "The common ratio r is 1/2. Since |r| < 1, it converges. Use S = a / (1 - r)." | S = (3)/(1 - 1/2) = 6 |
...
| lim _n → ∞ a_n eq 0 | Does the sequence approach zero? | Divergence Test: Series diverges. |

</div>
</div>## Chapter Assessment
<div class="user-quiz" data-question="1. Evaluate lim _x → 5 (2x - 1)."></div>
<div class="user-quiz" data-question="2. Evaluate lim _x → ∞ (4x³)/(2x³ - 7)."></div>
<div class="user-quiz" data-question="3. Sum the infinite series where a=5 and r=1/3."></div>
<div class="user-quiz" data-question="4. Evaluate lim _x → 0 (x²+3x)/(x)."></div>
...
This ratio (dy)/(dx) is called the differential coefficient or the **derivative**. It measures relative continuous growth.

<div class="plot-interactive my-6" data-plot-id="tangent_line"></div>

## ELI5 Explanation (Engineering Analogy)
**The Growing Square:** 
Imagine a monster living in Flatland that is born a square of side x. Its area is y = x². We let it grow at a steady rate. If its side grows a tiny bit (dx), its area grows by (dy). The ratio of its area growth to its side growth is exactly 2x. The mathematics says: if the square's side is 10 inches, its area is growing 2 × 10 = 20 times as fast as its edge is growing!
**The Speedometer:**
When you drive, you aren't doing point-to-point average math in your head. Your speedometer points to 45 mph. It is taking an infinitely small snapshot of your distance traveled (dy) over an infinitely small slice of time (dx) and displaying the derivative: (dy)/(dt).

## Glossary of Key Terms
- **Derivative:** The exact rate at which a dependent variable grows with respect to an independent variable. Symbolized as (dy)/(dx).
...
## Why This Matters in Electrical Engineering
Voltage V_L across an inductor is directly proportional to the **rate of change** of the current: V_L = L (di)/(dt). A derivative! Capacitor current I_C is proportional to the **rate of change** of voltage: I_C = C (dV)/(dt). The entire universe of electrical transients and differential equations hinges entirely upon calculating these derivatives using the power rule.

## Formal Definitions and Notation
**Formal Definition of the Derivative:**
(dy)/(dx) = lim _Δ x → 0 (f(x + Δ x) - f(x))/(Δ x)
...
**The Sum Rule:**
(d)/(dx) [f(x) + g(x)] = f'(x) + g'(x). You can just differentiate them one after the other.

## Step\u2011by\u2011Step Method for Problem Solving
1. **Prepare the Expression:** Convert all roots into fractional exponents (e.g. √(x) → x^1/2) and all bottom-heavy fractions into negative exponents (e.g. (1)/(x³) → x^-3).
...
<div class="grid grid-cols-2 gap-4 mt-2 border-t border-slate-200 pt-2 min-w-[600px]">
  ...
  <div class="text-sm font-mono bg-slate-100 p-1 rounded whitespace-normal">(dy)/(dx) = 3x^dots</div>

  <div class="col-span-2 border-t border-slate-100"></div>
  <div class="text-sm text-slate-700">Subtract 1 from the power 3-1 = 2.</div>
  <div class="text-sm font-mono bg-slate-100 p-1 rounded whitespace-normal">(dy)/(dx) = 3x²</div>
...
<div class="grid grid-cols-2 gap-4 mt-2 border-t border-slate-200 pt-2 min-w-[600px]">
  ...
  <div class="col-span-2 border-t border-slate-100"></div>
  <div class="text-sm text-slate-700">Bring the 4 down and multiply it by 7. (4 × 7 = 28).</div>
  <div class="text-sm font-mono bg-slate-100 p-1 rounded whitespace-normal">(dy)/(dx) = 28x^dots</div>

  <div class="col-span-2 border-t border-slate-100"></div>
  <div class="text-sm text-slate-700">Subtract 1 from the power 4-1 = 3.</div>
  <div class="text-sm font-mono bg-slate-100 p-1 rounded whitespace-normal">(dy)/(dx) = 28x³</div>
...
**Question:** Differentiate y = x² + x³ + 5.
...
<div class="grid grid-cols-2 gap-4 mt-2 border-t border-slate-200 pt-2 min-w-[600px]">
  ...
  <div class="text-sm font-mono bg-slate-100 p-1 rounded whitespace-normal">d/dx (x²) = 2x</div>

  <div class="col-span-2 border-t border-slate-100"></div>
  <div class="text-sm text-slate-700">Next term.</div>
  <div class="text-sm font-mono bg-slate-100 p-1 rounded whitespace-normal">d/dx (x³) = 3x²</div>
  ...
  <div class="text-sm font-mono bg-slate-100 p-1 rounded whitespace-normal">(dy)/(dx) = 2x + 3x²</div>
...
<div class="grid grid-cols-2 gap-4 mt-2 border-t border-slate-200 pt-2 min-w-[600px]">
  ...
  <div class="text-sm font-mono bg-slate-100 p-1 rounded whitespace-normal">(dy)/(dx) = -2x^dots</div>

  <div class="col-span-2 border-t border-slate-100"></div>
  <div class="text-sm text-slate-700">Subtract 1 from -2. Beware: -2 - 1 = -3.</div>
  <div class="text-sm font-mono bg-slate-100 p-1 rounded whitespace-normal">(dy)/(dx) = -2x^-3</div>

  <div class="col-span-2 border-t border-slate-100"></div>
  <div class="text-sm text-slate-700">Rewrite as a fraction if desired.</div>
  <div class="text-sm font-mono bg-slate-100 p-1 rounded whitespace-normal">(dy)/(dx) = -(2)/(x³)</div>
...
**Question:** Differentiate y = (5)/(x^4).
...
<div class="grid grid-cols-2 gap-4 mt-2 border-t border-slate-200 pt-2 min-w-[600px]">
  ...
  <div class="text-sm font-mono bg-slate-100 p-1 rounded whitespace-normal">(dy)/(dx) = -20x^-5 = -(20)/(x^5)</div>
...
<div class="grid grid-cols-2 gap-4 mt-2 border-t border-slate-200 pt-2 min-w-[600px]">
  ...
  <div class="text-sm font-mono bg-slate-100 p-1 rounded whitespace-normal">(dy)/(dx) = (1)/(2)x^-1/2</div>

  <div class="col-span-2 border-t border-slate-100"></div>
  <div class="text-sm text-slate-700">Rewrite nicely. Negative half power means square root on the bottom.</div>
  <div class="text-sm font-mono bg-slate-100 p-1 rounded whitespace-normal">(dy)/(dx) = (1)/(2√(x))</div>
...
<div class="grid grid-cols-2 gap-4 mt-2 border-t border-slate-200 pt-2 min-w-[600px]">
  ...
  <div class="text-sm font-mono bg-slate-100 p-1 rounded whitespace-normal">10 × 1 = 10</div>

  <div class="col-span-2 border-t border-slate-100"></div>
  <div class="text-sm text-slate-700">Reduce power: x^0 = 1. The x disappears.</div>
  <div class="text-sm font-mono bg-slate-100 p-1 rounded whitespace-normal">(dy)/(dx) = 10</div>
...
**Question:** Differentiate y = 3x^4 - (2)/(x) + 7√(x) - 42.
...
<div class="grid grid-cols-2 gap-4 mt-2 border-t border-slate-200 pt-2 min-w-[600px]">
  ...
  <div class="text-sm font-mono bg-slate-100 p-1 rounded whitespace-normal">12x³ - (-1)2x^-2 + (1)/(2)(7)x^-1/2 - 0</div>

  <div class="col-span-2 border-t border-slate-100"></div>
  <div class="text-sm text-slate-700">Clean it up.</div>
  <div class="text-sm font-mono bg-slate-100 p-1 rounded whitespace-normal">(dy)/(dx) = 12x³ + (2)/(x²) + (7)/(2√(x))</div>
...
**Question:** If distance s = 16t², find the velocity v (which is ds/dt).
...
<div class="grid grid-cols-2 gap-4 mt-2 border-t border-slate-200 pt-2 min-w-[600px]">
  ...
  <div class="text-sm font-mono bg-slate-100 p-1 rounded whitespace-normal">v = (ds)/(dt)</div>

  <div class="col-span-2 border-t border-slate-100"></div>
  <div class="text-sm text-slate-700">Apply power rule to 16t². Bring down 2.</div>
...
**Question:** Find slope of y = x² - 4x at the point x=3.
...
<div class="grid grid-cols-2 gap-4 mt-2 border-t border-slate-200 pt-2 min-w-[600px]">
  ...
  <div class="text-sm font-mono bg-slate-100 p-1 rounded whitespace-normal">(dy)/(dx) = 2x - 4</div>

  <div class="col-span-2 border-t border-slate-100"></div>
  <div class="text-sm text-slate-700">Plug the specific coordinate x=3 into the slope generator.</div>
  <div class="text-sm font-mono bg-slate-100 p-1 rounded whitespace-normal">(dy)/(dx) = 2(3) - 4 = 6</div>
...
**Question:** Find the derivative of f(x) = 4x³ - (1)/(x²).

| Inner voice | What you write |
| :--- | :--- |
| "Before differentiating, rewrite the fraction with a negative exponent so I can use the power rule." | f(x) = 4x³ - x^-2 |
| "Bring down the exponent, subtract one from the power." | f'(x) = 12x² - (-2)x^-3 = 12x² + (2)/(x³) |
...
| √(x) | Rewrite as x^1/2? | Apply power rule. |
...
<div class="user-quiz" data-question="3. Find (dy)/(dx) for y = 3x^-4."></div>
<div class="user-quiz" data-question="4. Differentiate y = 2x² + 5x - 7."></div>
<div class="user-quiz" data-question="5. Prepare (7)/(x³) into power notation."></div>
`
  },
{
    id: "chapter-4",
  ...
  },
  {
    ...
    markdown: `
## Introduction & Core Concepts
We have learned how to differentiate simple algebraical functions such as x²+c or ax^4, and we have now to consider how to tackle the product or quotient of two or more functions. 
A naive student might assume that to differentiate y = u × v, you just multiply the derivative of u by the derivative of v. **This is spectacularly wrong.**
If you have a growing rectangle where both the length u and height v are stretching by du and dv, the new area involves u • dv + v • du + du • dv. Since du • dv is infinitely small of the second order, it vanishes, leaving u • dv + v • du.
...
Imagine a metal sheet that is being heated. Its width u is expanding, and its height v is expanding. The new area isn't just the main square, it has two new "strips" added to the sides. One strip is (old width × new height change), which is u • v'. The other strip is (old height × new width change), which is v • u'. The total area grew by uv' + vu'. 
...
- **Quotient Rule:** The formula used to find the derivative of a function divided by another function: (u/v)' = (vu' - uv')/(v²).
- **Derivatives u' and v':** Shorthand notation for (du)/(dx) and (dv)/(dx).

## Why This Matters in Electrical Engineering
In circuit analysis, Instantaneous Power is P(t) = v(t) × i(t) (voltage times current). To find the maximum power delivery over time (which requires differentiating power), you MUST use the Product Rule since both voltage and current are functions of time changing simultaneously. Mixing them up causes grid failures.
...
**The Quotient Rule:**
If y = (u(x))/(v(x)), then:
(dy)/(dx) = (v frac(du)/(dx) - u (dv)/(dx))(v²)
Or simply: (u/v)' = (vu' - uv')/(v²).
...
3. Plug into (low • d(high) - high • d(low))/(low²).
  ...
  <table class="w-full text-left border-collapse min-w-[500px]">
    ...
    <tbody class="bg-white">
      <tr class="">
        ...
        <td class="p-3 border-b text-sm font-mono">Differentiate y = (x)/(x²+1)</td>
...
<div class="grid grid-cols-2 gap-4 mt-2 border-t border-slate-200 pt-2 min-w-[600px]">
  ...
  <div class="col-span-2 border-t border-slate-100"></div>
  <div class="text-sm text-slate-700">Identify u = x²+1 and v = x³+2. Find derivatives.</div>
  <div class="text-sm font-mono bg-slate-100 p-1 rounded whitespace-normal">u' = 2x quad v' = 3x²</div>

  <div class="col-span-2 border-t border-slate-100"></div>
  <div class="text-sm text-slate-700">Assemble uv' + vu'.</div>
  <div class="text-sm font-mono bg-slate-100 p-1 rounded whitespace-normal">(dy)/(dx) = (x²+1)(3x²) + (x³+2)(2x)</div>

  <div class="col-span-2 border-t border-slate-100"></div>
  <div class="text-sm text-slate-700">Expand and simplify.</div>
  <div class="text-sm font-mono bg-slate-100 p-1 rounded whitespace-normal">= 3x^4 + 3x² + 2x^4 + 4x = 5x^4 + 3x² + 4x</div>
...
<div class="grid grid-cols-2 gap-4 mt-2 border-t border-slate-200 pt-2 min-w-[600px]">
  ...
  <div class="text-sm font-mono bg-slate-100 p-1 rounded whitespace-normal">y = x^5 + 2x² + x³ + 2</div>

  <div class="col-span-2 border-t border-slate-100"></div>
  <div class="text-sm text-slate-700">Apply the standard power rule.</div>
  <div class="text-sm font-mono bg-slate-100 p-1 rounded whitespace-normal">(dy)/(dx) = 5x^4 + 4x + 3x²</div>
...
<div class="grid grid-cols-2 gap-4 mt-2 border-t border-slate-200 pt-2 min-w-[600px]">
  ...
  <div class="col-span-2 border-t border-slate-100"></div>
  <div class="text-sm text-slate-700">Identify u = x² and v = x+1. Find derivatives.</div>
  ...
  <div class="text-sm font-mono bg-slate-100 p-1 rounded whitespace-normal">(dy)/(dx) = ((x+1)(2x) - (x²)(1))/((x+1)²)</div>

  <div class="col-span-2 border-t border-slate-100"></div>
  <div class="text-sm text-slate-700">Simplify numerator: (2x² + 2x) - x².</div>
  <div class="text-sm font-mono bg-slate-100 p-1 rounded whitespace-normal">(dy)/(dx) = (x² + 2x)/((x+1)²)</div>
...
**Question:** Differentiate y = (1)/(x²+a²).
...
<div class="grid grid-cols-2 gap-4 mt-2 border-t border-slate-200 pt-2 min-w-[600px]">
  ...
  <div class="col-span-2 border-t border-slate-100"></div>
  <div class="text-sm text-slate-700">Top is u=1, Bottom is v=x²+a². a is a constant.</div>
  ...
  <div class="text-sm font-mono bg-slate-100 p-1 rounded whitespace-normal">((x²+a²)(0) - 1(2x))/((x²+a²)²)</div>

  <div class="col-span-2 border-t border-slate-100"></div>
  <div class="text-sm text-slate-700">Clean up.</div>
  <div class="text-sm font-mono bg-slate-100 p-1 rounded whitespace-normal">(dy)/(dx) = (-2x)/((x²+a²)²)</div>
...
<div class="grid grid-cols-2 gap-4 mt-2 border-t border-slate-200 pt-2 min-w-[600px]">
  ...
  <div class="text-sm font-mono bg-slate-100 p-1 rounded whitespace-normal">y = x(x² + 3x + 2) = x³ + 3x² + 2x</div>

  <div class="col-span-2 border-t border-slate-100"></div>
  <div class="text-sm text-slate-700">Power rule.</div>
  <div class="text-sm font-mono bg-slate-100 p-1 rounded whitespace-normal">(dy)/(dx) = 3x² + 6x + 2</div>
...
**Question:** Differentiate y = frac{√(x)}{x+1}.
...
<div class="grid grid-cols-2 gap-4 mt-2 border-t border-slate-200 pt-2 min-w-[600px]">
  ...
  <div class="col-span-2 border-t border-slate-100"></div>
  <div class="text-sm text-slate-700">u = x^1/2, v = x+1. u' = (1)/(2√(x)), v'=1.</div>
  <div class="text-sm font-mono bg-slate-100 p-1 rounded whitespace-normal">(dy)/(dx) = ((x+1)(frac{1)/(2√(x))) - √(x)(1)}{(x+1)²}</div>

  <div class="col-span-2 border-t border-slate-100"></div>
  <div class="text-sm text-slate-700">Multiply top and bottom by 2√(x) to clear ugly complex fractions.</div>
  <div class="text-sm font-mono bg-slate-100 p-1 rounded whitespace-normal">= ((x+1) - 2x)/(2√(x)(x+1)²) = (1-x)/(2√(x)(x+1)²)</div>
...
<div class="grid grid-cols-2 gap-4 mt-2 border-t border-slate-200 pt-2 min-w-[600px]">
  ...
  <div class="col-span-2 border-t border-slate-100"></div>
  <div class="text-sm text-slate-700">u=x³, v=1+x². u'=3x², v'=2x.</div>
  <div class="text-sm font-mono bg-slate-100 p-1 rounded whitespace-normal">((1+x²)(3x²) - x³(2x))/((1+x²)²)</div>

  <div class="col-span-2 border-t border-slate-100"></div>
  <div class="text-sm text-slate-700">Expand top.</div>
  <div class="text-sm font-mono bg-slate-100 p-1 rounded whitespace-normal">(3x² + 3x^4 - 2x^4)/((1+x²)²)</div>

  <div class="col-span-2 border-t border-slate-100"></div>
  <div class="text-sm text-slate-700">Combine like terms.</div>
  <div class="text-sm font-mono bg-slate-100 p-1 rounded whitespace-normal">(x^4+3x²)/((1+x²)²)</div>
...
**Question:** Differentiate y = (x²+1)/(5).
...
<div class="grid grid-cols-2 gap-4 mt-2 border-t border-slate-200 pt-2 min-w-[600px]">
  ...
  <div class="col-span-2 border-t border-slate-100"></div>
  <div class="text-sm text-slate-700">Rewrite as a multiplied fraction y = (1)/(5)(x²+1) and just use power rule.</div>
  <div class="text-sm font-mono bg-slate-100 p-1 rounded whitespace-normal">(dy)/(dx) = (2x)/(5)</div>
...
**Question:** Differentiate y = (5)/(x²+1).
...
<div class="grid grid-cols-2 gap-4 mt-2 border-t border-slate-200 pt-2 min-w-[600px]">
  ...
  <div class="text-sm font-mono bg-slate-100 p-1 rounded whitespace-normal">((x²+1)(0) - 5(2x))/((x²+1)²)</div>

  <div class="col-span-2 border-t border-slate-100"></div>
  <div class="text-sm text-slate-700">Simplify.</div>
  <div class="text-sm font-mono bg-slate-100 p-1 rounded whitespace-normal">= (-10x)/((x²+1)²)</div>
...
**Question:** Differentiate y = (ax+b)/(cx+d).
...
<div class="grid grid-cols-2 gap-4 mt-2 border-t border-slate-200 pt-2 min-w-[600px]">
  ...
  <div class="text-sm font-mono bg-slate-100 p-1 rounded whitespace-normal">((cx+d)(a) - (ax+b)(c))/((cx+d)²)</div>

  <div class="col-span-2 border-t border-slate-100"></div>
  <div class="text-sm text-slate-700">Expand: acx + ad - acx - bc.</div>
  <div class="text-sm font-mono bg-slate-100 p-1 rounded whitespace-normal">= (ad - bc)/((cx+d)²)</div>
...
**Question:** Differentiate y = (x² + 1) e^x.

| Inner voice | What you write |
| :--- | :--- |
| "I see two distinct functions of x multiplied together. I must use the Product Rule: u'v + uv'." | Let u = x²+1, v = e^x |
| "Calculate u' and v', then plug into the formula." | y' = (2x)(e^x) + (x²+1)(e^x) |
...
| (u(x))/(v(x)) | Is it a fraction? | Use Quotient Rule: (u'v - uv')/(v²) |

</div>
</div>## Chapter Assessment
<div class="user-quiz" data-question="1. Differentiate y = x²(x+5)."></div>
<div class="user-quiz" data-question="2. Identify u' and v' for the product y = (x³)(ln  x). (Hint: d/dx ln  x = 1/x)."></div>
<div class="user-quiz" data-question="3. Write the complete formula for the Quotient Rule."></div>
<div class="user-quiz" data-question="4. Differentiate y = (x)/(x+1)."></div>
<div class="user-quiz" data-question="5. Why do we not differentiate u × v as u' × v'?"></div>
`
  },
{
    id: "chapter-5",
...
Second differentiation: 5 × 4x³ = 20x³.
Third: 20 × 3x² = 60x².
Fourth: 60 × 2x = 120x.
Fifth: 120 × 1 = 120.
Sixth: 0.
There is a certain notation with which we are already acquainted... f'(x) is the derived function. f''(x) is the second derivative. The formal notation is (d²y)/(dx²), read as "dee-two-why over dee-ex-squared". It means that the operation of differentiating y with respect to x has been performed twice over.
...
## Glossary of Key Terms
- **First Derivative:** The rate of change of a function. The velocity. f'(x) or (dy)/(dx).
- **Second Derivative:** The rate of change of the rate of change! The acceleration. f''(x) or (d²y)/(dx²).
- **Third Derivative:** The rate of change of the acceleration. Jerk. f'''(x) or (d³y)/(dx³).
- **Prime Notation:** The use of hash marks f'(x), f''(x) created by Joseph-Louis Lagrange.

## Why This Matters in Electrical Engineering
The fundamental equation governing a capacitor-inductor (LC) oscillation circuit is L (d² i)/(dt²) + (1)/(C)i = 0. That right there is a **Second Order Differential Equation**, governed entirely by the second derivative. Without the second derivative, we could not mathematically describe radio waves, Wi-Fi, audio frequencies, or AC power grids.

## Formal Definitions and Notation
**Successive Differentiation Notation:**
1st Derivative: (dy)/(dx) or y' or f'(x) or D_x y
2nd Derivative: (d²y)/(dx²) or y'' or f''(x) or D²_x y
3rd Derivative: (d³y)/(dx³) or y''' or f'''(x) or D³_x y
nth Derivative: (d^ny)/(dx^n) or y^(n) or f^(n)(x)

**Note:** The 2 in (d²y)/(dx²) DOES NOT mean squared. It simply means "execute the derivative operator d/dx two times sequentially on y".
...
**Question:** If y = 3x^4 - 2x² + 5x, find y''.
...
<div class="grid grid-cols-2 gap-4 mt-2 border-t border-slate-200 pt-2 min-w-[600px]">
  ...
  <div class="text-sm font-mono bg-slate-100 p-1 rounded whitespace-normal">y' = 12x³ - 4x + 5</div>

  <div class="col-span-2 border-t border-slate-100"></div>
  <div class="text-sm text-slate-700">Derive y' to get y''.</div>
  <div class="text-sm font-mono bg-slate-100 p-1 rounded whitespace-normal">y'' = 36x² - 4</div>
...
<div class="grid grid-cols-2 gap-4 mt-2 border-t border-slate-200 pt-2 min-w-[600px]">
  ...
  <div class="text-sm font-mono bg-slate-100 p-1 rounded whitespace-normal">y'' = 20x³</div>

  <div class="col-span-2 border-t border-slate-100"></div>
  <div class="text-sm text-slate-700">Step 3: Third derivative.</div>
  <div class="text-sm font-mono bg-slate-100 p-1 rounded whitespace-normal">y''' = 60x²</div>
...
**Question:** Find the second derivative of y = (1)/(x).
...
<div class="grid grid-cols-2 gap-4 mt-2 border-t border-slate-200 pt-2 min-w-[600px]">
  ...
  <div class="text-sm font-mono bg-slate-100 p-1 rounded whitespace-normal">y'' = 2x^-3 = (2)/(x³)</div>
...
<div class="grid grid-cols-2 gap-4 mt-2 border-t border-slate-200 pt-2 min-w-[600px]">
  ...
  <div class="col-span-2 border-t border-slate-100"></div>
  <div class="text-sm text-slate-700">First derivative.</div>
  <div class="text-sm font-mono bg-slate-100 p-1 rounded whitespace-normal">y' = (1)/(2)x^-1/2</div>

  <div class="col-span-2 border-t border-slate-100"></div>
  <div class="text-sm text-slate-700">Second derivative. (-1/2) × (1/2) = -1/4.</div>
  <div class="text-sm font-mono bg-slate-100 p-1 rounded whitespace-normal">y'' = -(1)/(4)x^-3/2 = -(1)/(4√(x³))</div>
...
<div class="grid grid-cols-2 gap-4 mt-2 border-t border-slate-200 pt-2 min-w-[600px]">
  ...
  <div class="text-sm font-mono bg-slate-100 p-1 rounded whitespace-normal">y = x^4 + 2x² + 1</div>

  <div class="col-span-2 border-t border-slate-100"></div>
  <div class="text-sm text-slate-700">First derivative.</div>
  <div class="text-sm font-mono bg-slate-100 p-1 rounded whitespace-normal">y' = 4x³ + 4x</div>

  <div class="col-span-2 border-t border-slate-100"></div>
  <div class="text-sm text-slate-700">Second derivative.</div>
  <div class="text-sm font-mono bg-slate-100 p-1 rounded whitespace-normal">y'' = 12x² + 4</div>
...
<div class="grid grid-cols-2 gap-4 mt-2 border-t border-slate-200 pt-2 min-w-[600px]">
  ...
  <div class="col-span-2 border-t border-slate-100"></div>
  <div class="text-sm text-slate-700">y' = ((x+1)(1) - x(1))/((x+1)²)</div>
  <div class="text-sm font-mono bg-slate-100 p-1 rounded whitespace-normal">y' = (1)/((x+1)²)</div>
  ...
  <div class="text-sm font-mono bg-slate-100 p-1 rounded whitespace-normal">y'' = -2(x+1)^-3 = (-2)/((x+1)³)</div>
...
**Question:** Evaluate f''(1) for f(x) = 4x³ - 2x².
...
<div class="grid grid-cols-2 gap-4 mt-2 border-t border-slate-200 pt-2 min-w-[600px]">
  ...
  <div class="text-sm font-mono bg-slate-100 p-1 rounded whitespace-normal">f'(x) = 12x² - 4x</div>
...
<div class="grid grid-cols-2 gap-4 mt-2 border-t border-slate-200 pt-2 min-w-[600px]">
  ...
  <div class="col-span-2 border-t border-slate-100"></div>
  <div class="text-sm text-slate-700">The power is 4. The 4th derivative will be a constant (4 × 3 × 2 × 1).</div>
...
**Question:** If distance s = 5t³ + 2t, find formula for acceleration a.
...
<div class="grid grid-cols-2 gap-4 mt-2 border-t border-slate-200 pt-2 min-w-[600px]">
  ...
  <div class="text-sm font-mono bg-slate-100 p-1 rounded whitespace-normal">v = 15t² + 2</div>
...
**Question:** Given y = ax² + bx + c, find y''.
...
| Inner voice | What you write |
| :--- | :--- |
| "First, find the first derivative. Since ln (kx) differentiates to 1/x, I'll just write that." | y' = (1)/(x) = x^-1 |
| "Now differentiate y' to get y''. Use the standard power rule." | y'' = -1 • x^-2 = -(1)/(x²) |
...
<div class="user-quiz" data-question="1. Find y'' if y = 7x³."></div>
<div class="user-quiz" data-question="2. True or False: f''(x) tells you the acceleration of a particle."></div>
<div class="user-quiz" data-question="3. Find the 4th derivative of y = x²."></div>
<div class="user-quiz" data-question="4. Evaluate y'' at x=1 for y = 5x² - x."></div>
<div class="user-quiz" data-question="5. If y = 1/x², what is y''?"></div>
`
  },
...
## ELI5 Explanation (Engineering Analogy)
**The Falling Stone:**
Imagine you drop a stone from a bridge. If you take a picture every second, the distance between the stone in each picture gets larger and larger. The speed is increasing constantly! Calculus allows you to take infinitely fast pictures. If position is a function s(t) = 16t², the first derivative v(t) = 32t gives you the exact speed at any microsecond. The second derivative a(t) = 32 confirms the constant pull of Earth's gravity.

## Glossary of Key Terms
- **Position (s(t)):** The location of an object at a given time.
- **Velocity (v(t)):** The rate at which position changes over time. It tells you speed AND direction. v = (ds)/(dt).
- **Acceleration (a(t)):** The rate at which velocity changes over time. a = (dv)/(dt) or a = (d²s)/(dt²).
- **Rest:** A state where the object is momentarily stopped. Mathematically, velocity is exactly 0.

## Why This Matters in Electrical Engineering
This mathematical structure directly applies to electric charge. The electric current i(t) is simply the exact continuous rate of change (velocity) of the electrical charge q(t) moving through a wire. i = (dq)/(dt). All physics problems mapping physical location to velocity directly map to evaluating charge and the resulting current in power systems.

## Formal Definitions and Notation
**Equations of Motion:**
1. Position: s(t)
2. Velocity: v(t) = (ds)/(dt) = s'(t)
3. Acceleration: a(t) = (dv)/(dt) = s''(t)
...
4. **Answer the specific question:**
   - "When is it at rest?" → Set v(t) = 0 and solve for t.
   - "What is the speed at t=3?" → Plug 3 into v(t) and take absolute value.
   - "When does it hit the ground?" → Set position s(t) = 0 and solve for t.
...
**Question:** If s(t) = 5t² + 2t + 4, find velocity at t=2.
...
**Question:** If s(t) = t³ - 6t² + 9t, when is it horizontally at rest?
...
<div class="grid grid-cols-2 gap-4 mt-2 border-t border-slate-200 pt-2 min-w-[600px]">
  ...
  <div class="text-sm font-mono bg-slate-100 p-1 rounded whitespace-normal">v(t) = 3t² - 12t + 9</div>

  <div class="col-span-2 border-t border-slate-100"></div>
  <div class="text-sm text-slate-700">Set to zero and factor.</div>
  <div class="text-sm font-mono bg-slate-100 p-1 rounded whitespace-normal">3(t² - 4t + 3) = 0 implies 3(t-3)(t-1) = 0</div>
...
**Question:** A stone is dropped: s(t) = 16t². How far has it fallen when its velocity is 64 ft/s?
...
<div class="grid grid-cols-2 gap-4 mt-2 border-t border-slate-200 pt-2 min-w-[600px]">
  ...
  <div class="text-sm font-mono bg-slate-100 p-1 rounded whitespace-normal">s(2) = 16(2)² = 16(4) = 64 ft</div>
...
**Question:** Prove that a falling object s(t) = -4.9t² + v_0t + s_0 has constant acceleration.
...
**Question:** A bullet is shot up: s(t) = -16t² + 128t. What is the maximum height?
...
<div class="grid grid-cols-2 gap-4 mt-2 border-t border-slate-200 pt-2 min-w-[600px]">
  ...
  <div class="text-sm font-mono bg-slate-100 p-1 rounded whitespace-normal">s(4) = -16(16) + 128(4) = 256 ft</div>
...
**Question:** If v(t) = (1)/(t+1), find a(t).
...
<div class="grid grid-cols-2 gap-4 mt-2 border-t border-slate-200 pt-2 min-w-[600px]">
  ...
  <div class="text-sm font-mono bg-slate-100 p-1 rounded whitespace-normal">a(t) = -(t+1)^-2 = (-1)/((t+1)²)</div>
...
**Question:** Given s(t) = t³ - 4t + 10, what is its initial position and initial velocity?
...
<div class="grid grid-cols-2 gap-4 mt-2 border-t border-slate-200 pt-2 min-w-[600px]">
  ...
  <div class="text-sm font-mono bg-slate-100 p-1 rounded whitespace-normal">v(t) = 3t² - 4 implies v(0) = -4</div>
...
**Question:** If s(t) = t² - 8t, during what time interval is it moving backwards?
...
<div class="grid grid-cols-2 gap-4 mt-2 border-t border-slate-200 pt-2 min-w-[600px]">
  ...
  <div class="text-sm font-mono bg-slate-100 p-1 rounded whitespace-normal">0 ≤ t < 4</div>
...
**Question:** Position is s(t) = t³ - 6t². When is the particle at rest?

| Inner voice | What you write |
| :--- | :--- |
| "'At rest' means velocity is zero. I need to find the velocity function by taking the derivative of position." | v(t) = s'(t) = 3t² - 12t |
...
<div class="user-quiz" data-question="1. If s(t) = 3t² - 12t, find the velocity at t=2."></div>
<div class="user-quiz" data-question="2. If v(t) = -32t + 64, at what time is the object at rest?"></div>
<div class="user-quiz" data-question="3. If a rocket's position is s(t) = t³ + t, find its acceleration at t=3."></div>
  ...
  },
{
  ...
  },
  {
    ...
    markdown: `
## Introduction & Core Concepts
Sometimes one is stumped by finding that the expression to be differentiated is too complicated to tackle directly. Thus, the equation y = (x² + a²)³ is awkward to a beginner.
Now the dodge to turn the difficulty is this: Write some symbol, such as u, for the expression x² + a²; then the equation becomes y = u³. You can easily manage this, and you can easily manage u. Then all that remains is plain sailing by multiplying the derivatives together.
...
- **Chain Rule:** The multiplicative law used to differentiate layered functions: (dy)/(dx) = (dy)/(du) × (du)/(dx).

## Why This Matters in Electrical Engineering
When analyzing alternating current, you will constantly differentiate sine and cosine wave equations like V(t) = 170sin (377t + φ). Taking the derivative of this requires the Chain Rule perfectly, since the outer function is sin () and the inner function is (377t + φ). It pulls out the 377 (the angular frequency ω) to multiply the front amplitude.
  ...
  <table class="w-full text-left border-collapse min-w-[500px]">
    ...
    <tbody class="bg-white">
      <tr class="">
        ...
        <td class="p-3 border-b text-sm font-mono">Differentiate √(x²+1)</td>
        <td class="p-3 border-b text-sm">Rewrite as (x²+1)^1/2 and apply chain rule.</td>
      </tr><tr class="">
...
<div class="grid grid-cols-2 gap-4 mt-2 border-t border-slate-200 pt-2 min-w-[600px]">
  ...
  <div class="col-span-2 border-t border-slate-100"></div>
  <div class="text-sm text-slate-700">The outside is u³. Inside is x²+5. Bring 3 down, leave inside alone, reduce power to 2.</div>
  <div class="text-sm font-mono bg-slate-100 p-1 rounded whitespace-normal">3(x² + 5)² dots</div>

  <div class="col-span-2 border-t border-slate-100"></div>
  <div class="text-sm text-slate-700">Multiply by the derivative of the inside (2x).</div>
  <div class="text-sm font-mono bg-slate-100 p-1 rounded whitespace-normal">dots × (2x)</div>

  <div class="col-span-2 border-t border-slate-100"></div>
  <div class="text-sm text-slate-700">Clean up by multiplying coefficients.</div>
  <div class="text-sm font-mono bg-slate-100 p-1 rounded whitespace-normal">y' = 6x(x² + 5)²</div>
...
<div class="grid grid-cols-2 gap-4 mt-2 border-t border-slate-200 pt-2 min-w-[600px]">
  ...
  <div class="text-sm font-mono bg-slate-100 p-1 rounded whitespace-normal">y = (x³ + 1)^1/2</div>

  <div class="col-span-2 border-t border-slate-100"></div>
  <div class="text-sm text-slate-700">Apply chain rule. Outside first.</div>
  <div class="text-sm font-mono bg-slate-100 p-1 rounded whitespace-normal">(1)/(2)(x³ + 1)^-1/2 dots</div>

  <div class="col-span-2 border-t border-slate-100"></div>
  <div class="text-sm text-slate-700">Multiply by inner derivative (3x²).</div>
  <div class="text-sm font-mono bg-slate-100 p-1 rounded whitespace-normal">(dy)/(dx) = (1)/(2)(x³+1)^-1/2 × 3x² = (3x²)/(2√(x³+1))</div>
...
**Question:** Differentiate y = (1)/((2x-3)^4).
...
<div class="grid grid-cols-2 gap-4 mt-2 border-t border-slate-200 pt-2 min-w-[600px]">
  ...
  <div class="text-sm font-mono bg-slate-100 p-1 rounded whitespace-normal">(dy)/(dx) = -8(2x - 3)^-5 = (-8)/((2x-3)^5)</div>
...
**Question:** If derivative of sin (x) is cos (x), differentiate y = sin (x²).
...
<div class="grid grid-cols-2 gap-4 mt-2 border-t border-slate-200 pt-2 min-w-[600px]">
  ...
  <div class="col-span-2 border-t border-slate-100"></div>
  <div class="text-sm text-slate-700">Outer is sine, inner is x². Derivative of outer is cosine (keeping x² inside).</div>
  <div class="text-sm font-mono bg-slate-100 p-1 rounded whitespace-normal">cos (x²) dots</div>

  <div class="col-span-2 border-t border-slate-100"></div>
  <div class="text-sm text-slate-700">Multiply by inner derivative (2x).</div>
  <div class="text-sm font-mono bg-slate-100 p-1 rounded whitespace-normal">(dy)/(dx) = 2x cos (x²)</div>
...
<div class="grid grid-cols-2 gap-4 mt-2 border-t border-slate-200 pt-2 min-w-[600px]">
  ...
  <div class="text-sm font-mono bg-slate-100 p-1 rounded whitespace-normal">y = (sin  x)²</div>
  ...
  <div class="text-sm font-mono bg-slate-100 p-1 rounded whitespace-normal">(dy)/(dx) = 2sin (x)cos (x)</div>
...
<div class="grid grid-cols-2 gap-4 mt-2 border-t border-slate-200 pt-2 min-w-[600px]">
  ...
  <div class="text-sm font-mono bg-slate-100 p-1 rounded whitespace-normal">Layers: 1/2 power → sine → 3x</div>

  <div class="col-span-2 border-t border-slate-100"></div>
  <div class="text-sm text-slate-700">Peel layer 1: exact fraction.</div>
  <div class="text-sm font-mono bg-slate-100 p-1 rounded whitespace-normal">(1)/(2)(sin (3x))^-1/2 dots</div>

  <div class="col-span-2 border-t border-slate-100"></div>
  <div class="text-sm text-slate-700">Peel layer 2: derive sine to cosine.</div>
  <div class="text-sm font-mono bg-slate-100 p-1 rounded whitespace-normal">dots × cos (3x) dots</div>

  <div class="col-span-2 border-t border-slate-100"></div>
  <div class="text-sm text-slate-700">Peel layer 3: derive 3x to 3.</div>
  <div class="text-sm font-mono bg-slate-100 p-1 rounded whitespace-normal">(dy)/(dx) = (3cos (3x))/(2√(sin (3x)))</div>
...
<div class="grid grid-cols-2 gap-4 mt-2 border-t border-slate-200 pt-2 min-w-[600px]">
  ...
  <div class="col-span-2 border-t border-slate-100"></div>
  <div class="text-sm text-slate-700">Primary rule needed is the Product Rule (uv' + vu'). u = x and v = (x+1)³.</div>
  ...
  <div class="text-sm font-mono bg-slate-100 p-1 rounded whitespace-normal">v' = 3(x+1)²(1)</div>

  <div class="col-span-2 border-t border-slate-100"></div>
  <div class="text-sm text-slate-700">Assemble Product Rule.</div>
  <div class="text-sm font-mono bg-slate-100 p-1 rounded whitespace-normal">(dy)/(dx) = x(3(x+1)²) + (x+1)³(1)</div>
...
<div class="grid grid-cols-2 gap-4 mt-2 border-t border-slate-200 pt-2 min-w-[600px]">
  ...
  <div class="text-sm font-mono bg-slate-100 p-1 rounded whitespace-normal">2 ≤ft( (x)/(x+1) right)^1 dots</div>

  <div class="col-span-2 border-t border-slate-100"></div>
  <div class="text-sm text-slate-700">Inner function requires the Quotient Rule.</div>
  <div class="text-sm font-mono bg-slate-100 p-1 rounded whitespace-normal">((x+1)(1) - x(1))/((x+1)²) = (1)/((x+1)²)</div>

  <div class="col-span-2 border-t border-slate-100"></div>
  <div class="text-sm text-slate-700">Multiply them together.</div>
  <div class="text-sm font-mono bg-slate-100 p-1 rounded whitespace-normal">y' = 2 ≤ft( (x)/(x+1) right) (1)/((x+1)²) = (2x)/((x+1)³)</div>
...
**Question:** If y = 5u² and u = 3x - 1, find dy/dx.
...
<div class="grid grid-cols-2 gap-4 mt-2 border-t border-slate-200 pt-2 min-w-[600px]">
  ...
  <div class="text-sm font-mono bg-slate-100 p-1 rounded whitespace-normal">dy/dx = 10(3x-1) × 3 = 30(3x-1)</div>
...
**Question:** Differentiate y = cos (ω t + φ) with respect to t. (Assume derivative of cosine is negative sine).
...
<div class="grid grid-cols-2 gap-4 mt-2 border-t border-slate-200 pt-2 min-w-[600px]">
  ...
  <div class="col-span-2 border-t border-slate-100"></div>
  <div class="text-sm text-slate-700">Outer is cosine → negative sine.</div>
  <div class="text-sm font-mono bg-slate-100 p-1 rounded whitespace-normal">-sin (ω t + φ) dots</div>

  <div class="col-span-2 border-t border-slate-100"></div>
  <div class="text-sm text-slate-700">Inner is (ω t + φ). ω, φ are constants. Derivative is just ω.</div>
  <div class="text-sm font-mono bg-slate-100 p-1 rounded whitespace-normal">dots × ω</div>

  <div class="col-span-2 border-t border-slate-100"></div>
  <div class="text-sm text-slate-700">Combine.</div>
  <div class="text-sm font-mono bg-slate-100 p-1 rounded whitespace-normal">dy/dt = -ω sin (ω t + φ)</div>
...
**Question:** Find (dy)/(dx) for y = sin (x² + 3).

| Inner voice | What you write |
| :--- | :--- |
| "There's an 'inside' function x²+3 inside an 'outside' function sin (). I apply the Chain Rule." | Outer: sin (u) rightarrow cos (u) |
| "Derivative of outside, keep inside exactly the same, multiply by derivative of inside." | y' = cos (x²+3) • (2x) |
...
| If you see dots | Ask yourself dots | Action |
| :--- | :--- | :--- |
| f(g(x)) | Is there an inside function? | Derivative is f'(g(x)) • g'(x) |
| sin ²(x) | Is it really (sin  x)²? | Outer is (.)², inner is sin  x. |

</div>
</div>## Chapter Assessment
<div class="user-quiz" data-question="1. Differentiate y = (3x+4)^5."></div>
<div class="user-quiz" data-question="2. Differentiate y = √(5x² - 2)."></div>
...
<div class="user-quiz" data-question="5. Explain why the derivative of y=(x³)² using the chain rule yields the same result as expanding to x^6 first."></div>
`
  },
{
    id: "chapter-8",
  ...
  },
  {
    ...
    markdown: `
## Introduction & Core Concepts
So far, all the functions we have differentiated have been "explicit" functions, where y is neatly isolated on the left side of the equals sign: y = x² + 3x.
However, equations defining curves often come completely scrambled, such as the equation of a circle x² + y² = 25. Attempting to isolate y here forces us to deal with a ± square root, introducing unnecessary brutality to the algebra.
...
## ELI5 Explanation (Engineering Analogy)
**The Hidden Camera:**
When an equation is implicit, y isn't just a simple letter; it's a hidden function of x wearing a disguise. If you take the derivative of x³ with respect to x, you just get 3x². But if you take the derivative of y³ with respect to x, the chain rule says you must peel the onion: outer derivative 3y², multiplied by the hidden core's derivative dy/dx (or y'). Every time you differentiate a y term, a y' pops out like a little flag!
...
## Formal Definitions and Notation
**The Implicit Rule:**
We apply the derivative operator (d)/(dx) to both sides of the equation.
1. When differentiating x terms, differentiate normally.
2. When differentiating y terms, differentiate normally, but MULTIPLY the result by (dy)/(dx) (due to the Chain Rule).
3. If x and y are multiplied together (e.g. xy), you MUST use the Product Rule.

## Step\u2011by\u2011Step Method for Problem Solving
1. **Differentiate both sides:** Apply (d)/(dx) to every term strictly. Attach y' to any derivation of a y term. Be paranoid about product rules for terms like xy.
  ...
  <table class="w-full text-left border-collapse min-w-[500px]">
    ...
    <tbody class="bg-white">
      <tr class="">
        ...
        <td class="p-3 border-b text-sm">Apply (d)/(dx) to both sides immediately.</td>
      </tr><tr class="">
...
<div class="grid grid-cols-2 gap-4 mt-2 border-t border-slate-200 pt-2 min-w-[600px]">
  ...
  <div class="col-span-2 border-t border-slate-100"></div>
  <div class="text-sm text-slate-700">Differentiate term by term.</div>
  <div class="text-sm font-mono bg-slate-100 p-1 rounded whitespace-normal">2x + 2y • (y') = 0</div>

  <div class="col-span-2 border-t border-slate-100"></div>
  <div class="text-sm text-slate-700">Isolate y' term.</div>
  <div class="text-sm font-mono bg-slate-100 p-1 rounded whitespace-normal">2y • y' = -2x</div>

  <div class="col-span-2 border-t border-slate-100"></div>
  <div class="text-sm text-slate-700">Divide.</div>
  <div class="text-sm font-mono bg-slate-100 p-1 rounded whitespace-normal">y' = (-2x)/(2y) = -(x)/(y)</div>
...
<div class="grid grid-cols-2 gap-4 mt-2 border-t border-slate-200 pt-2 min-w-[600px]">
  ...
  <div class="text-sm font-mono bg-slate-100 p-1 rounded whitespace-normal">3y² • y' + 3x² = 0</div>

  <div class="col-span-2 border-t border-slate-100"></div>
  <div class="text-sm text-slate-700">Move the x term.</div>
  <div class="text-sm font-mono bg-slate-100 p-1 rounded whitespace-normal">3y² • y' = -3x²</div>

  <div class="col-span-2 border-t border-slate-100"></div>
  <div class="text-sm text-slate-700">Divide.</div>
  <div class="text-sm font-mono bg-slate-100 p-1 rounded whitespace-normal">y' = -(3x²)/(3y²) = -(x²)/(y²)</div>
...
<div class="grid grid-cols-2 gap-4 mt-2 border-t border-slate-200 pt-2 min-w-[600px]">
  ...
  <div class="text-sm font-mono bg-slate-100 p-1 rounded whitespace-normal">x • y' = -y</div>

  <div class="col-span-2 border-t border-slate-100"></div>
  <div class="text-sm text-slate-700">Divide.</div>
  <div class="text-sm font-mono bg-slate-100 p-1 rounded whitespace-normal">y' = -(y)/(x)</div>
...
<div class="grid grid-cols-2 gap-4 mt-2 border-t border-slate-200 pt-2 min-w-[600px]">
  ...
  <div class="text-sm font-mono bg-slate-100 p-1 rounded whitespace-normal">y' = (-2x - y)/(x + 2y)</div>
...
<div class="grid grid-cols-2 gap-4 mt-2 border-t border-slate-200 pt-2 min-w-[600px]">
  ...
  <div class="text-sm font-mono bg-slate-100 p-1 rounded whitespace-normal">cos (y) • y' = 1</div>

  <div class="col-span-2 border-t border-slate-100"></div>
  <div class="text-sm text-slate-700">Isolate.</div>
  <div class="text-sm font-mono bg-slate-100 p-1 rounded whitespace-normal">y' = (1)/(cos (y))</div>
...
**Question:** Find slope of x²y + y²x = -2 at point (1, -2).
...
<div class="grid grid-cols-2 gap-4 mt-2 border-t border-slate-200 pt-2 min-w-[600px]">
  ...
  <div class="text-sm font-mono bg-slate-100 p-1 rounded whitespace-normal">[x² y' + y(2x)] + [y²(1) + x(2y y')] = 0</div>
...
<div class="grid grid-cols-2 gap-4 mt-2 border-t border-slate-200 pt-2 min-w-[600px]">
  ...
  <div class="text-sm font-mono bg-slate-100 p-1 rounded whitespace-normal">4x³ + 4y³(y') = 0</div>

  <div class="col-span-2 border-t border-slate-100"></div>
  <div class="text-sm text-slate-700">Isolate.</div>
  <div class="text-sm font-mono bg-slate-100 p-1 rounded whitespace-normal">y' = -(4x³)/(4y³) = -(x³)/(y³)</div>
...
<div class="grid grid-cols-2 gap-4 mt-2 border-t border-slate-200 pt-2 min-w-[600px]">
  ...
  <div class="col-span-2 border-t border-slate-100"></div>
  <div class="text-sm text-slate-700">Divide.</div>
  <div class="text-sm font-mono bg-slate-100 p-1 rounded whitespace-normal">y' = (2x)/(2y+1)</div>
...
<div class="grid grid-cols-2 gap-4 mt-2 border-t border-slate-200 pt-2 min-w-[600px]">
  ...
  <div class="col-span-2 border-t border-slate-100"></div>
  <div class="text-sm text-slate-700">Isolate.</div>
  <div class="text-sm font-mono bg-slate-100 p-1 rounded whitespace-normal">y' = (1)/(5)</div>
...
**Question:** If x²+y²=r², and y' = -x/y, find y''.
...
<div class="grid grid-cols-2 gap-4 mt-2 border-t border-slate-200 pt-2 min-w-[600px]">
  ...
  <div class="text-sm font-mono bg-slate-100 p-1 rounded whitespace-normal">y'' = (y(-1) - (-x)(y'))/(y²)</div>

  <div class="col-span-2 border-t border-slate-100"></div>
  <div class="text-sm text-slate-700">Substitute the known y' into the answer.</div>
  <div class="text-sm font-mono bg-slate-100 p-1 rounded whitespace-normal">y'' = (-y + x(-x/y))/(y²)</div>

  <div class="col-span-2 border-t border-slate-100"></div>
  <div class="text-sm text-slate-700">Simplify. Multiply top and bottom by y.</div>
  <div class="text-sm font-mono bg-slate-100 p-1 rounded whitespace-normal">y'' = (-y² - x²)/(y³) = (-(x²+y²))/(y³) = (-r²)/(y³)</div>
...
| "Solve algebraically for y'." | 2y y' = -2x implies y' = -(x)/(y) |
...
| If you see dots | Ask yourself dots | Action |
| :--- | :--- | :--- |
| Term with y | Deriving wrt x? | Multiply result by (dy)/(dx). |
| xy | Is it a product? | Use product rule: x y' + y. |

</div>
</div>## Chapter Assessment
<div class="user-quiz" data-question="1. Differentiate implicitly: x² + y² = 16."></div>
<div class="user-quiz" data-question="2. Identify the mistake: (d)/(dx)[xy] = xy'."></div>
<div class="user-quiz" data-question="3. Find y' for 3x - 4y = 12."></div>
<div class="user-quiz" data-question="4. Implicitly differentiate y^4 - x² = 2."></div>
...
- **Implicit Time Differentiation:** Taking (d)/(dt) of an equation, causing EVERY spatial variable (x, y, r, V) to sprout a derivative flag (e.g., dx/dt).
- **Static Geometry:** Formulas like A = π r² or x² + y² = L² that link the physical dimensions.

## Why This Matters in Electrical Engineering
Imagine a parallel plate capacitor. If you physically start pulling the metal plates apart at a velocity of 0.1 cm/sec, the capacitance C decreases over time, which simultaneously causes the voltage V to fluctuate over time. Finding the exact resulting fluctuation (dV)/(dt) based on the physical velocity (dx)/(dt) requires perfectly linking electrical laws with geometric related rates.

## Formal Definitions and Notation
**The Fundamental Methodology:**
You apply the (d)/(dt) operator to every term.
For x² + y² = 25:
(d)/(dt) (x² + y²) = (d)/(dt)(25)
2x (dx)/(dt) + 2y (dy)/(dt) = 0
...
## Step\u2011by\u2011Step Method for Problem Solving
1. **Draw a Picture:** Label the moving parts with variable letters (x, y, r, h, θ). Label the stationary, unchanging parts with their hard numbers.
...
4. **Differentiate implicitly with respect to t:** Pop out d(var)/dt for every variable.
  ...
  <table class="w-full text-left border-collapse min-w-[500px]">
    ...
    <tbody class="bg-white">
      <tr class="">
        ...
        <td class="p-3 border-b text-sm">Write down the translation: d(var)/dt = value.</td>
      </tr><tr class="bg-slate-50">
        ...
        <td class="p-3 border-b text-sm">Draft a right triangle, use Pythagoras x²+y²=s².</td>
      </tr>
    </tbody>
  </table>
</div>
...
<div class="grid grid-cols-2 gap-4 mt-2 border-t border-slate-200 pt-2 min-w-[600px]">
  ...
  <div class="text-sm font-mono bg-slate-100 p-1 rounded whitespace-normal">A = π r²</div>

  <div class="col-span-2 border-t border-slate-100"></div>
  <div class="text-sm text-slate-700">Derive with respect to t.</div>
  <div class="text-sm font-mono bg-slate-100 p-1 rounded whitespace-normal">(dA)/(dt) = 2π r (dr)/(dt)</div>

  <div class="col-span-2 border-t border-slate-100"></div>
  <div class="text-sm text-slate-700">Plug in the snapshot r=10 and known rate dr/dt=2.</div>
  <div class="text-sm font-mono bg-slate-100 p-1 rounded whitespace-normal">(dA)/(dt) = 2π (10) (2) = 40π  sq in/s</div>
...
<div class="grid grid-cols-2 gap-4 mt-2 border-t border-slate-200 pt-2 min-w-[600px]">
  ...
  <div class="text-sm font-mono bg-slate-100 p-1 rounded whitespace-normal">x² + y² = 100</div>

  <div class="col-span-2 border-t border-slate-100"></div>
  <div class="text-sm text-slate-700">Derive.</div>
  <div class="text-sm font-mono bg-slate-100 p-1 rounded whitespace-normal">2x (dx)/(dt) + 2y (dy)/(dt) = 0</div>

  <div class="col-span-2 border-t border-slate-100"></div>
  <div class="text-sm text-slate-700">Snapshot: x=6. Find y using 6²+y²=100. So y=8. Top descents so dy/dt = -1.</div>
  <div class="text-sm font-mono bg-slate-100 p-1 rounded whitespace-normal">2(6)(dx)/(dt) + 2(8)(-1) = 0</div>

  <div class="col-span-2 border-t border-slate-100"></div>
  <div class="text-sm text-slate-700">Solve for dx/dt.</div>
  <div class="text-sm font-mono bg-slate-100 p-1 rounded whitespace-normal">12(dx)/(dt) = 16 implies (dx)/(dt) = 1.33  ft/s</div>
...
<div class="grid grid-cols-2 gap-4 mt-2 border-t border-slate-200 pt-2 min-w-[600px]">
  ...
  <div class="text-sm font-mono bg-slate-100 p-1 rounded whitespace-normal">(dV)/(dt) = 4π r² (dr)/(dt)</div>

  <div class="col-span-2 border-t border-slate-100"></div>
  <div class="text-sm text-slate-700">Plug in dV/dt = 100, r=5.</div>
  <div class="text-sm font-mono bg-slate-100 p-1 rounded whitespace-normal">100 = 4π(25) (dr)/(dt)</div>

  <div class="col-span-2 border-t border-slate-100"></div>
  <div class="text-sm text-slate-700">Solve.</div>
  <div class="text-sm font-mono bg-slate-100 p-1 rounded whitespace-normal">(dr)/(dt) = (100)/(100π) = (1)/(π)  cm/s</div>
...
<div class="grid grid-cols-2 gap-4 mt-2 border-t border-slate-200 pt-2 min-w-[600px]">
  ...
  <div class="col-span-2 border-t border-slate-100"></div>
  <div class="text-sm text-slate-700">Distance between them z² = x² + y².</div>
  <div class="text-sm font-mono bg-slate-100 p-1 rounded whitespace-normal">2z (dz)/(dt) = 2x (dx)/(dt) + 2y (dy)/(dt)</div>
  ...
  <div class="text-sm font-mono bg-slate-100 p-1 rounded whitespace-normal">10z' = 320 + 180 = 500 implies z' = 50  mph</div>
...
**Question:** Water drains from a conical tank. Volume V = (1)/(3)π r² h. r/h ratio is 1/2. Find dV/dt if dh/dt = -1, and h=4.
...
<div class="grid grid-cols-2 gap-4 mt-2 border-t border-slate-200 pt-2 min-w-[600px]">
  ...
  <div class="text-sm font-mono bg-slate-100 p-1 rounded whitespace-normal">V = (1)/(3)π (h/2)² h = (π)/(12) h³</div>

  <div class="col-span-2 border-t border-slate-100"></div>
  <div class="text-sm text-slate-700">Derive.</div>
  <div class="text-sm font-mono bg-slate-100 p-1 rounded whitespace-normal">(dV)/(dt) = (π)/(4) h² (dh)/(dt)</div>

  <div class="col-span-2 border-t border-slate-100"></div>
  <div class="text-sm text-slate-700">Plug in.</div>
  <div class="text-sm font-mono bg-slate-100 p-1 rounded whitespace-normal">(dV)/(dt) = (π)/(4)(16)(-1) = -4π</div>
...
<div class="grid grid-cols-2 gap-4 mt-2 border-t border-slate-200 pt-2 min-w-[600px]">
  ...
  <div class="col-span-2 border-t border-slate-100"></div>
  <div class="text-sm text-slate-700">Similar triangles. Let x be man's position, y be shadow tip. (y)/(15) = (y-x)/(6).</div>
  <div class="text-sm font-mono bg-slate-100 p-1 rounded whitespace-normal">6y = 15y - 15x implies 15x = 9y</div>

  <div class="col-span-2 border-t border-slate-100"></div>
  <div class="text-sm text-slate-700">Derive.</div>
  <div class="text-sm font-mono bg-slate-100 p-1 rounded whitespace-normal">15(dx)/(dt) = 9(dy)/(dt)</div>

  <div class="col-span-2 border-t border-slate-100"></div>
  <div class="text-sm text-slate-700">Plug in man's speed dx/dt = 5.</div>
  <div class="text-sm font-mono bg-slate-100 p-1 rounded whitespace-normal">15(5) = 9y' implies y' = (75)/(9) = 8.33  ft/s</div>
...
**Question:** A cube melts, volume drops 2 in³/min. Rate of side s shrinking when s=3?
...
<div class="grid grid-cols-2 gap-4 mt-2 border-t border-slate-200 pt-2 min-w-[600px]">
  ...
  <div class="text-sm font-mono bg-slate-100 p-1 rounded whitespace-normal">V = s³</div>

  <div class="col-span-2 border-t border-slate-100"></div>
  <div class="text-sm text-slate-700">Derive.</div>
  <div class="text-sm font-mono bg-slate-100 p-1 rounded whitespace-normal">V' = 3s² s'</div>

  <div class="col-span-2 border-t border-slate-100"></div>
  <div class="text-sm text-slate-700">Plug in values. V' is negative because it's melting.</div>
  <div class="text-sm font-mono bg-slate-100 p-1 rounded whitespace-normal">-2 = 3(9) s' implies s' = (-2)/(27)</div>
...
<div class="grid grid-cols-2 gap-4 mt-2 border-t border-slate-200 pt-2 min-w-[600px]">
  ...
  <div class="text-sm font-mono bg-slate-100 p-1 rounded whitespace-normal">(1)/(R²)(dR)/(dt) = (1)/(R_1²)R_1' + (1)/(R_2²)R_2'</div>
...
<div class="grid grid-cols-2 gap-4 mt-2 border-t border-slate-200 pt-2 min-w-[600px]">
  ...
  <div class="col-span-2 border-t border-slate-100"></div>
  <div class="text-sm text-slate-700">Pythagoras with stationary vertical leg 10. x² + 10² = z².</div>
  <div class="text-sm font-mono bg-slate-100 p-1 rounded whitespace-normal">2x x' = 2z z'</div>

  <div class="col-span-2 border-t border-slate-100"></div>
  <div class="text-sm text-slate-700">Snapshot: z=26, z'=-2. Find x: x²+100=676 implies x=24.</div>
  <div class="text-sm font-mono bg-slate-100 p-1 rounded whitespace-normal">(24)x' = (26)(-2)</div>

  <div class="col-span-2 border-t border-slate-100"></div>
  <div class="text-sm text-slate-700">Solve.</div>
  <div class="text-sm font-mono bg-slate-100 p-1 rounded whitespace-normal">x' = (-52)/(24) = -2.17dots</div>
...
**Question:** Water in cylinder (V = π r² h). If r=10 is constant, and water fills at rate 5, find dh/dt.
...
<div class="grid grid-cols-2 gap-4 mt-2 border-t border-slate-200 pt-2 min-w-[600px]">
  ...
  <div class="text-sm font-mono bg-slate-100 p-1 rounded whitespace-normal">V = 100π h</div>
  ...
  <div class="col-span-2 border-t border-slate-100"></div>
  <div class="text-sm text-slate-700">Solve.</div>
  <div class="text-sm font-mono bg-slate-100 p-1 rounded whitespace-normal">5 = 100π h' implies h' = (5)/(100π)</div>
...
**Question:** A circle's radius is growing at 2 m/s. How fast is its area growing when r=5?

| Inner voice | What you write |
| :--- | :--- |
| "Write the geometric equation, then take the time derivative (d)/(dt) of both sides." | Area: A = π r²  A' = 2π r • r' |
| "Now plug in the specific moment values (r=5, r'=2)." | A' = 2π(5)(2) = 20π m²/s |
...
| If you see dots | Ask yourself dots | Action |
| :--- | :--- | :--- |
| "Growing at..." | Is it a rate? | Identify as (dr)/(dt), (dV)/(dt), etc. |
...
<div class="user-quiz" data-question="2. If x² + y² = 25, write the implicit time derivative expression."></div>
...
## Why This Matters in Electrical Engineering
Power Transfer Theorem states that maximum power is delivered to a load when the load resistance perfectly matches the internal source resistance (R_L = R_S). How did engineers prove this? They wrote the power equation P = I² R_L, took the derivative (dP)/(dR_L), set it to exactly zero to seek the maximum limit, and algebraically solved for the peak.
...
**Question:** Find local extrema of y = x² - 4x + 3.
...
**Question:** Find extrema of y = x³ - 3x.
...
<div class="grid grid-cols-2 gap-4 mt-2 border-t border-slate-200 pt-2 min-w-[600px]">
  ...
  <div class="text-sm font-mono bg-slate-100 p-1 rounded whitespace-normal">y' = 3x² - 3 = 0</div>

  <div class="col-span-2 border-t border-slate-100"></div>
  <div class="text-sm text-slate-700">Solve for x.</div>
  <div class="text-sm font-mono bg-slate-100 p-1 rounded whitespace-normal">3(x²-1) = 0 implies x = 1, x = -1</div>
...
<div class="grid grid-cols-2 gap-4 mt-2 border-t border-slate-200 pt-2 min-w-[600px]">
  ...
  <div class="text-sm font-mono bg-slate-100 p-1 rounded whitespace-normal">A = x(50-x) = 50x - x²</div>
...
<div class="grid grid-cols-2 gap-4 mt-2 border-t border-slate-200 pt-2 min-w-[600px]">
  ...
  <div class="text-sm font-mono bg-slate-100 p-1 rounded whitespace-normal">V = x(12-2x)² = x(144-48x+4x²)</div>

  <div class="col-span-2 border-t border-slate-100"></div>
  <div class="text-sm text-slate-700">Multiply out.</div>
  <div class="text-sm font-mono bg-slate-100 p-1 rounded whitespace-normal">V = 4x³ - 48x² + 144x</div>

  <div class="col-span-2 border-t border-slate-100"></div>
  <div class="text-sm text-slate-700">Derive and set to 0.</div>
  <div class="text-sm font-mono bg-slate-100 p-1 rounded whitespace-normal">12x² - 96x + 144 = 0 implies 12(x²-8x+12)=0</div>
...
**Question:** Find the point on y=x² closest to (3,0).
...
<div class="grid grid-cols-2 gap-4 mt-2 border-t border-slate-200 pt-2 min-w-[600px]">
  ...
  <div class="col-span-2 border-t border-slate-100"></div>
  <div class="text-sm text-slate-700">Distance squared is D = (x-3)² + (y-0)². Substitute y=x².</div>
  <div class="text-sm font-mono bg-slate-100 p-1 rounded whitespace-normal">D = (x-3)² + x^4</div>

  <div class="col-span-2 border-t border-slate-100"></div>
  <div class="text-sm text-slate-700">Derive and set to 0.</div>
  <div class="text-sm font-mono bg-slate-100 p-1 rounded whitespace-normal">D' = 2(x-3) + 4x³ = 4x³ + 2x - 6 = 0</div>
...
**Question:** Profit P(x) = -0.5x² + 40x - 100. Find units x to maximize profit.
...
<div class="grid grid-cols-2 gap-4 mt-2 border-t border-slate-200 pt-2 min-w-[600px]">
  ...
  <div class="col-span-2 border-t border-slate-100"></div>
  <div class="text-sm text-slate-700">Derive.</div>
  <div class="text-sm font-mono bg-slate-100 p-1 rounded whitespace-normal">y' = 3x² = 0 implies x=0</div>
...
**Question:** Minimize surface area of 1-liter (1000cm³) can. V = π r² h = 1000.
...
<div class="grid grid-cols-2 gap-4 mt-2 border-t border-slate-200 pt-2 min-w-[600px]">
  ...
  <div class="text-sm font-mono bg-slate-100 p-1 rounded whitespace-normal">A = 2π r² + 2π r h</div>

  <div class="col-span-2 border-t border-slate-100"></div>
  <div class="text-sm text-slate-700">Substitute constraint h = 1000/(π r²).</div>
  <div class="text-sm font-mono bg-slate-100 p-1 rounded whitespace-normal">A = 2π r² + 2000/r = 2π r² + 2000r^-1</div>

  <div class="col-span-2 border-t border-slate-100"></div>
  <div class="text-sm text-slate-700">Derive and set zero.</div>
  <div class="text-sm font-mono bg-slate-100 p-1 rounded whitespace-normal">4π r - 2000/r² = 0 implies r³ = (500)/(π)</div>
...
**Question:** Power P = I² R = (V²)/((r + R)²)R. Maximize P with respect to load R.
...
<div class="grid grid-cols-2 gap-4 mt-2 border-t border-slate-200 pt-2 min-w-[600px]">
  ...
  <div class="text-sm font-mono bg-slate-100 p-1 rounded whitespace-normal">P' = V² ≤ft[ ( (r+R)²(1) - R[2(r+R)] )/( (r+R)^4 ) right]</div>

  <div class="col-span-2 border-t border-slate-100"></div>
  <div class="text-sm text-slate-700">Set numerator strictly to zero.</div>
  <div class="text-sm font-mono bg-slate-100 p-1 rounded whitespace-normal">(r+R)² - 2R(r+R) = 0</div>
...
| "Substitute constraint into objective, then take derivative and find critical points." | A = x(10-x) = 10x - x²  A' = 10 - 2x = 0 implies x=5 |
...
<div class="user-quiz" data-question="4. Find the minimum value of y = x² - 10x."></div>
...
* **Standard Definition:** The value L that a function approaches as the input x gets arbitrarily close to a (lim _x \→ a f(x) = L).
...
* **Illustrative Example:** lim _x \→ 0 (sin (x))/(x) = 1. Even though plugging in x=0 yields a (0)/(0) indeterminate state, the limit reveals that the function perfectly approaches the value 1 geometrically.
* **Key Formula (L'Hôpital's Rule):** If approaching a yields (0)/(0), try lim _x \→ a (f'(x))/(g'(x)).

### 2. The Derivative: "The Speedometer"
* **Standard Definition:** The instantaneous rate of change of a function, geometrically represented as the slope of the tangent line. Formally, f'(x) = lim _h \→ 0 (f(x+h) - f(x))/(h).
...
* **Illustrative Example:** If profit over time is given by the equation P(t) = -t² + 10t, the derivative P'(t) = -2t + 10. Setting this to zero, -2t + 10 = 0 \implies t = 5. Thus, peak profit happens exactly at t=5.
* **Essential Formulas:** 
  * Power Rule: (d)/(dx) x^n = n x^n-1 (Bring down the power, subtract one!)
  * Chain Rule: (d)/(dx) f(g(x)) = f'(g(x))g'(x) (Peel the onion: derivative of the outside times derivative of the inside).
...
* **Illustrative Example:** If the rate water flows into a tank is r(t) = 2t gallons per minute, the total water accumulated between 0 and 3 minutes is the integral: ∫_0³ 2t \\, dt = [t²]_0³ = 3² - 0² = 9 gallons.
* **Essential Formulas:**
  * Power Rule: ∫ x^n dx = (x^(n+1))/(n+1) + C
  * Integration by Parts: ∫ u \\, dv = uv - ∫ v \\, du

### 4. The Fundamental Theorem of Calculus: "The Grand Connection"
* **Standard Definition:** ∫_a^b f'(x) dx = f(b) - f(a).
...
* **Illustrative Example:** To find the area under f'(x) = 2x safely from x=1 to x=3, we use the theorem: ∫_1³ 2x \\, dx = [x²]_1³ = 3² - 1² = 9 - 1 = 8.
...
* **Problem:** Find the exact rate of change (derivative) of the nested function f(x) = sin (x³ + 2x).
* **Thinking Method:** This is a function trapped inside another function, like an onion. The outer layer is the sine wave (sin), and the inner core is the polynomial (x³ + 2x). We must peel the onion layer by layer using the "Chain Rule".
* **Worked-Out Solution:** 
  1. **Peel the Outer Layer First:** The derivative of an outer sin (stuff) is cos (stuff). We leave the inside exactly alone for now. This gives us cos (x³ + 2x).
  2. **Attack the Inner Core:** Now, multiply by the derivative of the inner stuff. What is the derivative of x³ + 2x? Using the basic power rule, it's 3x² + 2.
  3. **Chain them together:** We multiply step 1 and step 2. 
  4. The final answer is f'(x) = cos (x³ + 2x) • (3x² + 2).
...
* **Problem:** Differentiate (find the slope rule for) the multiplied function y = x²e^3x.
* **Thinking Method:** We have two completely different mathematical objects—a parabola (x²) and an exponential curve (e^3x)—mashed together by multiplication. We cannot just differentiate them separately. We must use the Product Rule: "First times the derivative of the Second, PLUS Second times the derivative of the First". Let's call the first part u and the second part v.
* **Worked-Out Solution:** 
  1. Define u = x². Its derivative is exactly u' = 2x.
  2. Define v = e^3x. Its derivative requires a mini chain-rule for the power 3x. So v' = e^3x • 3 = 3e^3x.
...
* **Problem:** Find the accumulated total (evaluate the integral) of ∫ 2x cos (x²) dx.
* **Thinking Method:** Integration can be an absolute nightmare if the pieces don't fit. But notice a clue! We have an x² trapped inside a cosine, and perfectly waiting on the outside is its exact derivative, 2x. This is a screaming signal to use "U-Substitution"—we temporarily disguise x² to simplify the visual puzzle.
* **Worked-Out Solution:** 
  1. Choose a disguise variable: Let u perfectly equal the trapped inside portion, u = x².
  2. Take the derivative to find the trade-in cost: du = 2x \\, dx. 
  3. Look at our original problem: ∫ cos (x²) (2x \\, dx). The pieces perfectly translate to our new disguise variables u and du!
  4. Rewrite the clean, disguised problem: ∫ cos (u) \\, du. So much simpler!
  5. The anti-derivative of cos (u) is just sin (u).
  6. Final step: Rip off the disguise. Replace u back with x². 
  7. The final mathematical answer is sin (x²) + C (Never forget the constant C which represents any starting point height).
...
* **Problem:** Calculate the exact physical area under the curve of 3x² - 4x + 1 strictly between the boundaries of x=1 and x=3. Notation: ∫_1³ (3x² - 4x + 1) dx.
...
* **Worked-Out Solution:** 
  ...
  2. 3x² builds up to x³.
  3. -4x builds up to -2x².
  4. +1 builds up to x.
  5. Step 2: Write out the evaluated bounds: [x³ - 2x² + x]_1³.
  6. Step 3: Plug in the top ceiling number (3) first: (3)³ - 2(3)² + (3) = 27 - 18 + 3 = 12.
  7. Step 4: Plug in the bottom floor number (1): (1)³ - 2(1)² + (1) = 1 - 2 + 1 = 0.
...
* **Problem:** Evaluate the limit of lim _x \→ 2 (x² - 4)/(x - 2).
* **Thinking Method:** The very first rule of limits is to just aggressively plug in the target number. Let's try plugging in 2. The top becomes 4-4 = 0. The bottom becomes 2-2 = 0. We hit (0)/(0). This is *not* a dead end; it's a flashing sign that says "Hold on, there's a trick to mathematically clean this up!" We have an algebraic 'hole' blocking us. Let's factor!
* **Worked-Out Solution:** 
  1. Look at the top piece: x² - 4. This is a classic "Difference of Squares" pattern from algebra class. It breaks down perfectly into (x-2)(x+2).
  2. Rewrite the limit fraction: lim _x \→ 2 ((x-2)(x+2))/(x - 2).
  ...
  4. The messy fraction collapses into a beautiful, clean line: lim _x \→ 2 (x+2).
...
| Inner voice | What you write |
| :--- | :--- |
| "Direct substitution gives 0/0. I need to factor the numerator to remove the discontinuity." | ((x-2)(x+2))/(x-2) |
| "Cancel the (x-2) terms, then plug in x = 2 again." | lim _x → 2 (x+2) = 4 |
...
| If you see dots | Ask yourself dots | Action |
| :--- | :--- | :--- |
| lim _x → a (f(x))/(g(x)) = (0)/(0) | Can I factor or use L'Hôpital? | Differentiate top and bottom. |
| ∞ / ∞ | Which grows faster? | Divide by highest power or L'Hôpital. |
...
>
> 2. Evaluate the definite integral ∫_0^π sin (x) dx.
*(Use the Chat Attachment Button to upload your mathematical derivation for grading!)*

> **QUIZ:** Compute the limit as x approaches 0 of sin(3x) / x.
>
> 3. Compute lim _x \→ 0 (sin (3x))/(x).
*(Use the Chat Attachment Button to upload your mathematical derivation for grading!)*
...
**The Concept:**
We are going to completely strip away the terrifying formula u • v = |u||v|cos(θ) for a moment. Instead, I want you to learn the Dot Product simply as **"Agreement."** The dot product answers one vital physical question: *How much do these two vectors work together?* 
...
When you cross Vector **r** with Vector **F** (written as r × F), you are calculating the torque driving the bolt downward into the engine block. The resulting vector points along the axis of the bolt, entirely perpendicular to your wrench and your pushing hand!
...
- **Dot Product (u • v):** The "Agreement Multiplier." A scalar value representing how much two vectors point in the same direction.
- **Cross Product (u × v):** The "Twist Multiplier." A vector operation that generates a new vector perfectly perpendicular to the initial two vectors.
...
## Visuals / Graphs
### 0. Points and Distance in Space
*(Interactive Visualization)*

<div class="plot-interactive" data-plot-id="3d_coordinate_system"></div>
...
### 4. Plane and Normal Vector
*(Interactive Visualization)*

<div class="plot-interactive" data-plot-id="plane_equation"></div>
...
**Question:** An industrial robot arm moves a component along vector u = ⟨ 2, 4, -1 ⟩, then pushes it along vector v = ⟨ -1, 3, 2 ⟩. Find the net displacement.
...
<table class="w-full text-left border-collapse mt-2">
  ...
  <tbody class="divide-y divide-slate-200">
    <tr>
      ...
      <td class="py-2 pr-4 font-mono bg-slate-100 p-1 rounded">u + v = ⟨ 2+(-1), 4+3, (-1)+2 ⟩</td>
    </tr>
    <tr>
      ...
      <td class="py-2 pr-4 font-mono bg-slate-100 p-1 rounded">= ⟨ 1, 7, 1 ⟩</td>
    </tr>
  </tbody>
</table>
</div>
...
**Question:** A technician pushes a cart with force F = ⟨ 3, 4, 0 ⟩ along a path D = ⟨ 6, 8, 0 ⟩. Calculate the work done (the dot product F • D).
...
<table class="w-full text-left border-collapse mt-2">
  ...
  <tbody class="divide-y divide-slate-200">
    <tr>
      ...
      <td class="py-2 pr-4 font-mono bg-slate-100 p-1 rounded">W = F • D = (3)(6) + (4)(8) + (0)(0)</td>
    </tr>
    <tr>
...
**Question:** Does the magnetic field vector B = ⟨ 2, -1, 4 ⟩ pierce perpendicularly through a solar panel oriented along v = ⟨ 1, 6, 1 ⟩? Check if they are orthogonal.
...
<table class="w-full text-left border-collapse mt-2">
  ...
  <tbody class="divide-y divide-slate-200">
    <tr>
      ...
      <td class="py-2 pr-4 font-mono bg-slate-100 p-1 rounded">B • v = (2)(1) + (-1)(6) + (4)(1)</td>
    </tr>
    <tr>
...
**Question:** Calculate torque \\tau = r × F if the wrench handle vector is r = ⟨ 1, 2, 0 ⟩ and applied force is F = ⟨ 0, 3, 0 ⟩.
...
<table class="w-full text-left border-collapse mt-2">
  ...
  <tbody class="divide-y divide-slate-200">
    <tr>
      ...
      <td class="py-2 pr-4 font-mono bg-slate-100 p-1 rounded">τ = Det[ e1 e2 e3 | 1 2 0 | 0 3 0 ]</td>
    </tr>
    <tr>
      ...
      <td class="py-2 pr-4 font-mono bg-slate-100 p-1 rounded">τ = 0e_1 - 0e_2 + (3 - 0)e_3 = ⟨ 0, 0, 3 ⟩</td>
    </tr>
  </tbody>
</table>
</div>
...
**Question:** Find a vector perfectly perpendicular to the plane formed by u = ⟨ 1, -1, 0 ⟩ and v = ⟨ 2, 0, 1 ⟩.
...
<table class="w-full text-left border-collapse mt-2">
  ...
  <tbody class="divide-y divide-slate-200">
    <tr>
      <td class="py-2 pr-4 text-slate-700">I need a vector perpendicular to two existing vectors on a plane. This is exactly what the "Twist Multiplier" (Cross Product) produces! I will compute u × v.</td>
      <td class="py-2 pr-4 font-mono bg-slate-100 p-1 rounded">N = u × v = Det[ e1 e2 e3 | 1 -1 0 | 2 0 1 ]</td>
    </tr>
    <tr>
      ...
      <td class="py-2 pr-4 font-mono bg-slate-100 p-1 rounded">= ⟨ -1, -1, 2 ⟩</td>
    </tr>
  </tbody>
</table>
</div>
...
**Question:** An electron gets shot out of a cathode ray tube from position P(1, 4, -2) travelling rigidly along instruction vector v = ⟨ 2, -1, 3 ⟩. Write the line equation for its path.
...
<table class="w-full text-left border-collapse mt-2">
  ...
  <tbody class="divide-y divide-slate-200">
    <tr>
      <td class="py-2 pr-4 text-slate-700">A line equation just asks where I am right now, plus how far I travel along my instruction arrow. Let t be time. Starting point + (time × velocity arrow).</td>
      <td class="py-2 pr-4 font-mono bg-slate-100 p-1 rounded">L(t) = P_0 + t • v</td>
    </tr>
    <tr>
      <td class="py-2 pr-4 text-slate-700">Just plug the point and vector straight into the format.</td>
      <td class="py-2 pr-4 font-mono bg-slate-100 p-1 rounded">L(t) = ⟨ 1, 4, -2 ⟩ + t⟨ 2, -1, 3 ⟩</td>
    </tr>
    <tr>
...
**Question:** Calculate the angle between a magnetic field trace B = ⟨ 1, 1, 1 ⟩ and a sensor face S = ⟨ 1, 0, 0 ⟩.
...
<table class="w-full text-left border-collapse mt-2">
  ...
  <tbody class="divide-y divide-slate-200">
    <tr>
      <td class="py-2 pr-4 text-slate-700">To find angles between spatial arrows, always use the Agreement Multiplier (Dot Product). cos(θ) = (B • S) / (|B||S|). First get lengths.</td>
      <td class="py-2 pr-4 font-mono bg-slate-100 p-1 rounded">|B| = √(1^2+1^2+1^2) = √(3)<br>|S| = √(1^2) = 1</td>
    </tr>
    <tr>
      <td class="py-2 pr-4 text-slate-700">Now their dot product. (1)(1) + (1)(0) + (1)(0) = 1.</td>
      <td class="py-2 pr-4 font-mono bg-slate-100 p-1 rounded">B • S = 1</td>
    </tr>
    <tr>
      <td class="py-2 pr-4 text-slate-700">Now construct the arccosine to reveal the physical angle.</td>
      <td class="py-2 pr-4 font-mono bg-slate-100 p-1 rounded">cos(θ) = 1/√(3(1)) &rArr; θ = arccos(1/√(3))</td>
    </tr>
  </tbody>
</table>
</div>
...
**Question:** Construct the equation of a sheet of steel (a plane) that passes through point P(2, 3, 4) and has an iron nail (normal vector) pointing perfectly along n = ⟨ 1, -2, 5 ⟩.
...
<table class="w-full text-left border-collapse mt-2">
  ...
  <tbody class="divide-y divide-slate-200">
    <tr>
      <td class="py-2 pr-4 text-slate-700">The plane equation just forces any random point (x,y,z) on the plane to form a flat line with the start point P, such that it has ZERO agreement with the normal nail. So, n • ⟨ x-p_x, y-p_y, z-p_z ⟩ = 0.</td>
      <td class="py-2 pr-4 font-mono bg-slate-100 p-1 rounded">A(x-x_0) + B(y-y_0) + C(z-z_0) = 0<br>Where  ⟨ A,B,C ⟩ = n</td>
    </tr>
    <tr>
    ...
    <tr>
      ...
      <td class="py-2 pr-4 font-mono bg-slate-100 p-1 rounded">x - 2y + 5z = 2 - 6 + 20 &rArr; x - 2y + 5z = 16</td>
    </tr>
  </tbody>
</table>
</div>
...
<table class="w-full text-left border-collapse mt-2">
  ...
  <tbody class="divide-y divide-slate-200">
    <tr>
      ...
      <td class="py-2 pr-4 font-mono bg-slate-100 p-1 rounded">n_1 = ⟨ 2, -4, 6 ⟩<br>n_2 = ⟨ -1, 2, -3 ⟩</td>
    </tr>
    <tr>
      ...
      <td class="py-2 pr-4 font-mono bg-slate-100 p-1 rounded">n_1 = -2 • n_2. The normal vectors are parallel, therefore the planes are parallel.</td>
    </tr>
  </tbody>
</table>
</div>
...
**Question:** Find the projection (the vertical shadow) of vector u = ⟨ 1, 1, 2 ⟩ falling down directly onto vector v = ⟨ -2, 3, 1 ⟩.
...
<table class="w-full text-left border-collapse mt-2">
  ...
  <tbody class="divide-y divide-slate-200">
    <tr>
      ...
      <td class="py-2 pr-4 font-mono bg-slate-100 p-1 rounded">proj_v u = \\left( u • v/v • v \right) v</td>
    </tr>
    <tr>
      <td class="py-2 pr-4 text-slate-700">I need the dot products. u • v = -2 + 3 + 2 = 3. And v • v = (-2)^2 + 3^2 + 1^2 = 4 + 9 + 1 = 14.</td>
      <td class="py-2 pr-4 font-mono bg-slate-100 p-1 rounded">u • v = 3 <br> v • v = 14</td>
    </tr>
    <tr>
      ...
      <td class="py-2 pr-4 font-mono bg-slate-100 p-1 rounded">proj_v u = 3/14 ⟨ -2, 3, 1 ⟩ = ⟨ -6/14, 9/14, 3/14 ⟩</td>
    </tr>
  </tbody>
</table>
</div>
  ...
  <table class="w-full text-left border-collapse bg-white">
    ...
    <tbody class="text-sm">
      <tr class="hover:bg-slate-50 transition-colors border-b border-slate-100">
        ...
        <td class="p-4 font-mono text-indigo-600 bg-slate-50 border-l border-slate-100">|v| = √(x^2+y^2+z^2)</td>
      ...
      <tr class="hover:bg-slate-50 transition-colors border-b border-slate-100">
        ...
        <td class="p-4 font-mono text-indigo-600 bg-slate-50 border-l border-slate-100">u • v = u_1 v_1 + u_2 v_2 + u_3 v_3</td>
      ...
      <tr class="hover:bg-slate-50 transition-colors border-b border-slate-100">
        ...
        <td class="p-4 font-mono text-indigo-600 bg-slate-50 border-l border-slate-100">u × v = det(e_i, u, v)</td>
        <td class="p-4 text-slate-600 border-l border-slate-100">If exactly ⟨ 0,0,0 ⟩, the vectors are **parallel**.</td>
      </tr>
      <tr class="hover:bg-slate-50 transition-colors border-b border-slate-100">
        <td class="p-4 font-medium text-slate-800">Line Equation</td>
        <td class="p-4 font-mono text-indigo-600 bg-slate-50 border-l border-slate-100">r(t) = P_{start} + t • v_{dir}</td>
...
4.  **Poynting Vector, S = E × H:** Represents the directional energy flux (the rate of energy transfer per unit area) of an electromagnetic field.
...
F = (1)/(4π\varepsilon_0) (q q_0)/(r²) \hat{u}

Where \hat{u} is the unit vector pointing from q to q_0. The Electric Field E is defined as force per unit charge:
E(r) = (F(r))/(q_0) = (1)/(4π\varepsilon_0) (q)/(r²) \hat{u}

Often, the position vector from the origin to (x, y, z) is denoted by r = ix + jy + kz. Its magnitude is |r| = \√(x² + y² + z²). The unit vector pointing radially outward is \hat{r} = (r)/(|r|).
...
4.  **Check Magnitude Variation:** Calculate |F| = \√(F_x² + F_y² + F_z²). If |F| increases with distance from origin, arrows get longer as you go outward. If it decreases (like 1/r²), arrows get shorter.
...
2.  **Are you verifying if a field is radial or tangential?**
    *   Radial fields are parallel to r = ix + jy + kz. Take the cross product F × r. If it is 0, the field is purely radial.
    *   Tangential fields are perpendicular to r. Take the dot product F • r. If it is 0, the field is tangential.
3.  **Are you applying superposition?**
    *   Find the vector expression for each individual field.
    *   Add them component by component: F_total = i(F_1x + F_2x) + j(F_1y + F_2y) + k(F_1z + F_2z).

## Concept Recall: Trigger Words → Action

*   *"Evaluate the vector field..."* → Plug the given coordinate point (x, y, z) into the scalar component functions F_x, F_y, F_z.
*   *"Is the field radial?"* → Check if F is a scalar multiple of r = ix + jy + kz. Alternatively, check if F × r = 0.
*   *"Is the field circulating/tangential?"* → Check if the dot product of F and the position vector r is zero (F • r = 0).
*   *"Charge Q is located at... finding field..."* → Use E = (Q)/(4π\varepsilon_0) (r) / |r|³, where \mathbf{r} is the distance vector from the charge to the evaluation point.
...
<table class="w-full text-left border-collapse mt-2">
  ...
  <tbody class="divide-y divide-slate-200">
    ...
    <tr>
      <td class="py-2 pr-4 text-slate-700">At (1,0) the vector is i. At (0,1) it's j. At (x,y), the vector points from origin to (x,y) with length \√(x²+y²). This is radiating outwards.</td>
...
**Question:** Evaluate F(1, 2, 3) for F(x, y, z) = i(x²) + j(yz) + k(z²).
...
<table class="w-full text-left border-collapse mt-2">
  ...
  <tbody class="divide-y divide-slate-200">
    <tr>
      ...
      <td class="py-2 pr-4 font-mono bg-slate-100 p-1 rounded">F(1, 2, 3) = i(1²) + j(2 • 3) + k(3²)</td>
    </tr>
    <tr>
...
**Question:** Sketch F(x, y) = (-iy + jx)/(\√(x² + y²)).
...
<table class="w-full text-left border-collapse mt-2">
  ...
  <tbody class="divide-y divide-slate-200">
    <tr>
      <td class="py-2 pr-4 text-slate-700">Calculate magnitude: \frac{\√((-y)² + x²)}{\√(x²+y²)} = 1. So all arrows have length 1.</td>
      <td class="py-2 pr-4 font-mono bg-slate-100 p-1 rounded">|F| = \frac{\√(y² + x²)}{\√(x² + y²)} = 1</td>
    </tr>
    <tr>
      ...
      <td class="py-2 pr-4 font-mono bg-slate-100 p-1 rounded">F • r = (-yx + xy)/(\√(x²+y²)) = 0.<br>[Sketch uniform length arrows circulating counterclockwise around origin]</td>
    </tr>
  </tbody>
</table>
</div>
...
<table class="w-full text-left border-collapse mt-2">
  ...
  <tbody class="divide-y divide-slate-200">
    <tr>
      <td class="py-2 pr-4 text-slate-700">Coulomb's law gives field magnitude as proportional to 1/r² and pointing radially via unit vector \hat{u}.</td>
      <td class="py-2 pr-4 font-mono bg-slate-100 p-1 rounded">E = (1)/(4π\varepsilon_0) (q)/(|r|²) \hat{u}</td>
    </tr>
    <tr>
      <td class="py-2 pr-4 text-slate-700">Replace \hat{u} with (r)/(|r|), where r = ix + jy + kz and |r| = \√(x² + y² + z²).</td>
      <td class="py-2 pr-4 font-mono bg-slate-100 p-1 rounded">E(r) = (q)/(4π\varepsilon_0) (ix + jy + kz)/((x² + y² + z²)^3/2)</td>
    </tr>
  </tbody>
</table>
</div>
...
<table class="w-full text-left border-collapse mt-2">
  ...
  <tbody class="divide-y divide-slate-200">
    <tr>
      ...
      <td class="py-2 pr-4 font-mono bg-slate-100 p-1 rounded">F_1 • F_2 = (x)(-y) + (y)(x) = -xy + yx = 0</td>
    </tr>
    <tr>
...
**Question:** Two charges produce respective electric fields E_1 = 3i - 2j and E_2 = xi + yj. What is the total field E_total at (1, 4)?
...
<table class="w-full text-left border-collapse mt-2">
  ...
  <tbody class="divide-y divide-slate-200">
    <tr>
      ...
      <td class="py-2 pr-4 font-mono bg-slate-100 p-1 rounded">E_total = E_1 + E_2</td>
    </tr>
    <tr>
      <td class="py-2 pr-4 text-slate-700">Evaluate E_2 at (1,4), then add to the constant field E_1.</td>
      <td class="py-2 pr-4 font-mono bg-slate-100 p-1 rounded">E_2(1,4) = 1i + 4j<br>E_total = (3i - 2j) + (1i + 4j) = 4i + 2j</td>
    </tr>
  </tbody>
</table>
</div>
...
<table class="w-full text-left border-collapse">
  ...
  <tbody class="divide-y divide-slate-200">
    ...
    <tr>
      <td class="py-3 pr-4 text-slate-800">|F| = \√(F_x² + F_y² + F_z²)</td>
...
*   **Trap 1: Confusing vector functions with scalar functions.** A scalar function T(x,y,z)=x²+y² gives one number. A vector function F(x,y,z) = ix² + jy² gives a vector with magnitude and direction. Don't add them.
...
*   **Trap 4: Incorrect magnitude scaling.** In sketching 1/r² fields (like Coulomb or Gravity), students often draw arrows getting longer as they go outwards. WRONG. They should rapidly shrink.
...
The divergence operator div F (or ∇ • F) acts like a mathematical detector probe. Move it around in a vector field, and it reads a positive number over sources, a negative number over sinks, and zero everywhere else.
...
*   **Divergence (div F or ∇ • F):** A scalar measure of a vector field's tendency to originate from or converge upon a given point.
...
*   **Solenoidal Field:** A vector field with zero divergence everywhere (∇ • F = 0). Magnetic fields are always solenoidal.
...
1.  **Gauss' Law:** ∇ • E = rho / \varepsilon_0. The divergence of the electric field at a point is directly proportional to the charge density at that point. Positive charges are sources of the E-field, negative charges are sinks.
2.  **Continuity Equation:** ∇ • J + (∂ rho)/(∂ t) = 0. This expresses the conservation of charge. If current J is diverging away from a point, the charge density rho at that point must be decreasing over time.
3.  **No Magnetic Monopoles:** ∇ • B = 0. Magnetic fields have no point sources or sinks. They always form closed loops.

## Formal Definitions and Notation

The divergence is formally defined as the limit of the surface integral of F (flux) over a closed surface S shrinking to a point, divided by the volume Δ V enclosed by S:

div F = lim _Δ V → 0 (1)/(Δ V) ∬_S F • n} , dS

In manageable Cartesian coordinates, divergence is the dot product of the del operator ∇ = i(∂)/(∂ x) + j(∂)/(∂ y) + k(∂)/(∂ z) with the vector field F:

div F = ∇ • F = (∂ F_x)/(∂ x) + (∂ F_y)/(∂ y) + (∂ F_z)/(∂ z)

### Cylindrical Coordinates (r, θ, z):
∇ • F = (1)/(r) (∂)/(∂ r)(r F_r) + (1)/(r) (∂ F_θ)/(∂ θ) + (∂ F_z)/(∂ z)

### Spherical Coordinates (r, θ, φ):
∇ • F = (1)/(r²) (∂)/(∂ r)(r² F_r) + (1)/(r sin φ) (∂)/(∂ φ)(sin φ F_φ) + (1)/(r sin φ) (∂ F_θ)/(∂ θ)
...
1.  **Identify Coordinate System:** Is the field F written in terms of (x,y,z), (r,θ,z), or (r,θ,φ)?
2.  **Extract Components:** Write down the individual components F_x, F_y, F_z (or F_r, F_θ, etc.).
...
4.  **Differentiate:** Carefully take the partial derivative of each component with respect to its corresponding variable.
    *   *Warning in non-Cartesian:* Don't forget that inside the derivative, F_r is multiplied by r or r² before deriving!
...
1.  **Are you asked to find "Divergence" or "∇ • F"?**
    *   Apply the partial derivative formula directly to the components.
2.  **Are you asked for "Flux through a closed surface"?**
    *   Is the integral hard? Use the Divergence Theorem instead: compute ∇ • F, then integrate that simple scalar over the volume.
3.  **Are you given an electric field and asked for charge density rho?**
    *   Compute ∇ • E.
    *   Multiply by \varepsilon_0 because rho = \varepsilon_0 (∇ • E).

## Concept Recall: Trigger Words → Action

*   *"Calculate divergence..."* → Drop the i, j, k. Take (∂ F_x)/(∂ x) + (∂ F_y)/(∂ y) + (∂ F_z)/(∂ z).
...
*   *"Is the field solenoidal?"* → Calculate ∇ • F. Does it precisely equal 0 everywhere?
*   *"Is it a source or a sink?"* → Check the sign of ∇ • F. Positive = source, Negative = sink.
...
**Question:** Find ∇ • F for F(x, y, z) = ix² + jy² + kz².
...
<table class="w-full text-left border-collapse mt-2">
  ...
  <tbody class="divide-y divide-slate-200">
    <tr>
      ...
      <td class="py-2 pr-4 font-mono bg-slate-100 p-1 rounded">∇ • F = (∂)/(∂ x)(x²) + (∂)/(∂ y)(y²) + (∂)/(∂ z)(z²)</td>
    </tr>
    <tr>
      <td class="py-2 pr-4 text-slate-700">Just apply power rule to each independently. The answer must be a scalar.</td>
      <td class="py-2 pr-4 font-mono bg-slate-100 p-1 rounded">∇ • F = 2x + 2y + 2z</td>
    </tr>
  </tbody>
</table>
</div>
...
**Question:** Find ∇ • F for F(x, y, z) = iy + jx + k(0).
...
<table class="w-full text-left border-collapse mt-2">
  ...
  <tbody class="divide-y divide-slate-200">
    <tr>
      <td class="py-2 pr-4 text-slate-700">F_x = y, F_y = x. Take ∂ F_x/∂ x and ∂ F_y/∂ y.</td>
      <td class="py-2 pr-4 font-mono bg-slate-100 p-1 rounded">∇ • F = (∂)/(∂ x)(y) + (∂)/(∂ y)(x) + (∂)/(∂ z)(0)</td>
    </tr>
    <tr>
      <td class="py-2 pr-4 text-slate-700">The derivative of y with respect to x is zero. The field is solenoidal.</td>
      <td class="py-2 pr-4 font-mono bg-slate-100 p-1 rounded">∇ • F = 0 + 0 + 0 = 0<br>This field is solenoidal.</td>
    </tr>
  </tbody>
</table>
</div>
...
**Question:** Given E = (q)/(4π\varepsilon_0) (ix + jy + kz)/((x²+y²+z²)^3/2), state ∇ • E for r \≠q 0.
...
<table class="w-full text-left border-collapse mt-2">
  ...
  <tbody class="divide-y divide-slate-200">
    ...
    <tr>
      ...
      <td class="py-2 pr-4 font-mono bg-slate-100 p-1 rounded">∇ • E = 0 for r \≠q 0.</td>
    </tr>
  </tbody>
</table>
</div>
...
**Question:** A vector field F has ∇ • F = 2 at point (1, 1, 1). What does this mean?
...
**Question:** Find ∇ • F for F(r, θ, z) = \hat{e}_r (r²).
...
<table class="w-full text-left border-collapse mt-2">
  ...
  <tbody class="divide-y divide-slate-200">
    <tr>
      <td class="py-2 pr-4 text-slate-700">The field is in cylindrical coordinates. The only non-zero component is F_r = r².</td>
      <td class="py-2 pr-4 font-mono bg-slate-100 p-1 rounded">∇ • F = (1)/(r) (∂)/(∂ r)(r F_r)</td>
    </tr>
    <tr>
      <td class="py-2 pr-4 text-slate-700">I must multiply F_r by r BEFORE differentiating! So I derive r • r² = r³.</td>
      <td class="py-2 pr-4 font-mono bg-slate-100 p-1 rounded">∇ • F = (1)/(r) (∂)/(∂ r)(r³) = (1)/(r) (3r²) = 3r</td>
    </tr>
  </tbody>
</table>
</div>
...
**Question:** Find ∇ • F for F(r, θ, φ) = \hat{e}_r (r²).
...
<table class="w-full text-left border-collapse mt-2">
  ...
  <tbody class="divide-y divide-slate-200">
    <tr>
      <td class="py-2 pr-4 text-slate-700">Spherical coordinates. Only F_r = r² is non-zero. The spherical formula uses a 1/r² multiplier.</td>
      <td class="py-2 pr-4 font-mono bg-slate-100 p-1 rounded">∇ • F = (1)/(r²) (∂)/(∂ r)(r² F_r)</td>
    </tr>
    <tr>
      <td class="py-2 pr-4 text-slate-700">Multiply F_r by r² first. Make it r^4. Then take derivative.</td>
      <td class="py-2 pr-4 font-mono bg-slate-100 p-1 rounded">∇ • F = (1)/(r²) (∂)/(∂ r)(r^4) = (1)/(r²) (4r³) = 4r</td>
    </tr>
  </tbody>
</table>
</div>
...
<table class="w-full text-left border-collapse mt-2">
  ...
  <tbody class="divide-y divide-slate-200">
    <tr>
      ...
      <td class="py-2 pr-4 font-mono bg-slate-100 p-1 rounded">∇ • F = 1 + 1 + 1 = 3</td>
    </tr>
    <tr>
      <td class="py-2 pr-4 text-slate-700">Volume integral of the constant 3 is just 3 times the volume of the sphere.</td>
      <td class="py-2 pr-4 font-mono bg-slate-100 p-1 rounded">\iiint_V 3 \\, dV = 3 • ((4)/(3)π R³) = 4π R³</td>
    </tr>
  </tbody>
</table>
</div>
...
**Question:** If ∇ • E = rho / \varepsilon_0 and E = i(\α x) + j(\β y) + k(\γ z), find charge density rho.
...
<table class="w-full text-left border-collapse mt-2">
  ...
  <tbody class="divide-y divide-slate-200">
    <tr>
      ...
      <td class="py-2 pr-4 font-mono bg-slate-100 p-1 rounded">∇ • E = \α + \β + \γ</td>
    </tr>
    <tr>
      <td class="py-2 pr-4 text-slate-700">Equate to Gauss' Law differential form.</td>
      <td class="py-2 pr-4 font-mono bg-slate-100 p-1 rounded">\α + \β + \γ = rho / \varepsilon_0 \implies rho = \varepsilon_0(\α + \β + \γ)</td>
    </tr>
  </tbody>
</table>
</div>
...
<table class="w-full text-left border-collapse mt-2">
  ...
  <tbody class="divide-y divide-slate-200">
    <tr>
      ...
      <td class="py-2 pr-4 font-mono bg-slate-100 p-1 rounded">∇ • F = 1 + 1 = 2</td>
    </tr>
    <tr>
      <td class="py-2 pr-4 text-slate-700">It is a positive constant 2. Because it is positive, matter is expanding outward.</td>
      <td class="py-2 pr-4 font-mono bg-slate-100 p-1 rounded">∇ • F = 2 > 0.<br>The origin (and everywhere else) is a **source**.</td>
    </tr>
  </tbody>
</table>
</div>
...
**Question:** If (∂ rho)/(∂ t) + ∇ • J = 0 and charge density rho is increasing, what is the sign of ∇ • J?
...
<table class="w-full text-left border-collapse mt-2">
  ...
  <tbody class="divide-y divide-slate-200">
    <tr>
      <td class="py-2 pr-4 text-slate-700">If rho is increasing, then ∂ rho / ∂ t is a positive quantity.</td>
      <td class="py-2 pr-4 font-mono bg-slate-100 p-1 rounded">(∂ rho)/(∂ t) > 0</td>
    </tr>
    <tr>
      ...
      <td class="py-2 pr-4 font-mono bg-slate-100 p-1 rounded">∇ • J = - (∂ rho)/(∂ t) < 0<br>∇ • J is negative.</td>
    </tr>
  </tbody>
</table>
</div>
...
<table class="w-full text-left border-collapse">
  ...
  <tbody class="divide-y divide-slate-200">
    <tr>
      <td class="py-3 pr-4 text-slate-800">Cartesian ∇ • F</td>
      <td class="py-3 pr-4 text-slate-800">Do I just sum derivatives?</td>
      <td class="py-3 pr-4 text-slate-800">Yes, ∂ F_x / ∂ x + ∂ F_y / ∂ y + ∂ F_z / ∂ z. Drop the i, j, k.</td>
    </tr>
    <tr>
      <td class="py-3 pr-4 text-slate-800">Cylindrical ∇ • F</td>
      <td class="py-3 pr-4 text-slate-800">Did I multiply by r before deriving?</td>
      <td class="py-3 pr-4 text-slate-800">Use formula: (1)/(r) (∂)/(∂ r)(r F_r)</td>
    </tr>
    <tr>
      <td class="py-3 pr-4 text-slate-800">Spherical ∇ • F</td>
      <td class="py-3 pr-4 text-slate-800">Did I multiply by r² before deriving?</td>
      <td class="py-3 pr-4 text-slate-800">Use formula: (1)/(r²) (∂)/(∂ r)(r² F_r)</td>
    </tr>
    <tr>
      ...
      <td class="py-3 pr-4 text-slate-800">Use Divergence Theorem! \iiint (∇ • F) dV</td>
    </tr>
    <tr>
      <td class="py-3 pr-4 text-slate-800">Gauss' Law</td>
      <td class="py-3 pr-4 text-slate-800">Finding charge density?</td>
      <td class="py-3 pr-4 text-slate-800">rho = \varepsilon_0 (∇ • E)</td>
    </tr>
  </tbody>
</table>
...
*   **Trap 2: Ignoring coordinate system modifiers.** In cylindrical, the radial derivative is NOT just ∂ F_r / ∂ r. It is (1)/(r) (∂)/(∂ r)(r F_r). If you don't multiply F_r by r first, you fail.
...
*   **Circulation:** The line integral of a vector field around a closed loop (\oint_C F • \hat{t} \\, ds). Matches "work done" if F is a force.
*   **Curl (∇ × F):** A microscopic measure of rotation. Vector operator pointing along the axis of rotation.
*   **Irrotational Field:** A vector field where the curl is exactly zero everywhere (∇ × F = 0). Also called a conservative field.
...
*   **Ampère's Law:** ∇ × B = \μ_0 J. The curl of the magnetic field is created by current density J.

## Why This Matters in Electrical Engineering

Curl is the mathematical engine behind two of Maxwell's Equations:
1.  **Ampère's Law:** ∇ × B = \μ_0 J. Electric currents cause magnetic fields to circulate around them. This is how electromagnets and motors work.
2.  **Faraday's Law of Induction:** ∇ × E = - (∂ B)/(∂ t). A changing magnetic field generates a circulating electric field. This is how transformers, inductors, and generators produce voltage.
3.  **Magnetic Vector Potential:** Because ∇ • B = 0, we can define B = ∇ × A, converting magnetic field problems into a simpler vector potential space.

## Formal Definitions and Notation

The curl of F, denoted curl F or ∇ × F, is formally the limit of circulation per unit area:

(∇ × F) • n} = lim _Δ A → 0 (1)/(Δ A) ∮_C F • t} , ds
...
1.  **Write the Matrix:** Draw a 3x3 matrix.
    ...
    *   Middle row: partial derivative operators (∂)/(∂ x), (∂)/(∂ y), (∂)/(∂ z).
    *   Bottom row: the scalar components of the field F_x, F_y, F_z.
2.  **Evaluate the i component:** Cross out row 1 and column 1. Subtract the cross-diagonal derivatives: (∂)/(∂ y) (F_z) - (∂)/(∂ z) (F_y). Multiply by i.
3.  **Evaluate the j component:** Cross out row 1 and column 2. Apply derivatives similarly, but remember the **negative sign** inherent to the middle determinant expansion: - [ (∂)/(∂ x) (F_z) - (∂)/(∂ z) (F_x) ] • j.
4.  **Evaluate the k component:** Cross out row 1 and column 3. Calculate (∂)/(∂ x) (F_y) - (∂)/(∂ y) (F_x). Multiply by k.
...
2.  **Are you evaluating a circulation integral \oint F • ds?**
    *   If taking the path integral directly is an algebraic nightmare, use Stokes' Theorem. Calculate the curl ∇ × F, then take the surface integration \iint (∇ × F) • \hat{n} \\, dS.
3.  **Are you checking for path independence (conservative field)?**
    *   Calculate ∇ × F. If the curl is the zero vector, line integrals between two points are independent of the path taken.

## Concept Recall: Trigger Words → Action

*   *"Determine if the field is conservative..."* → Calculate ∇ × F. If it is 0, it is conservative (irrotational).
*   *"Find the work done moving a particle around closed loop C..."* → Work = \oint F • ds. If the loop creates a complex shape, use Stokes' Theorem to evaluate the surface integral of the curl instead.
*   *"Calculate circulation..."* → Another phrase for \oint F • ds. Use Stokes' theorem.
...
**Question:** Find ∇ × F for F(x, y, z) = iy - jx.
...
<table class="w-full text-left border-collapse mt-2">
  ...
  <tbody class="divide-y divide-slate-200">
    <tr>
      ...
      <td class="py-2 pr-4 font-mono bg-slate-100 p-1 rounded">∇ × F = | i\\; j\\; k |<br>| ∂_x\\; ∂_y\\; ∂_z |<br>| y\\; -x\\; 0 |</td>
    </tr>
    <tr>
      <td class="py-2 pr-4 text-slate-700">i-comp: ∂(0)/∂ y - ∂(-x)/∂ z = 0<br>j-comp: -[∂(0)/∂ x - ∂(y)/∂ z] = 0<br>k-comp: ∂(-x)/∂ x - ∂(y)/∂ y = -1 - 1 = -2</td>
      <td class="py-2 pr-4 font-mono bg-slate-100 p-1 rounded">∇ × F = -2k.</td>
    </tr>
  </tbody>
</table>
</div>
...
<table class="w-full text-left border-collapse mt-2">
  ...
  <tbody class="divide-y divide-slate-200">
    <tr>
      <td class="py-2 pr-4 text-slate-700">Set up the k component since F_z=0 and everything is flat. k: ∂_x(x) - ∂_y(y).</td>
      <td class="py-2 pr-4 font-mono bg-slate-100 p-1 rounded">k-component: (∂)/(∂ x)(x) - (∂)/(∂ y)(y)</td>
    </tr>
    <tr>
      <td class="py-2 pr-4 text-slate-700">1 - 1 = 0. Since all components evaluate to 0, this field is conservative.</td>
      <td class="py-2 pr-4 font-mono bg-slate-100 p-1 rounded">∇ × F = 0. (Irrotational)</td>
    </tr>
  </tbody>
</table>
</div>
...
**Question:** Given B = (\μ_0 I / 2π r) \hat{e}_θ for a wire. Explain physically what ∇ × B evaluates to for r \≠q 0.
...
<table class="w-full text-left border-collapse mt-2">
  ...
  <tbody class="divide-y divide-slate-200">
    <tr>
      <td class="py-2 pr-4 text-slate-700">Ampère's law says ∇ × B = \μ_0 J. The current J only exists inside the wire at the origin.</td>
      <td class="py-2 pr-4 font-mono bg-slate-100 p-1 rounded">By Ampère's Law, ∇ × B = \μ_0 J.</td>
    </tr>
    <tr>
      <td class="py-2 pr-4 text-slate-700">If I am at r \≠q 0, the current density J is zero (in empty space). So curl must be zero.</td>
      <td class="py-2 pr-4 font-mono bg-slate-100 p-1 rounded">For r \≠q 0, J = 0.<br>Therefore, ∇ × B = 0.</td>
    </tr>
  </tbody>
</table>
</div>
...
**Question:** If ∇ × F = 2k at a point, what does this mean physically for a paddle wheel?
...
<table class="w-full text-left border-collapse mt-2">
  ...
  <tbody class="divide-y divide-slate-200">
    ...
    <tr>
      <td class="py-2 pr-4 text-slate-700">The curl is twice the angular velocity \ω. So angular velocity is 1.</td>
...
<table class="w-full text-left border-collapse mt-2">
  ...
  <tbody class="divide-y divide-slate-200">
    <tr>
      <td class="py-2 pr-4 text-slate-700">Surface side: From Ex. 1, ∇ × F = -2k. The surface integral of the constant -2 over area π R² is -2π R².</td>
      <td class="py-2 pr-4 font-mono bg-slate-100 p-1 rounded">\iint (∇ × F) • \hat{n} \\, dS = -2k • k (π R²) = -2π R².</td>
    </tr>
    <tr>
      ...
      <td class="py-2 pr-4 font-mono bg-slate-100 p-1 rounded">\oint F • ds = -2π R². Confirmed.</td>
    </tr>
  </tbody>
</table>
</div>
...
**Question:** If ∇ × F = 0, what powerful statement can we make about the line integral of F between points A and B?
...
<table class="w-full text-left border-collapse mt-2">
  ...
  <tbody class="divide-y divide-slate-200">
    <tr>
      ...
      <td class="py-2 pr-4 font-mono bg-slate-100 p-1 rounded">The field is irrotational/conservative. Therefore F = ∇ \varphi.</td>
    </tr>
    <tr>
      <td class="py-2 pr-4 text-slate-700">Therefore, walking from A to B requires the same work regardless of path.</td>
      <td class="py-2 pr-4 font-mono bg-slate-100 p-1 rounded">The line integral ∫_A^B F • ds is universally path-independent.</td>
    </tr>
  </tbody>
</table>
</div>
...
**Question:** For F = i(-y) + j(x), is this a vortex? Compute ∇ × F.
...
<table class="w-full text-left border-collapse mt-2">
  ...
  <tbody class="divide-y divide-slate-200">
    <tr>
      <td class="py-2 pr-4 text-slate-700">F_x = -y, F_y = x. Take the k-component scalar ∂(F_y)/∂ x - ∂(F_x)/∂ y.</td>
      <td class="py-2 pr-4 font-mono bg-slate-100 p-1 rounded">∇ × F = k((∂)/(∂ x)(x) - (∂)/(∂ y)(-y))</td>
    </tr>
    <tr>
...
**Question:** Given ∇ × B = \μ_0 J, and B = (x² z) j, calculate the current density J.
...
<table class="w-full text-left border-collapse mt-2">
  ...
  <tbody class="divide-y divide-slate-200">
    <tr>
      <td class="py-2 pr-4 text-slate-700">Compute curl B. B_x=0, B_y=x² z, B_z=0.</td>
      <td class="py-2 pr-4 font-mono bg-slate-100 p-1 rounded">∇ × B = | i\\; j\\; k |<br>| ∂_x\\; ∂_y\\; ∂_z |<br>| 0\\; x²z\\; 0 |</td>
    </tr>
    <tr>
      <td class="py-2 pr-4 text-slate-700">i: -∂(x² z)/∂ z = -x².<br>j: 0.<br>k: ∂(x² z)/∂ x = 2xz.<br>Then J = (1/\μ_0) ∇ × B.</td>
      <td class="py-2 pr-4 font-mono bg-slate-100 p-1 rounded">∇ × B = -x² i + 2xz k<br>J = (1)/(\μ_0) (-x² i + 2xz k)</td>
    </tr>
  </tbody>
</table>
</div>
...
**Question:** Given Faraday's Law ∇ × E = - (∂ B)/(∂ t), if B = t² j, what relates to the electric field?
...
<table class="w-full text-left border-collapse mt-2">
  ...
  <tbody class="divide-y divide-slate-200">
    <tr>
      <td class="py-2 pr-4 text-slate-700">Take the time derivative of B. ∂ B / ∂ t = 2t j.</td>
      <td class="py-2 pr-4 font-mono bg-slate-100 p-1 rounded">- (∂ B)/(∂ t) = -2t j</td>
    </tr>
    <tr>
      <td class="py-2 pr-4 text-slate-700">This must equal the curl of the electric field.</td>
      <td class="py-2 pr-4 font-mono bg-slate-100 p-1 rounded">∇ × E = -2t j.<br>A circulating electric field is induced.</td>
    </tr>
  </tbody>
</table>
</div>
...
**Question:** What is the divergence of the curl of any valid vector field? (Calculate ∇ • (∇ × F)).
...
<table class="w-full text-left border-collapse mt-2">
  ...
  <tbody class="divide-y divide-slate-200">
    ...
    <tr>
      ...
      <td class="py-2 pr-4 font-mono bg-slate-100 p-1 rounded">∇ • (∇ × F) = 0 over all space.</td>
    </tr>
  </tbody>
</table>
</div>
...
<table class="w-full text-left border-collapse">
  ...
  <tbody class="divide-y divide-slate-200">
    <tr>
      <td class="py-3 pr-4 text-slate-800">Calculate Curl (∇ × F)</td>
      <td class="py-3 pr-4 text-slate-800">Do I remember the determinant?</td>
      <td class="py-3 pr-4 text-slate-800">Draw 3x3 matrix: i, j, k top row. ∂_x, ∂_y, ∂_z middle. F_x, F_y, F_z bottom.</td>
    </tr>
    <tr>
      <td class="py-3 pr-4 text-slate-800">Stokes' Theorem</td>
      <td class="py-3 pr-4 text-slate-800">Is line integral hard?</td>
      <td class="py-3 pr-4 text-slate-800">Convert \oint F • ds to \iint (∇ × F) • dS.</td>
    </tr>
    <tr>
      <td class="py-3 pr-4 text-slate-800">∇ × F = 0</td>
      <td class="py-3 pr-4 text-slate-800">What does irrotational imply?</td>
      <td class="py-3 pr-4 text-slate-800">F is conservative. Path independent. F = ∇ \varphi.</td>
    </tr>
    <tr>
      <td class="py-3 pr-4 text-slate-800">Ampère's Law</td>
      <td class="py-3 pr-4 text-slate-800">How do forces relate to curl?</td>
      <td class="py-3 pr-4 text-slate-800">∇ × B = \μ_0 J.</td>
    </tr>
  </tbody>
</table>
...
*   **Trap 1: The J-component negative sign.** When expanding the determinant, the j-component must be subtracted: -j ( (∂ F_z)/(∂ x) - (∂ F_x)/(∂ z) ). Alternatively, swap the internal subtraction: +j ( (∂ F_x)/(∂ z) - (∂ F_z)/(∂ x) ). Failure here ruins the whole vector.
...
*   **Trap 3: Divergence of Curl.** If asked for ∇ • (∇ × F), don't waste 10 minutes calculating it. The answer is always exactly zero by identity.
...
*   **Gradient (∇ f):** A vector operator that finds the direction of greatest increase for a scalar function f.
...
*   **Normal Vector:** A perpendicular vector. The gradient ∇ f is perfectly geometrically normal (perpendicular) to the level surface.

## Why This Matters in Electrical Engineering

1.  **Electric Field and Potential:** The fundamental bridge in electrostatics is E = -∇ V (Electric field is the negative gradient of Voltage). The E-field vectors naturally point "downhill" from high voltage to low voltage.
2.  **Current Flow:** Heat and electric current naturally flow down the gradient. J = -\σ ∇ V (Ohm's Law in microscopic form).
...
From the dot product definition A • B = |A||B| cos  θ, the maximum value of D_u f occurs when cos  θ = 1, meaning \hat{u} points exactly in the same direction as ∇ f. Therefore:
*   Maximum rate of change = |∇ f|
*   Direction of maximum rate = (∇ f)/(|∇ f|)
...
2.  **Take ∂ / ∂ x:** Derive the mathematical function with respect to x, treating y and z as physical constants. Attach an i vector.
3.  **Take ∂ / ∂ y:** Derive with respect to y, treating x and z as constants. Attach a j vector.
4.  **Take ∂ / ∂ z:** Derive with respect to z. Attach a k vector.
5.  **Build Vector:** Sum them up: ∇ f = i(f_x) + j(f_y) + k(f_z).
...
1.  **Need the Tangent Plane / Normal Vector?**
    ...
    *   Calculate ∇ f. 
    *   Evaluate ∇ f at the specific point (x_0, y_0, z_0). This gives the perfect geometrical normal vector n.
2.  **Need the Directional Derivative?**
    *   Calculate ∇ f. Evaluate it at the point.
    *   Find the direction vector v. Ensure it is a UNIT vector by dividing by magnitude: \hat{u} = v / |v|.
    *   Dot product: ∇ f • \hat{u}.
3.  **Need the maximum increase?**
    *   Calculate ∇ f.
    *   Magnitude |∇ f| is the max rate.
    *   Direction is ∇ f exactly.

## Concept Recall: Trigger Words → Action

*   *"Find the direction of steepest ascent..."* → Calculate ∇ f. Do not dot it. It is the vector.
*   *"Find the rate of steepest ascent..."* → Calculate magnitude |∇ f|.
*   *"Find the directional derivative in direction v..."* → Normalize v to \hat{u}. Then compute ∇ f • \hat{u}.
*   *"Find the normal to the level surface..."* → Calculate ∇ f and evaluate it at the point.
...
**Question:** Find ∇ f for f(x,y,z) = x² + y² + z².
...
<table class="w-full text-left border-collapse mt-2">
  ...
  <tbody class="divide-y divide-slate-200">
    <tr>
      <td class="py-2 pr-4 text-slate-700">Take partial derivatives: ∂ f/∂ x = 2x, ∂ f/∂ y = 2y, ∂ f/∂ z = 2z.</td>
    ...
    <tr>
      ...
      <td class="py-2 pr-4 font-mono bg-slate-100 p-1 rounded">∇ f = 2x i + 2y j + 2z k</td>
    </tr>
  </tbody>
</table>
</div>
...
**Question:** Find df/ds for f(x,y) = x² + y² in direction \hat{u} = (i+j)/\√(2) at (1,1).
...
<table class="w-full text-left border-collapse mt-2">
  ...
  <tbody class="divide-y divide-slate-200">
    <tr>
      ...
      <td class="py-2 pr-4 font-mono bg-slate-100 p-1 rounded">∇ f = 2x i + 2y j<br>∇ f(1,1) = 2 i + 2 j</td>
    </tr>
    <tr>
      <td class="py-2 pr-4 text-slate-700">Dot the gradient with the unit vector \hat{u}. It's correctly normalized already.</td>
      <td class="py-2 pr-4 font-mono bg-slate-100 p-1 rounded">df/ds = (2i+2j) • (i+j)/\√(2)<br>= (2+2)/\√(2) = 4/\√(2) = 2\√(2)</td>
    </tr>
  </tbody>
</table>
</div>
...
**Question:** In what direction does f(x,y) = x² + y² increase fastest at (1,1)? What's the rate?
...
<table class="w-full text-left border-collapse mt-2">
  ...
  <tbody class="divide-y divide-slate-200">
    <tr>
      ...
      <td class="py-2 pr-4 font-mono bg-slate-100 p-1 rounded">∇ f(1,1) = 2i + 2j</td>
    </tr>
    <tr>
      <td class="py-2 pr-4 text-slate-700">Direction is the vector. Rate is magnitude.</td>
      <td class="py-2 pr-4 font-mono bg-slate-100 p-1 rounded">Direction = i + j<br>Max Rate = |∇ f| = \√(2² + 2²) = \√(8) = 2\√(2)</td>
    </tr>
  </tbody>
</table>
</div>
...
**Question:** Find the normal vector to the sphere f(x,y,z) = x² + y² + z² = 3 at point (1,1,1).
...
<table class="w-full text-left border-collapse mt-2">
  ...
  <tbody class="divide-y divide-slate-200">
    <tr>
      <td class="py-2 pr-4 text-slate-700">By geometric rule, ∇ f is perfectly perpendicular to the exterior shape of level surfaces f = C.</td>
      <td class="py-2 pr-4 font-mono bg-slate-100 p-1 rounded">∇ f = 2x i + 2y j + 2z k</td>
    </tr>
    <tr>
      <td class="py-2 pr-4 text-slate-700">Evaluate gradient at (1,1,1).</td>
      <td class="py-2 pr-4 font-mono bg-slate-100 p-1 rounded">Normal N = ∇ f(1,1,1) = 2i + 2j + 2k</td>
    </tr>
  </tbody>
</table>
</div>
...
**Question:** If ∇ f = 3i + 4j and you want the derivative in the direction of v = 3i - 4j, find df/ds.
...
<table class="w-full text-left border-collapse mt-2">
  ...
  <tbody class="divide-y divide-slate-200">
    <tr>
      ...
      <td class="py-2 pr-4 font-mono bg-slate-100 p-1 rounded">\hat{u} = (3i - 4j)/(\√(3² + (-4)²)) = (3i - 4j)/(5)</td>
    </tr>
    <tr>
      <td class="py-2 pr-4 text-slate-700">Now compute dot product ∇ f • \hat{u}.</td>
      <td class="py-2 pr-4 font-mono bg-slate-100 p-1 rounded">df/ds = (3i + 4j) • ((3i - 4j)/(5)) = (9 - 16)/(5) = -(7)/(5)</td>
    </tr>
  </tbody>
</table>
</div>
...
**Question:** In what direction is the directional derivative precisely 0 if ∇ f = 3i + 4j?
...
<table class="w-full text-left border-collapse mt-2">
  ...
  <tbody class="divide-y divide-slate-200">
    <tr>
      <td class="py-2 pr-4 text-slate-700">A directional derivative of 0 means the dot product ∇ f • \hat{u} = 0. This means orthogonal direction.</td>
      <td class="py-2 pr-4 font-mono bg-slate-100 p-1 rounded">We need \hat{u} such that (3i + 4j) • \hat{u} = 0.</td>
    </tr>
    <tr>
      ...
      <td class="py-2 pr-4 font-mono bg-slate-100 p-1 rounded">Orthogonal vector v = 4i - 3j.<br>Normalize: \hat{u} = (4i - 3j)/(5) or (-4i + 3j)/(5).</td>
    </tr>
  </tbody>
</table>
</div>
...
**Question:** If absolute potential is \Phi(x,y,z) = (q)/(4π\varepsilon_0 r), calculate electric field E = -∇ \Phi.
...
<table class="w-full text-left border-collapse mt-2">
  ...
  <tbody class="divide-y divide-slate-200">
    <tr>
      <td class="py-2 pr-4 text-slate-700">Let r = \√(x²+y²+z²). ∂\Phi / ∂ x demands chain rule.</td>
      <td class="py-2 pr-4 font-mono bg-slate-100 p-1 rounded">\Phi = k (x²+y²+z²)^-1/2<br>∂\Phi/∂ x = k(-(1)/(2))(x²+y²+z²)^-3/2(2x) = -(kx)/(r³)</td>
    </tr>
    <tr>
      <td class="py-2 pr-4 text-slate-700">Assemble the gradient and negate it.</td>
      <td class="py-2 pr-4 font-mono bg-slate-100 p-1 rounded">E = -∇ \Phi = - ( -(k)/(r³) (xi + yj + zk) )<br>E = (q)/(4π\varepsilon_0) rrr</td>
    </tr>
  </tbody>
</table>
</div>
...
**Question:** Equation of the tangent plane to the surface x² + y² + z² = 9 at (2, 2, 1).
...
<table class="w-full text-left border-collapse mt-2">
  ...
  <tbody class="divide-y divide-slate-200">
    <tr>
      <td class="py-2 pr-4 text-slate-700">Tangent planes require a normal vector. ∇ f provides exactly that.</td>
      <td class="py-2 pr-4 font-mono bg-slate-100 p-1 rounded">∇ f = 2xi + 2yj + 2zk<br>∇ f(2,2,1) = 4i + 4j + 2k</td>
    </tr>
    <tr>
...
**Question:** Find ∇ f for f(r, θ, z) = r² sin  θ.
...
<table class="w-full text-left border-collapse mt-2">
  ...
  <tbody class="divide-y divide-slate-200">
    <tr>
      <td class="py-2 pr-4 text-slate-700">Cylindrical gradient formula is \hat{e}_r (∂ f)/(∂ r) + \hat{e}_θ (1)/(r)(∂ f)/(∂ θ) + \hat{e}_z (∂ f)/(∂ z).</td>
      <td class="py-2 pr-4 font-mono bg-slate-100 p-1 rounded">∇ f = \hat{e}_r (2r sin  θ) + \hat{e}_θ (1)/(r) (r² cos  θ) + \hat{e}_z (0)</td>
    </tr>
    <tr>
      <td class="py-2 pr-4 text-slate-700">Simplify the 1/r term.</td>
      <td class="py-2 pr-4 font-mono bg-slate-100 p-1 rounded">∇ f = 2r sin  θ \hat{e}_r + r cos  θ \hat{e}_θ</td>
    </tr>
  </tbody>
</table>
</div>
...
**Question:** Find ∇ f for f(r, θ, φ) = (1)/(r).
...
<table class="w-full text-left border-collapse mt-2">
  ...
  <tbody class="divide-y divide-slate-200">
    <tr>
      <td class="py-2 pr-4 text-slate-700">Spherical gradient formula. Function only depends on r, so ∂/∂ θ and ∂/∂ φ are 0.</td>
      <td class="py-2 pr-4 font-mono bg-slate-100 p-1 rounded">∇ f = \hat{e}_r (∂)/(∂ r)(r^-1)</td>
    </tr>
    <tr>
      <td class="py-2 pr-4 text-slate-700">Take standard power rule derivative.</td>
      <td class="py-2 pr-4 font-mono bg-slate-100 p-1 rounded">∇ f = -(1)/(r²) \hat{e}_r</td>
    </tr>
  </tbody>
</table>
</div>
...
<table class="w-full text-left border-collapse">
  ...
  <tbody class="divide-y divide-slate-200">
    <tr>
      ...
      <td class="py-3 pr-4 text-slate-800">Compute ∇ f, divide v by |v|, take dot product.</td>
    </tr>
    <tr>
      ...
      <td class="py-3 pr-4 text-slate-800">No, the answer is exactly the gradient ∇ f.</td>
    </tr>
    <tr>
      ...
      <td class="py-3 pr-4 text-slate-800">Normal n is accurately computed as ∇ f at that point.</td>
    </tr>
    <tr>
      ...
      <td class="py-3 pr-4 text-slate-800">Use E = -∇ V.</td>
    </tr>
  </tbody>
</table>
...
*   **Trap 1: Forgetting to Normalize the Direction Vector.** When calculating directional derivative D_u f = ∇ f • \hat{u}, the vector u *MUST* have a magnitude of exactly 1. If it doesn't, you calculate a scaled, incorrect value. This is the #1 easiest mistake in vector calculus.
*   **Trap 2: Ignoring the negative sign in Electric Field.** Voltage is a potential. Positive charges want to fall DOWN the gradient (from high potential to low potential). Therefore, E = -∇ V. If you drop the minus sign, your entire physics model propagates in reverse.
...
*   If the function is **f(x,y) = (x², y)**, you stretch the sheet unevenly. Squares further to the right get stretched massively into rectangles, while squares near zero stay small.
...
*   **Jacobian Determinant (det(J) or (∂(u,v))/(∂(x,y))):** A scalar value representing the local "area scaling factor". If it is 2, a microscopic square of area dA maps to an area of 2\\,dA.
...
A mapping from \mathbb{R}² \→ \mathbb{R}² is written as:
T(x,y) = (u(x,y), v(x,y))
...
∬ f(x,y) dx dy = ∬ f(x(u,v), y(u,v)) ≤ft| (∂(x,y))/(∂(u,v)) | du dv
...
2.  **Evaluate Corners:** Find where the vertices map. (e.g. (0,0) \→ F(0,0)). Plot these in the (u,v) plane.
...
5.  **Interpret Area:** If det(J) > 1, the region is universally expanding. If det(J) < 1, it is compressing. If det(J) < 0, the rubber sheet was flipped entirely upside down (orientation reversed).
...
*   *"Find the Jacobian..."* → Construct the 2 × 2 (or 3 × 3) matrix of partial derivatives. Cross multiply and subtract.
*   *"What is the area element dA in polar?"* → Calculate Jacobian of x = r cos θ, y = r sin θ. Determinant is r. So dA = r \\, dr \\, dθ.
...
**Question:** Map the unit square (x,y \∈ [0,1]) under F(x,y) = (2x, y). Describe the result.
...
<table class="w-full text-left border-collapse mt-2">
  ...
  <tbody class="divide-y divide-slate-200">
    <tr>
      ...
      <td class="py-2 pr-4 font-mono bg-slate-100 p-1 rounded">u = 2x \implies u \∈ [0,2]<br>v = y \implies v \∈ [0,1]</td>
    </tr>
    <tr>
...
<table class="w-full text-left border-collapse mt-2">
  ...
  <tbody class="divide-y divide-slate-200">
    ...
    <tr>
      ...
      <td class="py-2 pr-4 font-mono bg-slate-100 p-1 rounded">det(J) = 6. Area scales by factor 6.</td>
    </tr>
  </tbody>
</table>
</div>
...
**Question:** Compute Jacobian for F(x,y) = (x², y). Describe area scaling at (3,1).
...
<table class="w-full text-left border-collapse mt-2">
  ...
  <tbody class="divide-y divide-slate-200">
    <tr>
      <td class="py-2 pr-4 text-slate-700">Derivatives of u=x² are 2x, 0. Derivatives of v=y are 0, 1.</td>
      <td class="py-2 pr-4 font-mono bg-slate-100 p-1 rounded">det(J) = \begin{vmatrix} 2x & 0 \\\\ 0 & 1 \end{vmatrix} = 2x</td>
    </tr>
    <tr>
...
**Question:** Define the standard mapping x = r cos θ, y = r sin θ. Calculate its Jacobian.
...
<table class="w-full text-left border-collapse mt-2">
  ...
  <tbody class="divide-y divide-slate-200">
    <tr>
      <td class="py-2 pr-4 text-slate-700">Create the matrix with derivatives with respect to r and θ.</td>
      <td class="py-2 pr-4 font-mono bg-slate-100 p-1 rounded">J = \begin{vmatrix} cos θ & -rsin θ \\\\ sin θ & rcos θ \end{vmatrix}</td>
    </tr>
    <tr>
      <td class="py-2 pr-4 text-slate-700">Cross multiply: (cos θ)(rcos θ) - (sin θ)(-rsin θ) = rcos ² + rsin ² = r.</td>
      <td class="py-2 pr-4 font-mono bg-slate-100 p-1 rounded">det(J) = r(cos ²θ + sin ²θ) = r.<br>Area element dA = r \\, dr \\, dθ.</td>
    </tr>
  </tbody>
</table>
</div>
...
**Question:** Compute the linear approximation of f(x,y) = x² + y² at (1,1).
...
<table class="w-full text-left border-collapse mt-2">
  ...
  <tbody class="divide-y divide-slate-200">
    <tr>
      <td class="py-2 pr-4 text-slate-700">Unit circle is x² + y² = 1. The transformation gives u=2x and v=3y.</td>
    ...
    <tr>
      ...
      <td class="py-2 pr-4 font-mono bg-slate-100 p-1 rounded">(u/2)² + (v/3)² = 1 \implies (u²)/(4) + (v²)/(9) = 1<br>The circle becomes an ellipse.</td>
    </tr>
  </tbody>
</table>
</div>
...
<table class="w-full text-left border-collapse mt-2">
  ...
  <tbody class="divide-y divide-slate-200">
    ...
    <tr>
      ...
      <td class="py-2 pr-4 font-mono bg-slate-100 p-1 rounded">J_det = (1)(-1) - (0)(0) = -1.<br>The area is preserved, but orientation flips.</td>
    </tr>
  </tbody>
</table>
</div>
...
**Question:** Map the first quadrant unit square x,y \∈ [0,1] under the non-linear polar-like map F(x,y) = (x² - y², 2xy). Find the map of the line x=1.
...
<table class="w-full text-left border-collapse mt-2">
  ...
  <tbody class="divide-y divide-slate-200">
    <tr>
      <td class="py-2 pr-4 text-slate-700">Let x=1. Then u = 1 - y² and v = 2y. I need to eliminate parameter y.</td>
      <td class="py-2 pr-4 font-mono bg-slate-100 p-1 rounded">On x=1:<br>u = 1 - y²<br>v = 2y \implies y = v/2</td>
    </tr>
    <tr>
      <td class="py-2 pr-4 text-slate-700">Substitute y=v/2 into u.</td>
      <td class="py-2 pr-4 font-mono bg-slate-100 p-1 rounded">u = 1 - (v/2)² = 1 - v²/4.<br>This boundary maps to a downward facing parabola.</td>
    </tr>
  </tbody>
</table>
</div>
...
**Question:** Compute Jacobian for spherical transformation x = rsin φcos θ, y = rsin φsin θ, z = rcos φ.
...
<table class="w-full text-left border-collapse mt-2">
  ...
  <tbody class="divide-y divide-slate-200">
    <tr>
      <td class="py-2 pr-4 text-slate-700">This requires building a 3 × 3 determinant matrix and expanding. It's a huge operation so I will summarize the canonical geometric output.</td>
      <td class="py-2 pr-4 font-mono bg-slate-100 p-1 rounded">det \begin{vmatrix} x_r & x_θ & x_φ \\\\ \dots & \dots & \dots \end{vmatrix} evaluates to r² sin φ.</td>
    </tr>
    <tr>
      <td class="py-2 pr-4 text-slate-700">This is why the volumetric integral chunk dV in spherical is r² sin φ \\, dr \\, dφ \\, dθ.</td>
      <td class="py-2 pr-4 font-mono bg-slate-100 p-1 rounded">Volume scaling factor is r² sin φ.</td>
    </tr>
  </tbody>
</table>
</div>
...
<table class="w-full text-left border-collapse">
  ...
  <tbody class="divide-y divide-slate-200">
    <tr>
      ...
      <td class="py-3 pr-4 text-slate-800">Yes, dA = |det(J)| \\, du \\, dv. Compute u_x v_y - u_y v_x.</td>
    </tr>
    <tr>
...
*   **Trap 2: Creating Tangent Planes but leaving variables squared.** A linear approximation (tangent plane) is LINEAR. Your final equation L(x,y) must solely contain x and y variables. If it contains x² or sin (y), you failed to evaluate the partial derivatives at the target constant point (a,b).
*   **Trap 3: Cartesian / Polar variable mixing.** If you map exactly to (r, θ), your Jacobian must only contain r and θ, no x's or y's surviving.
...
If \mathbf{E} = -∇ \Phi, then by the Fundamental Theorem for Line Integrals:
∫_A^B E} • ds} = -[(B) - (A)]
Because the result only depends on the start and end heights \Phi, the path between them doesn't matter! 
Furthermore, if \mathbf{F} = ∇ \Phi, then ∇ × \mathbf{F} = 0. A conservative vector field is fundamentally irrotational (zero curl).
  `
},
  {
    id: "chapter-14-1",
...
A real-valued function f on a set D of n-tuples of real numbers (x_1, x_2, ..., x_n) is a rule that assigns a unique real number 
w = f(x_1, x_2, ..., x_n)
...
**Question:** Find the domain of f(x,y) = √(y - x²).
...
2. Draw the parabola y = x² as a solid line (solid because the inequality is ≥).
3. Test a point, say (0,1). 1 ≥ 0². It works. Shade the region "inside" or "above" the parabola.
This region includes its boundary (the parabola) so it is closed. It stretches infinitely upwards, so it is unbounded.

**Solution:**
Domain: y ≥ x². This is the closed, unbounded region bounded below by the parabola y=x².

### Example 2: Graphing and Level Curves
**Question:** Graph f(x,y) = 100 - x² - y², plot level curves for f=0, 51, 75.

**Thought Process:** 
The function z = 100 - (x² + y²) represents a downward-opening paraboloid with its peak at (0,0,100).
For level curves f(x,y) = c, substitute c for f(x,y):
- c=0: 100 - x² - y² = 0 Rightarrow x² + y² = 100 (circle radius 10)
- c=51: 100 - x² - y² = 51 Rightarrow x² + y² = 49 (circle radius 7)
- c=75: 100 - x² - y² = 75 Rightarrow x² + y² = 25 (circle radius 5)
...
**Question:** Describe the level surfaces of f(x,y,z) = √(x²+y²+z²).

**Thought Process:**
A level surface is where f(x,y,z) = c. So √(x²+y²+z²) = c. Squaring both sides: x²+y²+z² = c².
This requires c ≥ 0. This is the equation of a sphere of radius c centered at the origin.
...
**Question:** Find the domain of f(x,y) = sin ^-1(x² + y² - 2).

| Inner voice | What you write |
| :--- | :--- |
| "Inverse sine is incredibly picky. It only accepts inputs exactly between -1 and 1. I will trap the inner argument and solve the inequality." | -1 ≤ x² + y² - 2 ≤ 1 |
| "Add 2 to all sides to isolate the x² + y² circle equation. This forms an annulus (a ring)." | 1 ≤ x² + y² ≤ 3<br>**Domain:** Annulus between circles 1 and √(3). |
...
| If you see dots | Ask yourself dots | Action |
| :--- | :--- | :--- |
| √(dots) | Inside ≥ 0? | Write inequality |
...
lim _(x,y) → (x_0, y_0) f(x,y) = L
if, for every number epsilon > 0, there exists a corresponding number δ > 0 such that for all (x,y) in the domain of f,
|f(x,y) - L| < epsilon quad whenever quad 0 < √((x-x_0)² + (y-y_0)²) < δ
...
If a function f(x,y) has different limits along two different paths as (x,y) approaches (x_0,y_0), then lim _(x,y) → (x_0,y_0) f(x,y) **does not exist**.

### 4. Continuity
A function f(x,y) is continuous at (x_0,y_0) if:
1. f(x_0,y_0) is defined.
2. lim _(x,y) → (x_0,y_0) f(x,y) exists.
3. lim _(x,y) → (x_0,y_0) f(x,y) = f(x_0,y_0).
...
**Question:** Find lim _(x,y) → (0,0) (x² - xy)/(√(x) - √{y)}.

**Thought Process:**
Direct substitution gives 0/0. We must algebraically manipulate. We can multiply up and down by the conjugate √(x) + √(y), or factor the numerator.
Let's factor: x²-xy = x(x-y). And we know x-y = (√(x) - √(y))(√(x) + √(y)).

*Sketching Guide for student:* There isn't much to sketch here, but visualizing the domain is good: x ≥ 0, y ≥ 0, x ≠q y. The function lives only in the first quadrant, away from the line y=x.

**Solution:**
begin{align*} lim  (x(x-y))/(√(x) - √{y)} &= lim  frac{x(√(x)-√(y))(√(x)+√(y))}{√(x)-√(y)} \\ &= lim  x(√(x) + √(y)) \\ &= 0(√(0) + √(0)) = 0 end{align*}

### Example 2: The Two-Path Test
**Question:** Show that lim _(x,y)→(0,0) (2x²y)/(x^4+y²) does not exist.

**Thought Process:**
Let's check straight lines y = mx.
f(x, mx) = (2x²(mx))/(x^4+m²x²) = (2mx³)/(x²(x²+m²)) = (2mx)/(x²+m²).
As x → 0, this goes to 0. So along all lines, the limit is 0.
Wait, does that mean the limit is 0? No! Let's check a parabola path matching the degrees: y = kx².

*Sketching Guide for student:* Draw the axes. Draw straight lines radiating from the origin. The function looks flat (value 0) if you approach along these lines. But now draw a parabola y=x² curling into the origin. Along this parabola crest, the function rides high at a non-zero value!

**Solution:**
Check along y=kx²:
begin{align*} f(x, kx²) &= (2x²(kx²))/(x^4 + (kx²)²) \\ &= (2kx^4)/(x^4 + k²x^4) \\ &= (2k)/(1+k²) end{align*}
If we approach along y=x² (k=1), the limit is (2)/(2) = 1.
...
**Question:** Evaluate lim _(x,y)→(0,0) (x³)/(x²+y²).

**Thought Process:**
The denominator x²+y² strongly suggests polar coordinates.
x = r cos  θ, y = r sin  θ. x²+y² = r². As (x,y) → (0,0), r → 0 independently of θ.
...
**Solution:**
begin{align*} lim _r → 0 (r³ cos ³ θ)/(r²) &= lim _r → 0 r cos ³ θ end{align*}
Since |cos ³ θ| ≤ 1, the expression is bounded by -r ≤ rcos ³θ ≤ r. By the Squeeze Theorem, as r → 0, the limit is 0, regardless of θ.
The limit is 0.
...
**Question:** Show lim _(x,y) → (0,0) (xy)/(x² + y²) does not exist.

| Inner voice | What you write |
| :--- | :--- |
| "I will approach along the path y = kx. If the limit answer depends on my chosen slope k, the limit cannot exist." | lim _x → 0 (x(kx))/(x² + (kx)²) |
| "Factor out x² and simplify." | lim _x → 0 (k)/(1+k²) = (k)/(1+k²).<br>Depends on k. Does not exist. |
...
| x²+y² in denom | Can I use polar (r → 0)? | Substitute x=rcos θ, y=rsin θ. |
...
(∂ f)/(∂ x) = lim _h → 0 (f(x_0+h, y_0) - f(x_0, y_0))/(h)
Notations: (∂ f)/(∂ x), f_x, (∂ z)/(∂ x), z_x.

**Geometric interpretation:** The graph z = f(x,y) is intersected by the plane y=y_0, forming a curve. The partial derivative ∂ f/∂ x evaluated at (x_0,y_0) is the slope of the tangent line to this curve at that point.

### 2. Calculations
To calculate ∂ f/∂ x, treat y as a constant and differentiate with respect to x. 
To calculate ∂ f/∂ y, treat x as a constant and differentiate with respect to y.

### 3. Second-Order Partial Derivatives
Differentiate twice to get second-order derivatives:
- f_xx = (∂² f)/(∂ x²)
- f_yy = (∂² f)/(∂ y²)
- f_xy = (∂² f)/(∂ y ∂ x) (differentiate first wrt x, then y)
- f_yx = (∂² f)/(∂ x ∂ y) 

**Theorem 2: The Mixed Derivative Theorem (Clairaut's Theorem)**
If f(x,y) and its partial derivatives f_x, f_y, f_xy, f_yx are defined throughout an open region containing (a,b) and are all continuous at (a,b), then:
f_xy(a,b) = f_yx(a,b)
...
f(x,y) is differentiable if Δ z can be expressed as:
Δ z = f_x Δ x + f_y Δ y + epsilon_1 Δ x + epsilon_2 Δ y
where epsilon_1, epsilon_2 → 0 as Δ x, Δ y → 0.
...
**Question:** If f(x,y) = y sin (xy), find ∂ f / ∂ y.
...
**Solution:**
begin{align*} (∂ f)/(∂ y) &= (∂)/(∂ y)(y sin (xy)) &&Step 1: Set up product rule. \\ &= y • (∂)/(∂ y)(sin (xy)) + sin (xy) • (∂)/(∂ y)(y) &&Step 2: Apply product rule. \\ &= y • (cos (xy) • x) + sin (xy) • 1 &&Step 3: Chain rule on inner function  xy  treating x as constant. \\ &= xy cos (xy) + sin (xy) &&Step 4: Cleanup. end{align*}

### Example 2: Implicit Partial Differentiation
**Question:** Find ∂ z/∂ x if yz - ln  z = x + y.

**Thought Process:**
Assume z is a function of x and y. We need to differentiate both sides with respect to x, holding y constant.

**Solution:**
begin{align*} (∂)/(∂ x)(yz - ln  z) &= (∂)/(∂ x)(x + y) &&Step 1: Differentiate both sides wrt x. \\ y (∂ z)/(∂ x) - (1)/(z) (∂ z)/(∂ x) &= 1 + 0 &&Step 2: Normal diff rules, tacking on  ∂ z/∂ x  where z is modified. \\ (∂ z)/(∂ x) ≤ft(y - (1)/(z)right) &= 1 &&Step 3: Factor out derivative. \\ (∂ z)/(∂ x) &= (1)/(y - frac{1){z}} = (z)/(yz - 1) &&Step 4: Algebra to clear fractions. end{align*}

### Example 3: Verifying Clairaut's Theorem
**Question:** Verify f_xy = f_yx for f(x,y) = x cos  y + y e^x.

**Thought Process:**
We will calculate f_x, then f_xy. Then f_y, then f_yx, and compare.

**Solution:**
begin{align*} f_x &= cos  y + y e^x &&Step 1: Diff wrt x. \\ f_xy &= -sin  y + e^x &&Step 2: Diff  f_x  wrt y. end{align*}

begin{align*} f_y &= -x sin  y + e^x &&Step 3: Diff original wrt y. \\ f_yx &= -sin  y + e^x &&Step 4: Diff  f_y  wrt x. end{align*}
They match exactly.
...
**Question:** Find f_x and f_y for f(x,y) = x² y³ + sin (x).

| Inner voice | What you write |
| :--- | :--- |
| "To find f_x, I treat y entirely as a constant number. Just derive the x parts." | f_x = 2x(y³) + cos (x) |
| "To find f_y, I treat x as the constant. sin (x) is purely constant, so its derivative is 0." | f_y = x²(3y²) + 0 = 3x² y² |
...
| Mixed f_xy | Did I do x then y? | Differentiate f_x with respect to y. |
...
### Formal Definitions and Notation
*   **Chain Rule (1 Independent):** (dw)/(dt) = (∂ w)/(∂ x)(dx)/(dt) + (∂ w)/(∂ y)(dy)/(dt)
*   **Chain Rule (2 Independent):** (∂ w)/(∂ r) = (∂ w)/(∂ x)(∂ x)/(∂ r) + (∂ w)/(∂ y)(∂ y)/(∂ r)
*   **Implicit Formula:** (dy)/(dx) = - (F_x)/(F_y) (Works only if F(x,y) = 0).
...
*   **Step-by-Step Solution:**
    1.  Formula: (∂ u)/(∂ x) = (∂ u)/(∂ p)(∂ p)/(∂ x) + (∂ u)/(∂ q)(∂ q)/(∂ x) + (∂ u)/(∂ r)(∂ r)/(∂ x).
    2.  Find inner partials with respect to x: ∂p/∂x = 1, ∂q/∂x = 1, ∂r/∂x = 1.
    3.  This simplifies the formula to: (∂ u)/(∂ x) = (∂ u)/(∂ p) + (∂ u)/(∂ q) + (∂ u)/(∂ r).
    4.  Now find the partials of u with respect to p, q, r:
        *   ∂u/∂p = 1/(q-r)
        *   ∂u/∂q = (-(q-r)(1) - (p-q)(1))/((q-r)²) = (-q+r-p+q)/((q-r)²) = (r-p)/((q-r)²)
        *   ∂u/∂r = (-(p-q)(-1))/((q-r)²) = (p-q)/((q-r)²)
    5.  Sum them up: (∂ u)/(∂ x) = (1)/(q-r) + (r-p)/((q-r)²) + (p-q)/((q-r)²).
    6.  To simplify, find a common denominator: ((q-r) + (r-p) + (p-q))/((q-r)²) = (q-r+r-p+p-q)/((q-r)²) = (0)/((q-r)²) = 0.
    7.  Therefore, ∂u/∂x = 0.
...
**Question:** Let z = xy, where x=cos  t, y=sin  t. Find (dz)/(dt) without substituting first.

| Inner voice | What you write |
| :--- | :--- |
| "z depends on x,y, which depend on t. I draw the tree and write the chain sum." | (dz)/(dt) = (∂ z)/(∂ x)(dx)/(dt) + (∂ z)/(∂ y)(dy)/(dt) |
| "Compute partials and substitute the t derivatives." | (dz)/(dt) = (y)(-sin  t) + (x)(cos  t) |
...
| (∂ w)/(∂ u) | Did I trace all paths? | Sum product of derivatives down paths. |
...
>
> 4. If w = e^xy ln (z), x = t², y = sin  t, z = cos  t, find (dw)/(dt) at t = 0.
> *(Source: CUI Sahiwal FA25 Q4a)*

> **QUIZ:** Use implicit differentiation to find ∂z/∂y for x² y + y z² + z x = 0 at (1,1,-1).
>
> 5. Use implicit differentiation to find (∂ z)/(∂ y) for x² y + y z² + z x = 0 at (1,1,-1).
> *(Original)*
...
| Inner voice | What you write |
| :--- | :--- |
| "First, I need the gradient vector abla f at the point (1,0)." | abla f = ⟨ e^y, xe^y ⟩ rightarrow ⟨ 1, 1 ⟩ |
| "Second, I must normalize the direction vector v to a unit vector u." | u = (⟨ 3,4 ⟩)/(5) = ⟨ (3)/(5), (4)/(5) ⟩ |
| "Dot the gradient with the unit vector." | D_u f = ⟨ 1, 1 ⟩ • ⟨ (3)/(5), (4)/(5) ⟩ = (7)/(5) |
...
>
> 4. Find the direction(s) of zero change for f(x,y) = x² - 2xy + y² at (1,1).
...
>
> 5. The temperature in a room is T(x,y,z) = 100 - x² - y² - z². A bug at (1,2,2) wants to cool fastest. In which direction should it move? What is the rate of cooling?
> *(EE application)*
...
**Question:** Find tangent plane to surface F(x,y,z) = x²+y²+z² = 14 at (1,2,3).

| Inner voice | What you write |
| :--- | :--- |
| "For a level surface F(x,y,z)=c, the normal vector to the plane is precisely the gradient of F at the point." | abla F = ⟨ 2x, 2y, 2z ⟩ |
| "Evaluate abla F at (1,2,3)." | n = ⟨ 2, 4, 6 ⟩ |
| "Use the point-normal plane equation: n • (r - r_0) = 0." | 2(x-1) + 4(y-2) + 6(z-3) = 0 |
...
>
> 4. Find the linear approximation of f(x,y) = √(x² + y²) at (3,4) and use it to estimate √(3.1² + 3.9²).
...
>
> 5. The resistance of a wire is R = rho L / A. If rho = 1.68×10^-8 Ω•m, L = 10.0 m with error ±0.1 m, A = 2.0×10^-6 m² with error ±0.05×10^-6, estimate the maximum possible error in R.
> *(EE application)*
...
\`▓▓▓▓▓▓░░░░ 60%\` Course Progress`
  },
  {
    ...
    subheadings: [
      ...
      { id: "ch14-7-cheatsheet", title: "Ultimate Exam Room Cheat Sheet" },      { id: "ch14-7-visuals", title: "Visuals / Graphs" },
...
The expression D = f_xxf_yy - f_xy² is called the **discriminant** or **Hessian** of f. It is also written as a determinant:
D = begin(vmatrix) f_xx & f_xy  f_xy & f_yy end(vmatrix)

1. f has a **local maximum** at (a, b) if f_xx < 0 and D > 0.
2. f has a **local minimum** at (a, b) if f_xx > 0 and D > 0.
...
**Question:** Find the local extreme values of f(x, y) = x² + y².
...
*Sketching Guide for student:* To sketch z = x² + y², notice that z ≥ 0, and horizontal cross sections (z=c) are circles x²+y²=c. This forms a classic bowl-shaped paraboloid opening upwards, confirming the minimum at the bottom of the bowl.

**Solution:**
begin{align*} f_x &= 2x &&Step 1: Compute first partial derivatives to find critical points. \\ f_y &= 2y && \\ 2x &= 0 implies x=0 &&Step 2: Set partial derivatives to zero and solve.\\ 2y &= 0 implies y=0 && end{align*}

The only critical point is (0,0). The value of f(0,0) is 0.

begin{align*} f_xx &= 2, \, f_yy = 2, \, f_xy = 0 &&Step 3: Compute second derivatives to apply the Second Derivative Test. \\ D &= f_xxf_yy - f_xy² && \\ D &= (2)(2) - 0 = 4 > 0 &&Step 4: Compute the discriminant  D. end{align*}
Since D > 0 and f_xx > 0, the origin gives a **local minimum** of 0.

### Example 2: Identifying a Saddle Point
**Question:** Find the local extreme values (if any) of f(x, y) = y² - x².

**Thought Process:**
Set partials to 0. Find the critical point. Test it using D. I have to be careful with the signs.

*Sketching Guide for student:* Draw the x, y, and z axes. Notice that along the y-axis (x=0), the curve is a parabola opening up (z=y²). Along the x-axis (y=0), the curve is a parabola opening down (z=-x²). Combining these gives the classic Pringles chip shape stretching up along the y-axis and dropping down along the x-axis.

**Solution:**
begin{align*} f_x &= -2x &&Step 1: Find the first partial derivatives. \\ f_y &= 2y && \\ -2x &= 0 implies x=0 &&Step 2: Find critical points.\\ 2y &= 0 implies y=0 && The critical point is  (0,0). end{align*}

begin{align*} f_xx &= -2, \, f_yy = 2, \, f_xy = 0 &&Step 3: Evaluate the second derivatives. \\ D &= (-2)(2) - 0 = -4 < 0 &&Step 4: Compute the discriminant  D. end{align*}
...
**Question:** Find the local extreme values of f(x, y) = xy - x² - y² - 2x - 2y + 4.

**Thought Process:** 
I will solve the simultaneous system of equations formed by setting f_x=0 and f_y=0. 

**Solution:** 
begin{align*} f_x &= y - 2x - 2 = 0 &&Step 1: Setup system of equations from partials.\\ f_y &= x - 2y - 2 = 0 && \\ x &= (y-2)/(2) &&Step 2: Solve the first equation for  x. \\ (y-2)/(2) - 2y - 2 &= 0 &&Substitute into the second equation. \\ y - 2 - 4y - 4 &= 0 &&Multiply by 2 and simplify. \\ -3y &= 6 implies y = -2 &&Solve for  y.  Thus  x = -2. end{align*}
The critical point is (-2, -2).

begin{align*} f_xx &= -2, \, f_yy = -2, \, f_xy = 1 &&Step 3: Evaluate second partial derivatives. \\ D &= (-2)(-2) - (1)² = 3 > 0 &&Step 4: Compute the discriminant  D. end{align*}
Since D > 0 and f_xx < 0, f has a **local maximum** at (-2, -2). 
f(-2,-2) = 8.

### Example 4: Absolute Extrema on a Closed Bounded Region
**Question:** Find the absolute maximum and minimum values of f(x, y) = 2 + 2x + 2y - x² - y² on the triangular region R in the first quadrant bounded by the lines x=0, y=0, y=9-x.
...
**Solution:**
**(a) Interior points:**
begin{align*} f_x &= 2 - 2x = 0 implies x=1 &&Step 1: Check interior critical points. \\ f_y &= 2 - 2y = 0 implies y=1 && end{align*}
The point (1,1) is inside the triangle.
f(1,1) = 2 + 2 + 2 - 1 - 1 = 4.

**(b) Boundary points:**
begin{align*} Segment  OA: \, y&=0, \, 0 ≤ x ≤ 9. &&Step 2: Check the bottom edge. \\ f(x, 0) &= 2 + 2x - x². &&This is now a 1D function. \\ f'(x) &= 2 - 2x = 0 implies x=1. &&Find extreme values along the segment. end{align*}
Evaluating candidates on this segment: f(0,0) = 2, f(1,0) = 3, f(9,0) = -61.

begin{align*} Segment  OB: \, x&=0, \, 0 ≤ y ≤ 9. &&Step 3: Check the left edge. \\ f(0, y) &= 2 + 2y - y². && end{align*}
By symmetry, critical point at y=1.
Evaluating candidates: f(0,1) = 3, f(0,9) = -61.

begin{align*} Segment  AB: \, y &= 9-x, \, 0 ≤ x ≤ 9. &&Step 4: Check the hypotenuse edge. \\ f(x, 9-x) &= 2 + 2x + 2(9-x) - x² - (9-x)² &&Substitute the constraint into  f. \\  &= -61 + 18x - 2x². && \\ (d)/(dx)f(x, 9-x) &= 18 - 4x = 0 implies x = 9/2. &&Find extreme value along hypotenuse. end{align*}
At x=9/2, y=9/2.
f(9/2, 9/2) = -41/2.
...
begin{align*} V(y,z) &= (108 - 2y - 2z)yz &&Step 1: Eliminate x. \\ &= 108yz - 2y²z - 2yz². && \\ V_y &= 108z - 4yz - 2z² = z(108 - 4y - 2z) = 0 &&Step 2: Partial w.r.t y. \\ V_z &= 108y - 2y² - 4yz = y(108 - 2y - 4z) = 0 &&Step 3: Partial w.r.t z. end{align*}
Exclude y=0 or z=0 (these yield zero volume). We solve the simultaneous equations:
begin{align*} 4y + 2z &= 108 &&Step 4: Solve the system. \\ 2y + 4z &= 108 && end{align*}
Subtracting the second from twice the first gives 6y = 108 implies y=18. By symmetry z=18. Then x=108-36-36 = 36.
Checking second derivatives ensures this gives a maximum. Max Volume is V=11664  in³.
...
**Question:** Find and classify critical points of f(x,y)=x²+y²-2x.
...
| "Use the Second Derivative Test (discriminant D)." | f_xx=2, f_yy=2, f_xy=0  D = (2)(2) - 0 = 4 > 0. f_xx>0 implies Local Min. |
...
| If you see dots | Ask yourself dots | Action |
| :--- | :--- | :--- |
| D > 0 and f_xx > 0 | Is it a cup up? | Local Minimum. |
| D > 0 and f_xx < 0 | Is it a cup down? | Local Maximum. |
...
`
  },
  {
    ...
    subheadings: [
      ...
      { id: "ch14-8-cheatsheet", title: "Ultimate Exam Room Cheat Sheet" },      { id: "ch14-8-visuals", title: "Visuals / Graphs" },
...
Suppose that f(x, y, z) is differentiable in a region whose interior contains a smooth curve C parametrized by mathbf{r}(t). If P_0 is a point on C where f has a local maximum or minimum relative to its values on C, then ∇ f is orthogonal to C at P_0.

**Corollary to Theorem 12:**
At the points on a smooth curve mathbf{r}(t) where a differentiable function f(x,y) takes on its local maxima and minima relative to its values on the curve, ∇ f • mathbf{v} = 0, where mathbf{v} = dmathbf{r}/dt.

### 2. The Method of Lagrange Multipliers
Suppose that f(x, y, z) and g(x, y, z) are differentiable. To find the local maximum and minimum values of f subject to the constraint g(x, y, z) = 0, find the values of x, y, z, and λ that simultaneously satisfy the equations:
∇ f = λ ∇ g quad and quad g(x, y, z) = 0
For functions of two independent variables, the condition is similar, but without the variable z. The scalar λ is called a **Lagrange multiplier**.

### 3. The Geometry of the Solution
Imagine the level curves f(x,y) = c. We want to find the extreme values of f(x,y) given that the point (x,y) also lies on a constraint curve g(x,y) = 0. The extremum occurs when the level curve of f just grazes (is tangent to) the constraint curve g. At the point of tangency, their normal vectors (the gradients) are parallel. Thus ∇ f = λ ∇ g.

### 4. Lagrange Multipliers with Two Constraints
Many problems require us to find the extreme values of a differentiable function f(x, y, z) whose variables are subject to two constraints. If the constraints are g_1(x, y, z) = 0 and g_2(x,y,z) = 0, and ∇ g_1 is not parallel to ∇ g_2, we introduce two Lagrange multipliers λ and μ. We locate the points P(x,y,z) where f takes on its constrained extreme values by finding values that satisfy:
∇ f = λ ∇ g_1 + μ ∇ g_2
...
## ELI5 Explanation
Imagine you are walking along a winding path (the constraint g(x,y)=0) on the side of a mountain f(x,y). You want to find the highest point on your path. The rule of Lagrange multipliers says: at the exact highest (or lowest) point on your path, the direction pointing straight up the mountain (the gradient ∇ f) will be perpendicular to your path. Why? Because if the up-mountain direction tipped even slightly forward or backward along your path, you could just step that way to get higher!
...
**Thought Process:**
Instead of minimizing the true distance d = √(x²+y²+z²), we can minimize the square of the distance to avoid messy derivatives. Let f(x,y,z) = x²+y²+z². The constraint is the plane g(x,y,z) = 2x+y-z-5 = 0.
...
**Solution:**
begin{align*} f(x,y,z) &= x² + y² + z² &&Step 1: Define objective function. \\ g(x,y,z) &= 2x + y - z - 5 = 0 &&Step 2: Define constraint function. \\ ∇ f &= ⟨ 2x, 2y, 2z ⟩ &&Step 3: Calculate gradient of f. \\ ∇ g &= ⟨ 2, 1, -1 ⟩ &&Step 4: Calculate gradient of g. end{align*}

Set ∇ f = λ ∇ g:
begin{align*} 2x &= 2λ implies x = λ &&Step 5: Equate components. \\ 2y &= λ implies y = λ/2 && \\ 2z &= -λ implies z = -λ/2 && end{align*}

Substitute into the constraint 2x+y-z-5=0:
begin{align*} 2(λ) + (λ/2) - (-λ/2) - 5 &= 0 &&Step 6: Substitute components into constraint. \\ 2λ + λ - 5 &= 0 implies 3λ = 5 implies λ = 5/3 &&Solve for  λ. end{align*}

Thus x = 5/3, y = 5/6, z = -5/6. The closest point is (5/3, 5/6, -5/6).

### Example 2: Using the Method of Lagrange Multipliers
**Question:** Find the greatest and smallest values that the function f(x, y) = xy takes on the ellipse (x²)/(8) + (y²)/(2) = 1.

**Thought Process:**
Set ∇ f = λ ∇ g and g = 0. This will generate a system of nonlinear equations. Finding valid solutions requires exploring different cases (like separating when coordinates can be zero or non-zero).

*Sketching Guide for student:* Draw the xy-axes. Draw an ellipse passing through x = pm √(8) ≈ ± 2.8 and y = pm √(2) ≈ ± 1.4. The level curves of f(x,y)=xy are hyperbolas in quadrants 1/3 (positive values) and quadrants 2/4 (negative values). We look for where these hyperbolas just touch the ellipse!

**Solution:**
begin{align*} ∇ f &= ymathbf{i} + xmathbf{j} &&Step 1: Compute gradients. \\ ∇ g &= (x)/(4)mathbf{i} + ymathbf{j} && \\ y &= (λ x)/(4) &&Step 2: Set  ∇ f = λ ∇ g. \\ x &= λ y && end{align*}

Now, solve the system. Substitute x = λ y into the first equation:
begin{align*} y &= (λ)/(4) (λ y) = (λ²)/(4) y &&Step 3: Solve the system. end{align*}
If y = 0, then x = 0, but (0,0) does not satisfy the ellipse constraint 0²/8 + 0²/2 ≠q 1.
So y ≠q 0. This means we can divide by y:
1 = (λ²)/(4) implies λ² = 4 implies λ = ± 2.

Case 1: λ = 2 implies x = 2y.
Substitute into the ellipse constraint:
begin{align*} ((2y)²)/(8) + (y²)/(2) &= 1 &&Step 4: Plug substitution into constraint. \\ (4y²)/(8) + (y²)/(2) &= 1 implies (y²)/(2) + (y²)/(2) = 1 implies y² = 1 implies y = ± 1 && end{align*}
Points: (2, 1) and (-2, -1). f at these points equals 2.

Case 2: λ = -2 implies x = -2y.
Substitute into constraint:
((-2y)²)/(8) + (y²)/(2) = 1 implies y² = 1 implies y = ± 1.
Points: (-2, 1) and (2, -1). f at these points equals -2.

Extrema: **Maximum is 2**, **Minimum is -2**.

### Example 3: Finding Extreme Function Values on a Circle
**Question:** Find the max and min of f(x,y) = 3x + 4y on the circle x² + y² = 1.

**Thought Process:**
This is classic constrained optimization. We need ∇ f = λ ∇ g. The gradients will be very simple linear expressions.
...
**Solution:**
begin{align*} ∇ f &= ⟨ 3, 4 ⟩ &&Step 1: Compute gradients. \\ ∇ g &= ⟨ 2x, 2y ⟩ && \\ 3 &= 2λ x implies x = (3)/(2λ) &&Step 2: Equate components. \\ 4 &= 2λ y implies y = (2)/(λ) && end{align*}

Substitute into x²+y²=1:
begin{align*} ≤ft((3)/(2λ)right)² + ≤ft((2)/(λ)right)² &= 1 &&Step 3: Solve constraint for lambda. \\ (9)/(4λ²) + (4)/(λ²) &= 1 implies (9+16)/(4λ²) = 1 implies 25 = 4λ² implies λ = ± (5)/(2) && end{align*}

Case 1: λ = 5/2 implies x = 3/5, y = 4/5. 
f(3/5, 4/5) = 3(3/5) + 4(4/5) = 9/5 + 16/5 = 25/5 = 5.
Case 2: λ = -5/2 implies x = -3/5, y = -4/5. 
f(-3/5, -4/5) = -9/5 - 16/5 = -5.

**Maximum is 5**, **Minimum is -5**.

### Example 4: Extremes of Distance on an Ellipse (Two Constraints)
**Question:** The plane x+y+z=1 cuts the cylinder x²+y²=1 in an ellipse. Find the points on the ellipse closest to and farthest from the origin.

**Thought Process:** We want to optimize f(x,y,z) = x²+y²+z² subject to g_1 = x²+y²-1=0 and g_2 = x+y+z-1=0. This requires the two constraint formula ∇ f = λ ∇ g_1 + μ ∇ g_2.
...
**Solution:**
begin{align*} ∇ f &= ⟨ 2x, 2y, 2z ⟩ &&Step 1: Compute gradients. \\ ∇ g_1 &= ⟨ 2x, 2y, 0 ⟩ && \\ ∇ g_2 &= ⟨ 1, 1, 1 ⟩ && end{align*}

∇ f = λ ∇ g_1 + μ ∇ g_2:
begin{align*} 2x &= 2λ x + μ &&Step 2: Setup system of equations. \\ 2y &= 2λ y + μ && \\ 2z &= μ && end{align*}

Subtract equations 1 and 2:
begin{align*} 2x - 2y &= 2λ x - 2λ y \\ 2(x-y) &= 2λ(x-y) implies 2(x-y)(1-λ) = 0. &&Step 3: Exploit symmetry to find cases. end{align*}
This gives cases x=y or λ=1.

Case 1: x=y.
Using g_1: x²+x²=1 implies 2x²=1 implies x=± (1)/(√(2)). Thus y = ± (1)/(√(2)).
Using g_2: z = 1 - x - y = 1 - 2x.
Points: P_1((1)/(√(2)), (1)/(√(2)), 1-√(2)) and P_2(-(1)/(√(2)), -(1)/(√(2)), 1+√(2)).

Case 2: λ = 1.
Then 2x = 2x + μ implies μ=0.
Then from 2z = μ, z = 0.
Substitute into g_2: x+y+0=1 implies y=1-x.
Substitute into g_1: x²+(1-x)²=1 implies x²+1-2x+x²=1 implies 2x²-2x=0 implies 2x(x-1)=0.
So x=0 or x=1.
Produces points P_3(0,1,0) and P_4(1,0,0).

Now, compute distances (d² = x²+y²+z²) for these points.
P_1, P_2: d² = 1 + (1 ∓ √(2))² = 4 ∓ 2√(2). (approx 6.82 and 1.17).
P_3, P_4: d² = 1.

Closest points: **(0,1,0) and (1,0,0)**. Farthest point: **(-(1)/(√(2)), -(1)/(√(2)), 1+√(2))**.
...
**Question:** Minimize f(x,y)=x²+y² subject to x+y=2.

| Inner voice | What you write |
| :--- | :--- |
| "Set up the Lagrange equation: gradients must be parallel. abla f = λ abla g." | ⟨ 2x, 2y ⟩ = λ ⟨ 1, 1 ⟩ |
| "We get 2x=λ, 2y=λ, so x=y. Plug this back into the constraint." | x+x=2 implies x=1, y=1 |
| "The constrained minimum is just the function value there." | f(1,1) = 1²+1² = 2 |
...
| If you see dots | Ask yourself dots | Action |
| :--- | :--- | :--- |
| "Subject to..." | Is it an equality constraint? | Use abla f = λ abla g. |
...
`
  },
  {
    ...
    subheadings: [
      ...
      { id: "ch14-9-cheatsheet", title: "Ultimate Exam Room Cheat Sheet" },      { id: "ch14-9-visuals", title: "Visuals / Graphs" },
...
We now explain that sometimes variables are not universally independent (e.g., ideal gas law PV=nRT). The notation ((∂ w)/(∂ x))_y directly specifies actively which variable terms are logically held securely constant.
...
- **Notation ((∂ w)/(∂ x))_y:** Explicit constraint mapping derivative w.r.t x while holding y constant, treating x and y as the independent variables.
...
- Given equations relating variables, we choose which are considered independent. Then the partial derivative notation includes the fixed independent variables as subscripts, e.g., ((∂ w)/(∂ x))_y,z means x,y,z are independent and y,z are held constant during the x derivative.
...
* **Problem:** w=x²+y²+z², z=x²+y². Find (∂ w / ∂ x)_y. (Textbook 14.9 Ex 1)
* **Thinking Process:** Independent vars are x,y. Eliminate z!
* **Step-by-Step Solution:** 
  1. w = x²+y²+(x²+y²)².
  2. Differentiating w.r.t x: 2x + 2(x²+y²)(2x) = 2x + 4x³ + 4xy².

**Exercise 2: Alternative Constraint**
* **Problem:** w=x²+y²+z², z=x²+y². Find (∂ w / ∂ x)_z.
...
* **Problem:** For a resistor, V=IR, and R=R_0(1+α(T - T_0)). Find (∂ V/∂ I)_T.
* **Thinking Process:** Treat I and T as independent.
* **Step-by-Step Solution:** 
  1. Substitute R: V = I R_0(1+α(T - T_0)).
  2. (∂ V/∂ I)_T = R_0(1+α(T - T_0)).
...
**Question:** Given F(x,y,z)=0, find (∂ z / ∂ x)_y.

| Inner voice | What you write |
| :--- | :--- |
| "When we have an implicit surface and want a partial derivative, we use the negative ratio of partials." | (∂ z / ∂ x)_y = -(F_x)/(F_z) |
...
| Subscript notation | (∂ w / ∂ x)_y | y is explicitly held constant. |
...
`
  },
  {
    ...
    subheadings: [
      ...
      { id: "ch15-1-cheatsheet", title: "Ultimate Exam Room Cheat Sheet" },      { id: "ch15-1-visuals", title: "Visuals / Graphs" },
...
The double integral is defined as the limit of Riemann sums over small rectangular areas Δ A_k:
iint_R f(x,y) dA = lim _||P|| → 0 sum_k=1^n f(x_k,y_k) Δ A_k

### 2. Fubini's Theorem (First Form)
If f(x, y) is continuous throughout the rectangular region R: a ≤ x ≤ b, c ≤ y ≤ d, then the double integral can be calculated as an **iterated integral** in either order:
iint_R f(x, y) dA = ∫_c^d ∫_a^b f(x, y) , dx , dy = ∫_a^b ∫_c^d f(x, y) , dy , dx
...
1. If R is defined by a ≤ x ≤ b, g_1(x) ≤ y ≤ g_2(x):
   iint_R f(x,y) dA = ∫_a^b ∫_g_1(x)^g_2(x) f(x,y) , dy , dx
2. If R is defined by c ≤ y ≤ d, h_1(y) ≤ x ≤ h_2(y):
   iint_R f(x,y) dA = ∫_c^d ∫_h_1(y)^h_2(y) f(x,y) , dx , dy
...
- **Sum and Difference:** iint_R (f ± g) dA = iint_R f dA ± iint_R g dA
- **Domination:** If f(x,y) ≥ g(x,y) on R, then iint f dA ≥ iint g dA.
- **Additivity:** If R is union of two non-overlapping regions R_1, R_2, then iint_R = iint_R_1 + iint_R_2.
...
**Question:** Calculate iint_R f(x,y) dA for f(x,y)=1-6x²y and R: 0 ≤ x ≤ 2, -1 ≤ y ≤ 1.

**Thought Process:** 
Since the bounds are constants, Fubini allows evaluating in any order. The bounds of integration are -1 → 1 for y, and 0 → 2 for x. I will choose to evaluate dx\,dy.
...
**Solution:**
begin{align*} iint_R f(x,y) dA &= ∫_-1^1 ∫_0² (1 - 6x²y) \,dx \,dy &&Step 1: Set up iterated integral. \\ &= ∫_-1^1 ≤ft[ x - 2x³y right]_x=0^x=2 \,dy &&Step 2: Antiderivative of  x  and  x²  w.r.t  x. \\ &= ∫_-1^1 (2 - 2(8)y - (0)) \,dy &&Step 3: Evaluate bounds at  x=2. \\ &= ∫_-1^1 (2 - 16y) \,dy &&Step 4: Cleanup remaining integral. \\ &= ≤ft[ 2y - 8y² right]_-1^1 &&Step 5: Antiderivative w.r.t y. \\ &= (2 - 8) - (-2 - 8) = -6 - (-10) = 4 &&Step 6: Evaluate final bounds. end{align*}
...
**Solution:**
begin{align*} V &= ∫_0^1 ∫_0^x (3 - x - y) \,dy \,dx &&Step 1: Setup integral based on boundary sketches. \\ &= ∫_0^1 ≤ft[ 3y - xy - (y²)/(2) right]_y=0^y=x \,dx &&Step 2: Take antiderivative w.r.t y. \\ &= ∫_0^1 ≤ft( 3x - x² - (x²)/(2) right) \,dx &&Step 3: Evaluate the bounds. \\ &= ∫_0^1 ≤ft( 3x - (3x²)/(2) right) \,dx &&Step 4: Combine generic terms. \\ &= ≤ft[ (3x²)/(2) - (x³)/(2) right]_0^1 &&Step 5: Process standard antiderivative. \\ &= (3)/(2) - (1)/(2) = 1 &&Step 6: Final evaluation. end{align*}

### Example 3: Reversing the Order of Integration
**Question:** Calculate iint_R (sin  x)/(x) dA, where R is the triangle bounded by the x-axis, y=x, and x=1.

**Thought Process:**
This problem illustrates a key technique. ∫ (sin  x)/(x) dx famously has no elementary antiderivative. It is mathematically unsolvable in a simple way! Therefore, we MUST integrate with respect to y first. As defined in Example 2, the triangle has bounds y=0 to y=x. 

**Solution:**
begin{align*} V &= ∫_0^1 ∫_0^x (sin  x)/(x) \,dy \,dx &&Step 1: Write solvable order  dy dx. \\ &= ∫_0^1 ≤ft[ y (sin  x)/(x) right]_y=0^y=x \,dx &&Step 2: Integrate w.r.t y treating x as constant. \\ &= ∫_0^1 ≤ft( x (sin  x)/(x) right) \,dx &&Step 3: Evaluate limits. \\ &= ∫_0^1 sin  x \,dx &&Step 4: Miraculously, the  x  cancels out!. \\ &= [-cos  x]_0^1 = -cos (1) - (-1) &&Step 5: Antiderivative and substitution. \\ &= 1 - cos (1) ≈ 0.46 &&Step 6: Solved. end{align*}

### Example 4: Changing Bounds when Reversing Order
**Question:** Sketch the region of integration for ∫_0² ∫_x²^2x (4x+2) dy dx, and write an equivalent integral with the order reversed.

**Thought Process:**
The existing integral says: y goes from y=x² (a parabola) up to y=2x (a line). x goes from 0 to 2. Let's find their intersections: x² = 2x implies x=0, 2. Matches perfectly.
To reverse bounds to dx dy: we need x as horizontal strips. The left boundary is the line x = y/2. The right boundary is the parabola x = √(y). The y-axis range is from 0 to 4 (since y=2x, 2(2)=4).

*Sketching Guide for student:* Plot parabola y=x² and line y=2x. Shade the crescent moon shape trapped between them. A horizontal sweeping line enters at the straight line (x = y/2) and exits at the parabola wall (x = √(y)).

**Solution:**
begin{align*} New Integral &= ∫_0^4 ∫_y/2^√(y) (4x+2) \,dx \,dy &&Step 1: Invert bounds logically. \\ &= ∫_0^4 ≤ft[ 2x² + 2x right]_x=y/2^x=√(y) \,dy &&Step 2: Evaluate inner dx. \\ &= ∫_0^4 ≤ft( (2y + 2√(y)) - (2(y²/4) + y) right) \,dy &&Step 3: Plug in new horizontal bounds. \\ &= ∫_0^4 ≤ft( y + 2y^1/2 - (y²)/(2) right) \,dy &&Step 4: Algebraic simplification. \\ &= ≤ft[ (y²)/(2) + (4)/(3)y^3/2 - (y³)/(6) right]_0^4 &&Step 5: Integrate. \\ &= 8 + (4)/(3)(8) - (64)/(6) = 8 + (32)/(3) - (32)/(3) = 8 &&Step 6: Solution. end{align*}
...
**Question:** Reverse the bounds of ∫_0^1 ∫_x^1 e^y² dy dx.

| Inner voice | What you write |
| :--- | :--- |
| "I can't integrate e^y² wrt y. The given bounds mean: y goes from x → 1; x goes from 0 → 1." | Draw triangle: Vertices (0,0), (1,1), (0,1). |
| "Reverse order: shoot horizontal arrows. x goes from left (0) to right (y). y goes 0 → 1." | ∫_0^1 ∫_0^y e^y² dx dy = dots = (1)/(2)(e-1) |
...
| Type I Region | Top/bottom curves? | ∫_a^b ∫_g_1(x)^g_2(x) |
...
`
  },
  {
    ...
    subheadings: [
      ...
      { id: "ch15-2-cheatsheet", title: "Ultimate Exam Room Cheat Sheet" },      { id: "ch15-2-visuals", title: "Visuals / Graphs" },
...
Average value of  f  over  R = (1)/(area of ) R iint_R f dA

### 3. Mass and First Moments for Thin Flat Plates
Let δ(x, y) be the continuous density function (mass per unit area) of a thin flat plate covering a region R. 
- **Mass M:** M = iint_R δ(x, y) dA
- **First moment about x-axis M_x:** M_x = iint_R y δ(x, y) dA
- **First moment about y-axis M_y:** M_y = iint_R x δ(x, y) dA
- **Center of mass (bar{x}, bar{y}):** bar{x} = (M_y)/(M), quad bar{y} = (M_x)/(M)
...
- **About the x-axis I_x:** I_x = iint_R y² δ(x, y) dA
- **About the y-axis I_y:** I_y = iint_R x² δ(x, y) dA
- **Polar Moment of Inertia (about origin) I_0:** I_0 = iint_R (x²+y²)δ(x,y) dA = I_x + I_y
...
If the density δ(x,y) is perfectly constant across the plate, it cleanly cancels out of the center of mass fraction! In engineering, when the density is constant and uniform, we just call the geographic balance point the **Centroid** of the geometric shape. We simply set δ=1 to calculate it.

---

## ELI5 Explanation

Imagine spinning a pencil. If you tape two coins close to the center, it spins easily. If you tape the coins fully at the outer ends, it is very hard to start spinning! This is "Moment of Inertia" – it measures how mass is distributed structurally relative to the axis, calculated mathematically by a double integral multiplying density δ by squared radius r². The "centroid" is exactly where you put your finger underneath a weird cardboard shape to make it flawlessly balance without tipping.
...
**Thought Process:** 
I can just set up a double integral with f(x,y) = 1. The vertical strips move from bottom curve y=x² to top curve y=x. Horizontal bounds are intersections: x = x² implies x=0, 1.

*Sketching Guide for student:* Draw the linear line y=x rising 45 degrees. Draw the curved parabola bowl y=x² dipping below the line initially. They intersect at (0,0) and exactly at (1,1). The sliver area between them is our region R. 

**Solution:**
begin{align*} A &= ∫_0^1 ∫_x²^x 1 \,dy \,dx &&Step 1: Set up double integral with geometric bounds. \\ &= ∫_0^1 [y]_y=x²^y=x \,dx &&Step 2: Antiderivative of 1 is just y. \\ &= ∫_0^1 (x - x²) \,dx &&Step 3: Evaluate bounded expressions. \\ &= ≤ft[ (x²)/(2) - (x³)/(3) right]_0^1 &&Step 4: Standard power rule antiderivative. \\ &= (1)/(2) - (1)/(3) = (1)/(6) &&Step 5: Calculate fraction algebra. end{align*}

### Example 2: Finding Average Value
**Question:** Find the average value of f(x,y) = x cos (xy) over the rectangle R: 0 ≤ x ≤ π, 0 ≤ y ≤ 1.

**Thought Process:**
Calculate area of R. Then evaluate iint_R f(x,y) dA. Divide.

*Sketching Guide for student:* Draw a simple rectangle from 0 to π on the x-axis and 0 to 1 on the y-axis.

**Solution:**
begin{align*} Area of  R &= π × 1 = π &&Step 1: Area calculation. \\ iint_R f dA &= ∫_0^π ∫_0^1 x cos (xy) \,dy \,dx &&Step 2: Setup Double Integral. \\ &= ∫_0^π ≤ft[ sin (xy) right]_0^1 \,dx &&Step 3: Integrate w.r.t y. Note  x  serves as inner coefficient. \\ &= ∫_0^π sin (x) \,dx &&Step 4: Substitute limits  y=1  and  0. \\ &= ≤ft[ -cos  x right]_0^π = -cos (π) - (-cos (0)) = 1 + 1 = 2 &&Step 5: Integrate and solve. end{align*}

Average Value = Integral / Area = 2/π.

### Example 3: Finding the Center of Mass of Variable Density
**Question:** A thin plate covers the triangular region bounded by the x-axis and the lines x=1 and y=2x. Density is δ(x,y) = 6x + 6y + 6. Find the mass and center of mass.

**Thought Process:**
Calculating geometric moments means resolving iint δ, iint y δ, and iint x δ thoroughly. For bounds, x runs 0 → 1. y vertical strips run from 0 → 2x. 

**Solution:**
begin{align*} M &= ∫_0^1 ∫_0^2x (6x + 6y + 6) \,dy \,dx &&Step 1: Set up integral for Mass  M. \\ &= ∫_0^1 ≤ft[ 6xy + 3y² + 6y right]_y=0^y=2x \,dx &&Step 2: Integrate w.r.t y using power rule. \\ &= ∫_0^1 (12x² + 12x² + 12x) \,dx = ∫_0^1 (24x² + 12x) \,dx &&Step 3: Substitute limits. \\ &= ≤ft[ 8x³ + 6x² right]_0^1 = 14 &&Step 4: Final Mass  M = 14. end{align*}

begin{align*} M_x &= ∫_0^1 ∫_0^2x y(6x + 6y + 6) \,dy \,dx = ∫_0^1 ∫_0^2x (6xy + 6y² + 6y) \,dy \,dx &&Step 5: Setup first moment  M_x  (multiplier y). \\ &= ∫_0^1 ≤ft[ 3xy² + 2y³ + 3y² right]_y=0^y=2x \,dx &&Step 6: Integrate y. \\ &= ∫_0^1 (12x³ + 16x³ + 12x²) \,dx = ∫_0^1 (28x³ + 12x²) \,dx &&Step 7: Substitute limits. \\ &= ≤ft[ 7x^4 + 4x³ right]_0^1 = 11 &&Step 8: Final Moment  M_x = 11. end{align*}

begin{align*} M_y &= ∫_0^1 ∫_0^2x x(6x + 6y + 6) \,dy \,dx &&Step 9: Setup first moment  M_y  (multiplier x). \\ &= ∫_0^1 ≤ft[ 6x²y + 3xy² + 6xy right]_0^2x \,dx &&Step 10: Solve. \\ &= ∫_0^1 (12x³ + 12x³ + 12x²) \,dx = ∫_0^1 (24x³ + 12x²) \,dx && \\ &= [6x^4 + 4x³]_0^1 = 10. && end{align*}
...
| Inner voice | What you write |
| :--- | :--- |
| "The total mass M is the double integral of the density over the region." | M = iint_R δ(x,y) dA |
| "The x-coordinate of the COM requires multiplying the density by x (lever arm)." | bar{x} = (1)/(M) iint_R x δ(x,y) dA |
...
| If you see dots | Ask yourself dots | Action |
| :--- | :--- | :--- |
| "Center of mass" | Do I need x lever? | bar{x} uses integral of x δ. |
| "Moment of inertia" | Distance squared? | I_y uses integral of x² δ. |
...
**Question:** Set up the integral for the volume enclosed by z = x²+y² and z = 4.

| Inner voice | What you write |
| :--- | :--- |
| "The volume is a triple integral of 1 dV. Inner bounds for z go from floor (paraboloid) to roof (4)." | 0 ≤ x²+y² ≤ z ≤ 4 |
| "The shadow on the xy-plane is the intersection x²+y²=4, a circle. I can use polar components here later." | V = iint_D ≤ft[ ∫_x²+y²^4 dz right] dA |
  ...
  <div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
      ...
      <div class="bg-white border-l-4 border-rose-400 shadow-sm p-5 rounded-r-xl">
        <h4 class="font-bold text-slate-800 mb-2">The Gradient (∇ f)</h4>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
      <div>
        ...
        <ul class="space-y-3">
          ...
          <li class="flex items-start"><span class="w-2 h-2 rounded-full bg-rose-500 mt-2 mr-3 shrink-0"></span><span class="text-slate-700">Check parabolic paths (y = x² or x = y²) to force denominators to match.</span></li>
        </ul>
      </div>
      <div>
        ...
        <ul class="space-y-3">
          ...
          <li class="flex items-start"><span class="w-2 h-2 rounded-full bg-emerald-500 mt-2 mr-3 shrink-0"></span><span class="text-slate-700"><strong>Factoring:</strong> Try to cancel the bad (x²+y²) terms.</span></li>
          <li class="flex items-start"><span class="w-2 h-2 rounded-full bg-emerald-500 mt-2 mr-3 shrink-0"></span><span class="text-slate-700"><strong>Polar Coordinates:</strong> Substitute x = rcos (θ), y = rsin (θ). Prove the limit works as r \→ 0, independent of θ.</span></li>
      ...
      <table class="w-full text-left border-collapse bg-white">
        ...
        <tbody class="text-sm">
          ...
          <tr class="hover:bg-slate-50 transition-colors border-b border-slate-100">
            ...
            <td class="p-4 text-slate-600 border-l border-slate-100">When given z = f(x,y). Here, ∇ f(x_0,y_0) gives the slopes.</td>
          </tr>
          <tr class="hover:bg-slate-50 transition-colors">
            ...
            <td class="p-4 text-slate-600 border-l border-slate-100">When given F(x,y,z) = c. Here ∇ F is the NORMAL vector!</td>
          </tr>
        </tbody>
      </table>
    </div>
  ...
  <div>
    ...
    <div class="mb-6">
      <div class="bg-indigo-50 border border-indigo-200 p-5 rounded-xl text-center shadow-sm">
        ...
        <p class="font-mono text-2xl text-indigo-800">D = f_xxf_yy - (f_xy)²</p>
      </div>
    </div>
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
      ...
      <div class="border-2 border-sky-200 rounded-xl overflow-hidden shadow-sm bg-white hover:border-sky-300 hover:shadow-md transition-all">
        ...
        <div class="p-6 text-center">
          ...
          <div class="inline-block bg-sky-50 px-3 py-1 rounded-full text-sky-700 font-mono text-sm mb-3">and f_xx < 0</div>
          <p class="text-slate-600 text-sm leading-relaxed">It's a localized hill! The cross-sections agree (D>0), and it opens completely <strong>downwards</strong> (f_xx < 0).</p>
        </div>
      </div>
      ...
      <div class="border-2 border-emerald-200 rounded-xl overflow-hidden shadow-sm bg-white hover:border-emerald-300 hover:shadow-md transition-all">
        ...
        <div class="p-6 text-center">
          ...
          <div class="inline-block bg-emerald-50 px-3 py-1 rounded-full text-emerald-700 font-mono text-sm mb-3">and f_xx > 0</div>
          <p class="text-slate-600 text-sm leading-relaxed">It's a localized valley! The cross-sections agree (D>0), and it opens completely <strong>upwards</strong> (f_xx > 0).</p>
        </div>
      </div>
