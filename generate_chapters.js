...
* **Figure Reference:** Interactive plot \`ellipse_constraint\`.

**Exercise 2: Distance min**
* **Problem:** Find points on hyperbolic cylinder $x^2-z^2-1=0$ closest to the origin. (Textbook Example 2)
...
* **Problem:** Calculate $\\iint_R (1-6x^2y) dA$ for $0 \\le x \\le 2, -1 \\le y \\le 1$. (Textbook 14.1 Example 1)
* **Step-by-Step Solution:**
...
- Extend to 2D: Moments about completely planar $x$ and $y$ axes via rigorous double integrals!
...
**Watch out for these common mistakes:**
- Mixing up gracefully entirely cleanly $M_x$ (moment explicitly bounding about $x$-axis) completely cleanly mapping natively perfectly efficiently with $M_y$. Crucially, $M_x$ mathematically uses the strictly natively $y$ distance!
...
function buildContent() {
  ...
  // We want to insert our new chapters EXACTLY after the chapter with id: "final-cheat-sheet".
  // Let's find the string \`    id: "final-cheat-sheet",\` and find where its object ends.
  
  // Actually, we can just find where "Course Progress" is for the final cheat sheet, and append after it.
  ...
  if (spliceIndex !== -1) {
    ...
    console.log("data.ts updated successfully by inserting after the array!");
  } else {
    console.log("Failed to find proper insertion point in data.ts.");
  }
}
