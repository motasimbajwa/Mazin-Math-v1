...
async function main() {
    ...
    const chunks = data.split(/(?=\\s*\\{\\s*id:\\s*"chapter-)/);
    ...
    for (let i = 1; i < chunks.length; i++) {
        let chunk = chunks[i];
        let idMatch = chunk.match(/id:\\s*"chapter-(.+?)"/);
        if (!idMatch) {
        ...
        // Match the markdown block robustly
        let mkMatch = chunk.match(/markdown:\\s*\`([\\s\\S]*?)\`\\n\\s*},?/);
        if (!mkMatch) {
            processed.push(chunk);
            continue;
        }
        let originalMarkdown = mkMatch[1];
        let newMarkdown = originalMarkdown;
        
        if (chId === '14-1') {
            console.log("Processing 14.1...");
        ...
        } else if (promptInstructions[chId]) {
            console.log("Processing " + chId + " with Gemini...");
            const p = "You are updating a calculus textbook chapter.\\n" +
                ...
                "Requirements:\\n" +
                "1. Provide an 'ELI5 Explanation' section with an Electrical Engineering (EE) analogy if not present.\\n" +
                "2. In '10 Worked Exercises' or 'Exhaustive Worked Examples', ensure EACH example has '* **Figure Reference:** Figure ...' and '* **Common mistake:** ...' (invent reasonable ones!). DO NOT change the math.\\n" +
                ...
                promptInstructions[chId] + "\\n\\nOriginal Markdown:\\n" + originalMarkdown;
            try {
                const response = await ai.models.generateContent({ model: 'gemini-2.5-pro', contents: p });
                let t = response.text || '';
                t = t.replace(/^\\s*\\`\\`\\`markdown\\n/g, '').replace(/\\n\\`\\`\\`\\s*$/g, '');
                t = t.replace(/^\\s*\\`\\`\\`\\n/g, '').replace(/\\n\\`\\`\\`\\s*$/g, '');
                newMarkdown = t;
            } catch(e) {
                console.error("Failed on " + chId, e.message);
                newMarkdown = originalMarkdown;
            }
        } else {
             console.log("Skipping " + chId);
             newMarkdown = originalMarkdown;
        }

        const newChunk = chunk.replace(/markdown:\\s*\`[\\s\\S]*?\`\\n\\s*(},?)/, 'markdown: `\\n' + newMarkdown + '\\n`\\n    $1');
        processed.push(newChunk);
    }
    
    fs.writeFileSync('src/data.ts', processed.join(''));
    console.log("Done");
}
