...
def update_data():
    ...
    new_14_1_raw = r"""## Introduction: Escaping Flatland
...
"""
    new_14_1_js = new_14_1_raw.replace('\\', '\\\\')

    idx_ch14_1_start = content.find('id: "chapter-14-1"')
    md_start = content.find('markdown: `', idx_ch14_1_start) + len('markdown: `\n')
    md_end = content.find('\n`', md_start)
    
    content = content[:md_start] + new_14_1_js + content[md_end:]

    adds = {
        '14-2': r"""> **QUIZ:** Use polar coordinates to evaluate lim_{(x,y)->(0,0)} (x² y)/(x² + y²).
>
...
<br/>""",
        '14-3': r"""> **QUIZ:** Find f_xy and f_yx for f(x,y) = e^{xy} sin(x+y). Verify Clairaut’s theorem.
>
...
<br/>""",
        '14-4': r"""> **QUIZ:** If w = e^{xy} ln(z), x = t², y = sin t, z = cos t, find dw/dt at t = 0.
>
...
<br/>""",
        '14-5': r"""> **QUIZ:** Find the direction(s) of zero change for f(x,y) = x² - 2xy + y² at (1,1).
>
...
<br/>""",
        '14-6': r"""> **QUIZ:** Find the linear approximation of f(x,y) = √(x² + y²) at (3,4) and use it to estimate √(3.1² + 3.9²).
>
...
<br/>""",
        '14-7': r"""> **QUIZ:** Find all critical points of f(x,y) = x³ + y³ - 3xy and classify them.
>
...
<br/>"""
    ...
    for ch_suffix in ['14-2', '14-3', '14-4', '14-5', '14-6', '14-7']:
        ...
        content = content[:md_end_for_ch] + '\n' + text_to_insert_js + content[md_end_for_ch+len("\n<br/>"):]
